                ...
                if matched_kws:
                    ...
                    for kw in matched_kws:
                        for m in re.finditer(re.escape(kw), content):
                            ...
                            results.append("-" * 20)
                            # only show first match of each keyword to avoid huge logs
                            break
            except Exception as e:
                pass

with open('actual_evidence_chain_results.txt', 'w', encoding='utf-8') as f_out:
    f_out.write('\n'.join(results))
