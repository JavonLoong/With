...
2. \key{流线与迹线与速度场}：在\warn{定常流动中流线与迹线完全重合}，且当地加速度为0，仅含迁移（位变）加速度。自转角速度为涡量的\warn{一半}，是纯运动学量。速度梯度不为零时不一定会变形，如\warn{刚体旋转运动有速度梯度但应变率为零}。\\
3. \key{三大基本方程基本概念}：连续性方程源自质量守恒，粘性与理想流体形式完全相同。动能/动量修正系数因流速不均匀产生，层流修正系数大于紊流。管嘴出流量大于孔口，因收缩断面处存在真空起抽吸作用。渐变流流线近似平行直线，\warn{过流断面上静压分布仍满足静力学规律}（$z+p/\rho g = \text{const}$）。\\
...
4. \key{动力相似律与自模化}：重力主导选用 Fr 相似，粘性主导选用 Re 相似，二者通常冲突。在阻力平方区发生\warn{自模化}时，阻力系数与 Re 无关，此时只需满足几何相似即可。欧拉数 $Eu = \Delta p / (\rho V^2)$ 是压力与惯性力之比，$Eu \gg 1$ 代表压力起主导作用。
}
...
\infotxt{%
1. \key{边界层外界线与排挤}：由名义厚度 $\delta$ 标定的边界层外界线\warn{不是流线}（有外部流体跨越进入）。排挤厚度 $\delta^*$（质量流缺陷）和动量损失厚度 $\theta$（动量流缺陷）\warn{均显著小于名义厚度 $\delta$}。积分求解必须同时使用这三个厚度。\\
2. \key{流动分离与卡门涡街}：高Re绕流在后部减速段存在\warn{逆压梯度}（$dp/dx>0$）；壁面剪力降为零是分离的临界条件。卡门涡街释放频率 $f = St \frac{U}{d}$ 与来流速度 $U$ 成正比，与直径 $d$ 成反比，阻力危机前 $St \approx 0.2$。\\
