...
\usepackage[top=1.15mm,bottom=1.15mm,left=1.55mm,right=1.55mm]{geometry}
...
\setlength{\parskip}{0.02pt}
\setlength{\columnsep}{0.92mm}
\setlength{\columnseprule}{0.08pt}
\setlength{\tabcolsep}{0.32pt}
\setlength{\fboxsep}{0.24pt}
...
\newcommand{\mt}[1]{\vspace{0.12pt}\colorbox{M}{\parbox{\dimexpr\linewidth-2\fboxsep}{\color{white}\bfseries #1}}\par}
\newcommand{\qt}[1]{\vspace{0.04pt}\colorbox{Q}{\parbox{\dimexpr\linewidth-2\fboxsep}{\color{white}\bfseries #1}}\par}
...
\newcommand{\labtag}[3]{{\bfseries\color{#1}\colorbox{#1}{\color{white}\fontsize{2.18}{2.28}\selectfont #2}\,#3}\par}
...
\newcommand{\boxmini}[1]{\colorbox{warnbg}{\begin{minipage}{\dimexpr\linewidth-2\fboxsep\relax}\fontsize{2.56}{2.70}\selectfont #1\end{minipage}}\par}
...
\newenvironment{vfourWrap}{\par\begingroup\fontsize{2.56}{2.70}\selectfont\setlength{\fboxsep}{0.38pt}}{\par\endgroup}
\begin{document}
\fontsize{2.86}{2.98}\selectfont
\begin{center}
{\heiti\fontsize{6.1}{6.35}\selectfont 流体力学期末开卷资料：母题证据链版二十一}\par
{\fontsize{3.18}{3.34}\selectfont v21旧版全量归位校正版：v106/v108/重整版五内容只允许作为母题下的原题、子题、变式、公式链、查表和易错检查出现。}
\end{center}
\vspace{-3.4pt}
\begin{multicols}{3}
...
\ans{\textbf{旧版素材审计归位：}v106/v108的6页内容不单列：管路/静水/CV三块归母题1--3；势流基本流、镜像、圆柱、环量、半圆柱气膜馆归母题4；等熵Ma链、阻塞、最大流量、面积--马赫双支、汞柱反背压、三临界背压、内/外激波归母题5；H--P、Couette、Stokes、壁律、雷诺应力、边界层阻力归母题6--7；速度场、应力张量、量纲相似、水击水波归母题8--9/16。重整版五的21个母题被压成“完整题干+子题/变式+公式链+易错”贴在对应母题后；看到旧版标题时不要翻目录，直接进对应母题的真题链。}
\ans{\textbf{v21回填原则：}旧版里凡是“管路总包、可压概念、壁面律、量纲相似、外绕阻力、短问证明、查表动作、易错检查”的孤立块，都必须在对应母题里变成可落笔链：\textbf{看到什么}、\textbf{第一句写什么}、\textbf{未知量顺序}、\textbf{最后检查什么}。若考试只记住目录：液面管路找1，静水找2，CV受力找3，势流圆柱找4，Laval/激波找5，粘性解析找6，边界层阻力找7，速度应力找8，量纲水击找9，证明解释找10/12/18。}
\qt{子题：旧版标题找不到时，如何把它变成可写过程。}
\ans{不要按“公式名”找；按题目目标量归位：求 $Q,p,H,P$ 多数进母题1；求闸门/浮力进母题2；求支座力/喷流力进母题3；求 $\phi,\psi,\Gamma,C_p,L'$ 进母题4；求 $M,\dot m,p_b$ 或激波进母题5；求 $u(y),Q,\tau$ 进母题6；求 $\delta,C_f,D,C_D$ 进母题7；求 $P\bm n,\nabla\cdot V,\nabla\times V$ 进母题8；求 $\pi$群、水击、水波、Fr 进母题9/16。每次落笔只写四行：识别句；主方程；未知量顺序；最终检查。}
\qt{子题：每道计算题的标准落笔骨架。}
...
\mview{旧版“水池/油箱/管路/泵阀/水轮机综合”全部归本母题。共同骨架不是背公式，而是先选两个截面，写 PVZ：$P/\rho g+V^2/2g+z+$泵头$=$右端 PVZ$+$水轮机头$+$沿程$+$局部。再用 $Q=AV$ 把每段速度统一起来。子题顺序：普通池底放水管 $\to$ 虹吸/吸水泵防汽蚀 $\to$ 并联/串联管路 $\to$ 水轮机/水泵功率 $\to$ 反推粗糙度。}
\ans{\textbf{旧版回填到母题1：}v106/v108中的“水池管路泵阀、孔板文丘里、粗糙度反推、泵入口汽蚀、液面随时间下降、管道受力混合题”都不是新系统。考试统一先编号截面：自由液面 $p=0,V\approx0$；管内 $V=Q/A$；损失 $h_L=\sum(\lambda L/d+\zeta)V^2/(2g)$；若有泵/水轮机只在机械能方程中加 $H_p/H_T$。算完 $Q,V,Re,\lambda$ 后，再分别求压强、安装高度、功率或时间。}
...
\qt{子题：两水槽光滑管和粗糙管反推粗糙度。$D=10$cm、$L=200$m 水平管连两 $20^\circ$C 水槽，水位 $800$m 与 $650$m。问光滑管流量；若粗糙管实际 $Q=100$m$^3$/h，问平均粗糙度。}
...
\symq{$A,A^*,A_t,A_e$}
$A$任意截面面积；$A^*$为该质量流在$M=1$时对应临界面积；$A_t$几何喉部；$A_e$出口。阻塞且喉部临界才有$A_t=A^*$；过激波后总压变，波后$A^*$须重算。
...
\textbf{可压查表/Fanno等截面摩擦管}：空气$\gamma=1.4,R=287$；临界$T^*/T_0=0.833,p^*/p_0=0.528$；$V_{\max}=\sqrt{2c_pT_0}$，$M^*=V/a^*$；$\dot m_{\max}=0.0404A^*p_0/\sqrt{T_0}$；等熵/面积查$M$，正激波查$M_1$，斜激波查$M_{n1}$，PM查$\nu(M)$；Fanno等截面绝热摩擦管查$4fL^*/D$，摩擦使$M\to1$，$T_0$不变、$p_0$降。 
...
\form{静水链：先定自由液面和深度$h$。平面壁$F=\rho gh_cA,\ y_p=y_c+I_c/(y_cA)$；曲面拆$F_H=\rho gh_cA_v,\ F_V=$压力体水重；浮体$F_B=\rho gV_{\rm disp}$；测压管/U管沿同一静止液体等压面逐段写$p+\rho gz=C$。}
\warn{易错：压力中心比形心更深；曲面竖向力不是投影面积压力，是假想压力体重量；浮体稳定看$GM=BM-BG$；表压可为负，绝压不能为负。}
...
\form{CV链：先能量/连续求$Q,V,p$，再取控制体写$\sum\bm F=\sum_{\rm out}\dot m\bm V-\sum_{\rm in}\dot m\bm V$。压力力按外法线推入CV；重力、支座力、壁面对流体力一起列，最后题问管壁/螺栓/支座时取反。功率题用$P=\bm F\cdot\bm U$或$P=\rho gQH$。}
\warn{易错：自由出口表压0但速度不为0；弯管支座力方向常与“流体受力”相反；运动叶片必须用相对速度$W=V-U$进出，功率用绝对动量变化。}
\qt{真题原题：水平喷嘴法兰螺栓受力。水平喷嘴 $D_1=200$mm，$D_2=100$mm，进口绝压 $345$kPa，出口大气 $p_a=103.4$kPa，出口速度 $22$m/s。求固定喷嘴法兰螺栓受力，忽略摩擦。}
...
\mview{旧版“理想不可压无旋势流：基本流、镜像、圆柱、环量”和重整版“圆柱/半圆柱势流，压力积分与升力”合并在这里。通用解题链：先写不可压无旋 $\Rightarrow \nabla^2\phi=\nabla^2\psi=0$，解可线性叠加；再由 $\phi/\psi$ 求速度；再用 Bernoulli 求压力；最后按题意求驻点、流线、压力分布、合力或升力。}
\ans{\textbf{旧版回填到母题4：}“基本流公式表、给速度求$\phi/\psi$、镜像法、点源+均匀流半体、边角绕流、无环量圆柱、圆柱加环量、半圆柱气膜馆、机翼升力”全部按同一条链写：先写总 $\phi$ 或 $\psi$，再求 $v_r,v_\theta$ 或 $u,v$，壁面/物体边界用 $\psi=C$ 或 $v_n=0$，压力用定常 Bernoulli，升力/合力最后积分或直接 $L'=\rho U\Gamma$。}
\qt{真题原题：2022 春期末半圆柱气膜馆势流分析。半圆柱气膜馆半径 $a$，大风正面速度 $V$；求外部速度分布、表面压力分布；$a=5$m，$V=10$m/s，$\rho=1.29$kg/m$^3$ 求升力。}
...
\end{vfourWrap}
\mt{母题5 Laval/拉伐尔喷管、阻塞、正激波、面积-马赫}
\mview{旧版可压缩题全部归本母题：等熵 Ma 链、临界/最大流量、测压计反推背压、面积比求出口两支、三个关键背压、设计工况汞柱、渐缩喷管、给最大流量反求入口 Ma/面积、激波在管内给定位置、正/斜激波、PM 膨胀波、外喷管欠膨胀/过膨胀。共同入口：先找 $p_0,T_0,p_b,A_t,A_e,A^*,M$ 中哪些已知，再决定是“等熵一段算到底”还是“等熵--激波--换新总压--再等熵”。}
\ans{\textbf{旧版回填到母题5：}重整版五里“可压缩基本链、Laval汞柱真题、渐缩喷管最大流量、可压文丘里、正激波、斜激波、PM膨胀、Fanno、背压/出口压辨析、0.528易错、总压下降解释”都必须跟着本母题走。考试先写一句：本题先判等熵段、阻塞和是否跨激波；再按 $M\to p,T,\rho,V,\dot m$ 或 $A/A^*\to M$ 求中间量；遇激波立即换新总压 $p_{02}$。}
\qt{真题原题：Laval/拉伐尔喷管汞柱测压、正激波位置与设计工况。空气过连接两大气源 Laval 喷管。汞柱接喉部和下游气源，$h=15$cm，喉部端水银面低；$T_0=100^\circ$C，$p_0=300$kPa，$A_t=10$cm$^2$，$A_e=30$cm$^2$。求 $p_b$；判断正激波及位置；设计超声速工况汞柱高。}
...
\qt{子题：Laval喷管母题的弱化/面积-马赫/可压缩等熵流完整关系题型链}
\begin{vfourWrap}
...
\qt{子题：等熵流由压力求马赫数与温度。空气源 $T_0=500$K、$p_0=200$kPa，某截面 $p=150$kPa，求 $M,T$。}
\ans{第一步写“无激波/无摩擦/绝热，按等熵段处理”。公式链：$p_0/p=(1+0.2M^2)^{3.5}\Rightarrow M$，再 $T=T_0/(1+0.2M^2)$。检查：这里的 $p_0,T_0$ 是滞止量，$p,T$ 是截面静量。}
\qt{子题：Laval 喷管最大质量流量。大容器 $T_0=500$K、$p_0=300$kPa，喉部 $A_t=0.06$m$^2$，求 $\dot m_{\max}$。}
\ans{第一步写“最大质量流量发生在喉部阻塞，$M_t=1,A_t=A^*$”。空气速算：$\dot m_{\max}=0.0404A^*p_0/\sqrt{T_0}$；若不用速算常数，写 $\dot m=A^*p_0\sqrt{\gamma/(RT_0)}(2/(\gamma+1))^{(\gamma+1)/(2(\gamma-1))}$。}
\qt{子题：拉伐尔喷管出口参数与喉部参数。给 $T_0=350$K、$p_0=10^6$Pa、$A_e=0.001$m$^2$、$p_b=9.3\times10^5$Pa、喉部 $M_t=0.6$，求出口和喉部参数。}
\ans{第一步先判未阻塞：$M_t=0.6<1$，故喉部不是临界，不能写 $A_t=A^*$。喉部链：$T_t=T_0/(1+0.2M_t^2)$，$p_t=p_0/(1+0.2M_t^2)^{3.5}$，$\rho_t=p_t/(RT_t)$，$V_t=M_t\sqrt{\gamma RT_t}$。出口链：若出口压等于背压，先由 $p_0/p_e$ 反求 $M_e$，再求 $T_e,\rho_e,V_e$；最后用 $\dot m_t=\dot m_e$ 反求缺失面积。}
\qt{子题：缩放喷管喉部面积。给 $p_0=10^6$Pa，$T_0=350$K，$A_e=10$cm$^2$，$p_b=9\times10^5$Pa，要求喉部 $M_t=0.5$，求 $A_t$。}
\ans{第一步由出口背压当静压：$p_e=p_b$，用 $p_0/p_e=(1+0.2M_e^2)^{3.5}$ 求 $M_e$，再算出口 $\dot m=A_e p_0\sqrt{\gamma/(RT_0)}M_e(1+0.2M_e^2)^{-3}$。第二步用指定喉部 $M_t=0.5$ 的同一质量流量式反推 $A_t$。检查：$M_t=0.5$ 不是阻塞，不能套 $0.528$。}
\qt{子题：斜激波角和波后参数。二维尖劈顶角 $2\delta=20^\circ$，$M_1=2.5,p_1=85$kPa，$T_1=298$K，求 $\beta,M_2,p_2,T_2$。}
\ans{第一步判“超声速绕凹角/尖劈压缩”，用斜激波，不用 PM。由 $\theta$--$\beta$--$M$ 关系或斜激波表，以 $M_1=2.5,\theta=10^\circ$ 查 $\beta$。再 $M_{n1}=M_1\sin\beta$，按正激波关系求 $M_{n2},p_2/p_1,T_2/T_1$，最后 $M_2=M_{n2}/\sin(\beta-\theta)$。检查：正激波公式只能用法向马赫数。}
\varq{变式：给激波位置而不是给背压}
若题给激波截面$A_s$：
...
\form{N--S删项链：定常、充分发展、单向流时$0=-dp/dx+\mu u''+\rho g_x$；两板积分两次用无滑移/界面连续；圆管$u(r)=\frac{-dp/dx}{4\mu}(R^2-r^2),\ Q=\pi R^4(-dp/dx)/(8\mu),\ \tau_w=R(-dp/dx)/2$；Stokes小球$D=3\pi\mu dV$。}
\warn{易错：层流解析式不能用于入口发展段和湍流；两层Couette界面速度连续且剪应力连续；倾斜板重力项符号由沿流向正方向决定。}
\mview{旧版“管内层流/湍流、壁面律、摩擦系数、圆管湍流雷诺应力速度分布”不要另放，统一贴在本母题后。题干有“充分发展、圆管、两平板、层流、黏度、剪应力、壁律、$u^+,y^+$”时：先写 N--S 简化，再积分；若题干明确湍流或给平均速度/壁面律，则转到时均速度、摩擦速度、Moody/壁律链。}
\ans{\textbf{旧版回填到母题6：}所有“8.6双层Couette、倾斜板、圆管层流、H--P、Stokes沉降、湍流时均、雷诺应力、壁面律、粗糙管压降”都按两条链分流：\textbf{解析链}先假设定常充分发展再从 N--S 积分；\textbf{工程链}先算 $Re$，层流用 $64/Re$ 或 H--P，湍流用 Moody/壁律。不要把管流平均速度 $\bar u$ 当成壁律局部速度 $u(y)$。}
...
\qt{子题：湍流时均分解与雷诺应力。题干有 DNS/RANS/LES、时均速度、脉动速度或雷诺应力张量。}
\chain{先写分解：$u_i=\bar u_i+u_i'$，且 $\overline{u_i'}=0$。把分解代入 N--S 后，非线性对流项产生附加应力 $-\rho\overline{u_i'u_j'}$，这就是雷诺应力。}
\ans{考场短答：DNS 直接算瞬时 N--S，最贵；RANS 解时均方程，出现雷诺应力需湍流模型封闭；LES 直接算大涡，小涡建模。若题问“为什么湍流难”，答：三维非定常、多尺度、非线性，时均后方程不封闭。}
\warn{雷诺应力不是分子黏性应力；它来自速度脉动相关。题目要求工程压损时通常不直接算雷诺应力，而转用 Moody、壁律或经验摩阻系数。}
...
\mview{旧版“边界层厚度、平板层/湍/转捩、外绕球柱阻力、终端速度、氦气球风载、阻力危机、边界层积分法”统一归这里。共同入口：先用 $Re_x$ 或 $Re_L$ 判层流/湍流/混合，再选 $C_f,\delta,\delta^*,\theta$ 或 $C_D$；平板摩擦用湿面积，球/圆柱阻力用迎风面积。}
\qt{真题原题：零攻角平板边界层厚度与单面摩擦阻力。来流 2.5m/s 绕零攻角平板，$L=0.5$m，$b=3$m，求单面摩擦阻力和尾部名义厚度。$Re_{cr}=5\times10^5$。空气：$\rho=1.2,\nu=1.5\times10^{-5}$；水：$\rho=998,\nu=1.005\times10^{-6}$。}
...
\qt{子题：给定边界层速度分布求厚度。线性 $u=Uy/\delta$；三次式 $u=U[3y/(2\delta)-\frac12(y/\delta)^3]$，$y<\delta$，求排挤厚度和动量厚度。}
...
\qt{子题：湍流边界层 1/6 指数律。$u/U=(y/\delta)^{1/6}$，$\tau_w=0.0233\rho U^2(\mu/\rho U\delta)^{1/4}$，求 $\delta/x,C_f$。}
...
\form{运动学/应力链：不可压$\nabla\cdot V=0$；无旋$\nabla\times V=0$；平面流函数$u=\psi_y,\ v=-\psi_x$；势函数$u=\phi_x,\ v=\phi_y$；环量$\Gamma=\oint\bm V\cdot d\bm l$。应力面力$\bm t=P\bm n$，法向$t_n=\bm t\cdot\bm n$，切向$\bm t_t=\bm t-t_n\bm n$。}
\warn{易错：不可压不等于无旋；流函数只自动满足连续，不自动无旋；应力张量乘的是单位法向，圆柱面法向要由几何求；夹角用法向标量和$|\bm t|$。}
...
\qt{子题：旧版速度场 $u=3x^2-3y^2,\ v=-6xy,\ w=0$，求速度势。}
\chain{先判无旋：$\omega_z=v_x-u_y=-6y-(-6y)=0$，所以存在势函数。由 $\phi_x=u$ 积分：$\phi=x^3-3xy^2+f(y)$；再用 $\phi_y=-6xy+f'(y)=v=-6xy$，得 $f'(y)=0$。}
\res{$\phi=x^3-3xy^2+C$。若题问速度势，只要无旋；若题问流函数，还需不可压 $u_x+v_y=6x-6x=0$，再由 $\psi_y=u,\ -\psi_x=v$ 积分。}
\qt{真题原题：不可压缩无旋线性速度场。$u=Ax+By,\ v=Cx+Dy,\ w=0$，不可压缩且无旋，求 $A,B,C,D$ 条件。}
...
\warn{旧版写法“由 $P=-f$ 求体力”容易漏掉散度号。真正用的是 $\nabla\cdot P=-\rho\bm f$；先对每一行求散度，再除以 $-\rho$。}
...
\mview{旧版“量纲/模型试验、水击、水波、明渠、Fr/Re/Ma相似冲突、阻力按无量纲数换算”统一归这里。共同入口：列变量和基本量纲，选重复变量，写 $\pi$ 群；若是模型题，先判断主控准则，不能同时满足时说明取舍。}
\form{$\pi$链：列变量含目标量，数$n$和基本量纲$k$，得$n-k$个$\pi$；选重复变量要量纲独立且不含目标量。阻力常写$F/(\rho V^2L^2)=f(Re,Fr,Ma,\varepsilon/L)$；管压降$\Delta p/(\rho V^2)=f(L/d,\varepsilon/d,Re)$。}
\warn{易错：模型相似先定主导力；自由面/船舶优先$Fr$，管内黏性优先$Re$，高速气体要$Ma$；$Re$和$Fr$通常不能同时满足，要说明部分相似。}
...
\qt{真题原题：水下航行器阻力量纲分析。利用 $\pi$ 定理推求水下航行器阻力 $F$ 的表达式。}
...
\qt{子题：雷诺数准则与傅汝德数准则冲突。为什么 Re 准则和 Fr 准则总是矛盾？}
...
\qt{子题：管内定常流压降的 $\pi$ 定理无量纲方程。$\Delta p$ 与 $d,l,\varepsilon,U,\rho,\mu$ 有关，用 $\pi$ 定理求关系式。}
...
\qt{子题：非定常伯努利与柯西-拉格朗日积分。从无粘、质量力有势、不可压欧拉方程推积分式。}
...
\qt{子题：物质运动及相互作用规律。连续介质假设和守恒方程如何形成流体力学方程。}
...
面积--Ma：$\displaystyle \frac{A}{A^*}=\frac{1}{M}\left[\frac{2}{\gamma+1}\left(1+\frac{\gamma-1}{2}M^2\right)\right]^{(\gamma+1)/(2\gamma-2)}$；空气等熵 $p_0/p=(1+0.2M^2)^{3.5}$。\\
...
\mt{母题11 短问综合题：静水/CV/势流/相似也按完整题链写}
\form{短问链：不要只写结论。每小问按“题干关键词$\to$母题编号$\to$首方程$\to$目标量”四格写。静水写$p=p_0+\rho gh$；CV写$\sum F=\dot m\Delta V$；势流写$\phi/\psi$叠加；相似写$\pi$群或主导准则。}
\warn{易错：短问不是概念仓库，必须落到一个母题；若题目只给文字，也先补“取截面/CV/流线/物体”为对象。}
...
\mt{母题12 守恒律与物质模型概念题：从假设到控制方程}
\form{概念链：连续介质假设$\to$质量/动量/能量守恒$\to$本构关系$\to$控制方程。不可压无粘用Euler，牛顿有粘用N--S；无旋可引入$\phi$，二维不可压可引入$\psi$；湍流时均后出现雷诺应力，工程题用经验闭合。}
\warn{易错：不要把N--S当所有题都要解；多数考试只要说明删项条件。等熵、Bernoulli、H--P、Moody都有适用条件，概念题要写“条件+结果”。}
...
\qt{子题：PM函数入口/PM最大转角/PM膨胀波/小扰动等熵波。}
...
\qt{子题：Laval喷管状态图/Laval喷管状态母题/Laval背压状态树，看到图就写什么。}
...
\form{四题链：粗糙圆管放水$H=(1+\lambda L/D+\sum\zeta)V^2/2g$并迭代$\lambda$；氦气球$D=C_D\rho V^2A/2$与浮力平衡；势流圆柱用偶极子+环量求$V_\theta,p,L'$；Laval用$p^*/p_0,A/A^*,p_{b1,2,3}$判激波。}
...
\mt{母题14 泵吸水/水轮机/虹吸/并联管路工程题}
\form{工程链：泵吸水从水面到泵入口，约束$p_B\ge p_v+\Delta p$；水轮机从上游到下游扣损失求$H_T$，$P=\eta\rho gQH_T$；虹吸最高点压力最低；并联各支路$h_L$相等且$Q=\sum Q_i$。}
\qt{子题：零攻角平板边界层厚度与单面摩擦阻力。题干：来流 $U$ 绕过零攻角平板，长 $L$、宽 $b$，给空气或水的 $\rho,\nu$，求单面摩擦阻力和尾部边界层名义厚度，临界 $Re_{cr}=5\times10^5$。}
...
\mt{母题15 跨章混合题：势流/可压/粘性按条件反推}
\form{混合链：先判是否同一段可用同一方程。势流段用Bernoulli求$p$，可压等熵段用$M$链，激波处分段换$p_0$，粘性段用损失或N--S，边界层用$Re,C_f$。跨模型题按“分段+接口量连续”写。}
...
\qt{子题：斜激波和PM连算：正激波/斜激波/PM 的题干分流。}
...
\qt{子题：Fanno等截面摩擦管。题干给等截面、绝热、有摩擦、管长或极限长度时用。}
\ans{先判入口$M_1$亚/超声，Fanno摩擦使$M\to1$；查Fanno表输入$M_1$读$4fL^*/D,p/p^*,T/T^*,p_0/p_0^*$等。给实际$L$时用$4fL/D$从剩余长度中扣除并反查出口$M_2$。检查：Fanno中$T_0$不变、$p_0$下降，不能按等熵跨整段。}
...
\mt{母题16 孔口文丘里/水击水波/明渠/RTT非常规水力题}
\form{非常规链：孔口/文丘里先连续+Bernoulli，实际乘流量系数；水击$\Delta p=\rho a\Delta V$；水波$\omega^2=gk\tanh kh$，浅水$c=\sqrt{gh}$；明渠用$Fr=V/\sqrt{gh}$；RTT先写系统量变化=CV内变化+通量。}
\warn{易错：孔口压差计要先换$\Delta p$；水击必须考虑液体可压缩/管壁弹性给的波速；明渠临界不是可压缩$M=1$，而是$Fr=1$。}
\mview{旧版“文丘里/孔口/喷嘴--流量测量”“水击/水波/明渠快速入口”“RTT/系统到控制体”“查表动作标准写法”统一归这里。它们不是散知识点：共同写法是先说明为什么普通 Bernoulli 不够，再写替代方程或查表输入输出，最后回到 $Q,\Delta p,\Delta H,F$ 这类答案。}
...
\mt{母题17 测压计/U管/自由涡/外绕阻力零碎计算题}
\form{零碎题链：测压计逐段$p+\rho gz$，不同液体分段加减；U管振荡用水柱整体动量得$\ddot z+(2g/L)z=0$；自由涡$V_\theta=C/r$，强迫涡$V_\theta=\omega r$；外绕阻力$D=C_D\rho U^2A/2$。}
\warn{易错：测压计左右液面高低只说明压力大小，不要符号反；自由涡中心压低，强迫涡自由面为抛物面；外绕迎风面积和摩擦湿面积不同。}
...
\qt{子题：不可压缩无旋线性速度场与势/流函数。题干给 $u(x,y),v(x,y)$ 或线性速度场 $u=Ax+By,\ v=Cx+Dy$。}
...
\form{证明链：先写适用假设，再写微分/积分主方程，再写删项或引入势/流函数，最后给结论和适用范围。例：Bernoulli来自Euler沿流线积分；Cauchy--Lagrange来自无旋非定常Euler；达朗贝尔佯谬来自无粘势流前后压力对称。}
\warn{易错：证明题别只背结论；必须写“假设条件”。跨激波、有粘损失、泵/水轮机、非定常项存在时，普通Bernoulli不能直接用。}
\mview{旧版“若题目是证明/解释题”“老师终课关键词归位”放在这里，但每个概念都必须带方程来源或物理因果。最低过程分格式：定义对象 $\to$ 写守恒/控制方程 $\to$ 删项或比较量级 $\to$ 得结论。}
...
\mt{母题19 单位量级与反查收口：防止最后一步丢分}
\form{收口链：先统一SI，再量级检查。$1{\rm cm^2}=10^{-4}{\rm m^2}$，$1{\rm mm^2/s}=10^{-6}{\rm m^2/s}$，$1{\rm kPa}=10^3{\rm Pa}$，$p/\rho g$是m水头。若结果负压强，先查表压/绝压；若$Re$跨区，换公式；若$M$支路错，回背压判断。}
\warn{易错：效率乘除方向、泵/水轮机符号、局损速度、面积单位、压力表读数、气体温度K、汞柱/水柱密度，是最常见扣分点。}
...
\mt{母题20 高频漏题完整题干链压缩题}
\form{漏题压缩链：把遗漏题也并回母题：管路功率/汽蚀归1，闸门浮体归2，弯管喷嘴归3，源汇圆柱归4，Laval/激波归5，Couette/H--P归6，边界层/阻力归7，应力/速度场归8，量纲/水击归9。每题至少写“识别+首方程+未知顺序+检查”。}
\warn{易错：若某题找不到位置，不要另开公式仓库，先问它要求的是$Q,p,F,M,\delta,D,\pi$还是证明；目标量决定母题。}
\qt{子题：应力张量求任意斜面应力。题干给 $P=(p_{ij})$ 和单位法向 $\bm n$，求斜面应力、法向分量、切向分量、夹角。}
\ans{第一步写 Cauchy 公式：$\bm p_{\bm n}=P\bm n$。第二步法向标量：$p_N=\bm p_{\bm n}\cdot\bm n$；法向向量：$\bm p_N=p_N\bm n$。第三步切向向量：$\bm p_T=\bm p_{\bm n}-\bm p_N$，大小 $p_T=|\bm p_T|$。夹角：$\cos\alpha=p_N/|\bm p_{\bm n}|$。检查：$\bm n$ 必须是单位向量；应力张量对称来自角动量守恒，故 $p_{ij}=p_{ji}$。}
\qt{子题：速度场判连续/有旋/求 $\phi,\psi$。题干给二维 $u(x,y),v(x,y)$，问是否不可压、是否无旋、写流函数/势函数。}
\ans{第一步判不可压：$u_x+v_y=0$。第二步判无旋：$\Omega_z=v_x-u_y=0$。若不可压，令 $u=\psi_y,\ v=-\psi_x$ 积分求 $\psi$；若无旋，令 $u=\phi_x,\ v=\phi_y$ 积分求 $\phi$。两次积分得到的“常数”其实是另一个变量的函数，要用另一速度分量修正。检查：不可压只保证 $\psi$，无旋只保证 $\phi$，不能混写。}
\qt{子题：点源/点汇/点涡/偶极子叠加。题干出现均匀流、源汇、驻点、分界流线、半体、Rankine 体。}
\ans{先写总势函数或流函数，线性叠加。均匀流沿 $x$：$\phi=Ur\cos\theta,\ \psi=Ur\sin\theta$；点源：$\phi=Q\ln r/(2\pi),\ \psi=Q\theta/(2\pi)$；点涡：$\phi=\Gamma\theta/(2\pi),\ \psi=-\Gamma\ln r/(2\pi)$；偶极：$\phi=M\cos\theta/r,\ \psi=-M\sin\theta/r$。求驻点令 $v_r=v_\theta=0$；求边界令 $\psi=C$。检查：半平面壁面题常用镜像，分母可能由 $2\pi$ 变 $\pi$。}
\qt{子题：镜像法壁面势流。题干有无限平壁、点源/点涡靠近壁面，要求满足壁面不穿透。}
\ans{壁面条件是法向速度为0，即壁面是一条流线 $\psi=C$。点源靠近直壁：用同号点源镜像，使壁面法向速度抵消；点涡靠近直壁：用反号点涡镜像，使壁面成为流线。解题只在真实流场半平面取结果，镜像是数学构造。检查：镜像法保证不穿透，不保证无滑移；有粘壁面不能用势流镜像直接算剪应力。}
\qt{子题：U形管水柱自由振荡。题干有等截面U管、总液柱长 $L$、初始液面差 $h_0$，忽略黏性。}
\ans{设右液面相对平衡位移 $z$，左液面为 $-z$，两液面差 $2z$。回复力为 $2\rho gAz$，运动水柱质量为 $\rho AL$，故 $\rho AL\ddot z=-2\rho gAz$，即 $\ddot z+(2g/L)z=0$。初值 $z(0)=h_0/2,\dot z(0)=0$，得 $z=(h_0/2)\cos\sqrt{2g/L}\,t$，周期 $T=2\pi\sqrt{L/(2g)}$。检查：这里不是定常 Bernoulli。}
\qt{子题：圆管近壁层厚度。题干给管径 $d$、截面平均时均速度 $\bar u$、运动黏度 $\nu$，提示 $\bar u/u_\tau=2.5\ln(Ru_\tau/\nu)+1.75$。}
\ans{第一步 $R=d/2$，代提示式试算/迭代 $u_\tau$。第二步黏性底层厚度取 $y^+=5$：$\delta_l=5\nu/u_\tau$；缓冲层上界取 $y^+=30$：$\delta_b=30\nu/u_\tau$；完全湍流核心从 $\delta_b$ 到管心。检查：$\bar u$ 是截面平均速度，$u_\tau=\sqrt{\tau_w/\rho}$ 是摩擦速度，不是最大速度。}
\qt{子题：零攻角平板边界层阻力。题干给 $U,L,b,\nu,\rho,Re_{cr}$，比较空气/水绕流阻力和尾部边界层名义厚度。}
\ans{先算 $Re_L=UL/\nu$，再与 $Re_{cr}$ 比。全层流：$\bar C_f=1.328/\sqrt{Re_L}$，$\delta_L=5L/\sqrt{Re_L}$。若前层后湍：常用 $\bar C_f=0.074/Re_L^{1/5}-1742/Re_L$，$\delta_L\approx0.37L/Re_L^{1/5}$。阻力 $D=\bar C_f(\rho U^2/2)A_w$，平板双面 $A_w=2Lb$。检查：题问单面/双面；水的 $\nu$ 小，$Re$ 大，常更易转捩。}
\qt{子题：量纲分析标准落笔。题干说“试用量纲分析推导阻力/压降/周期关系”。}
\ans{第一行列变量并写量纲；第二行选重复变量，必须量纲独立、覆盖 $M,L,T$、不含待求量；第三行构造 $\pi$ 群。管内压降常写 $\Delta p/(\rho U^2)=f(l/d,\varepsilon/d,Re)$；外绕阻力常写 $F/(\rho U^2L^2)=f(Re)$；自由面船模常写 $Fr=V/\sqrt{gL}$。检查：$\pi$ 群个数 $=n-k$，不能少一个变量。}
\qt{子题：表压/绝压与测压计。题干给大气压、真空度、汞柱/水柱高、入口至少高于汽化压。}
\ans{能量方程和气体状态方程必须用绝压；题目问“表压”最后减 $p_a$。真空度是 $p_a-p_{\rm abs}$。U管测压沿液柱“下加上减”：$\Delta p=\rho_mgh$；若两侧接同一流体且测压液更重，常写 $\Delta p=(\rho_m-\rho)gh$。汽蚀/吸水泵用绝压约束：$p_{\rm in,abs}\ge p_v+\Delta p_{\rm safe}$。}
\qt{子题：泵、水轮机、机械功率符号。题干有泵功率、水轮机输出、扬程、效率。}
\ans{泵给流体加能：能量方程左侧加 $H_p$ 或右侧写 $+H_p$ 作为可用水头；水轮机从流体取能：把 $H_T$ 写在损失/取出项。功率链：泵水力功率 $P_h=\rho gQH_p$，轴功率 $P_{\rm shaft}=P_h/\eta$；水轮机输出 $P_{\rm out}=\eta\rho gQH_T$。检查：问“机械输出”才乘效率，问“水头损失”不乘效率。}
\qt{子题：低速气体能否当不可压。题干给飞机/管道气体速度和声速，问密度变化或压缩性。}
\ans{先算 $Ma=V/a$。若 $Ma<0.3$，常可按不可压近似；若接近或超过 0.3，用等熵关系。等熵空气：$T_0/T=1+0.2M^2$，$p_0/p=(1+0.2M^2)^{3.5}$，$\rho_0/\rho=(1+0.2M^2)^{2.5}$。检查：声速 $a=\sqrt{\gamma RT}$，温度用 K；“响声多久听到”常用相对运动/追及，不是简单距离除飞机速度。}
\qt{子题：激波/膨胀波概念判断。题干问为什么总压下降、为什么膨胀降压、什么时候用 PM。}
\ans{正激波和斜激波是压缩波合并成的不可逆薄层：静压、静温、密度上升，马赫数下降，总温近似不变，总压下降。膨胀波是连续小扰动组成，近似等熵：$p,T,\rho$ 下降，$M,V$ 上升，总压不变。凸角超声速外流用 PM，凹角压缩用斜激波；偏转角太大时斜激波会脱体，不能继续用附体斜激波表。}
\qt{子题：两层 Couette 流。题干给下层厚 $h_1,\mu_1$、上层厚 $h_2,\mu_2$，下板固定、上板速度 $U$，求速度和剪应力。}
\ans{无压差且定常充分发展，故两层速度都是直线。界面条件：速度连续、剪应力连续。把两层看成剪切阻力串联：$U=\tau h_1/\mu_1+\tau h_2/\mu_2$，故 $\tau=U/(h_1/\mu_1+h_2/\mu_2)=U\mu_1\mu_2/(\mu_2h_1+\mu_1h_2)$。速度：$u_1=\tau y/\mu_1$；$u_2=u_1(h_1)+\tau(y-h_1)/\mu_2$。}
\qt{子题：倾斜平板间层流。题干给板间距 $H$、倾角 $\theta$、上板速度 $U_0$、中面速度为0，求 $U_0$ 与壁面剪应力。}
\ans{取沿斜面向下为正，若无压差：$\mu u''+\rho g\sin\theta=0$。设 $A=\rho g\sin\theta/\mu$，积分 $u=-Ay^2/2+C_1y+C_2$。用边界 $u(0)=0$、$u(H/2)=0$、$u(H)=-U_0$ 定常数，得 $U_0=\rho g\sin\theta H^2/(4\mu)$。剪应力 $\tau=\mu u'=\rho g\sin\theta(H/4-y)$，下壁 $|\tau_0|=\rho gH\sin\theta/4$，上壁 $|\tau_H|=3\rho gH\sin\theta/4$。}
\qt{子题：并联/分支管路。题干有两条支路连接同两节点，给总流量或水头差，求各支路流量。}
\ans{节点间并联的核心不是“损失相加”，而是各支路两端总水头差相同：$h_{L,a}(Q_a)=h_{L,b}(Q_b)$，且节点连续 $Q=Q_a+Q_b$。每支路单独算 $V_i=Q_i/A_i$、$Re_i$、$\lambda_i$、$h_{L,i}=(\lambda_iL_i/d_i+\sum\zeta_i)V_i^2/(2g)$。若层流，$h_L\propto Q$；若湍流且 $\lambda$ 近似定，$h_L\propto Q^2$。检查：并联损失相等，串联损失相加；不同直径局部损失速度取本支路速度。}
\qt{子题：虹吸管/最高点压强。题干有水池、管越过高点、出口低于水面，问能否虹吸或最高点高度。}
\ans{先从上游自由液面到管最高点 $C$ 列能量：$p_a/\rho g+z_1=p_C/\rho g+z_C+V^2/(2g)+h_{L,1C}$。要求不断流/不汽化：$p_{C,abs}\ge p_v+\Delta p_{\rm safe}$。出口若通大气，再从自由液面到出口求 $V$：$z_1-z_e=(1+\lambda L/d+\sum\zeta)V^2/(2g)$。检查：虹吸高点必须用绝压；出口大气压可取表压0，但汽化压绝不能用表压。}
\qt{子题：闸门力矩/拉杆力。题干给矩形或圆弧闸门、铰点、拉杆方向，求拉力。}
\ans{平面门先求静水合力 $F=\rho gh_cA$，压力中心 $y_p=y_c+I_c/(y_cA)$。对铰点取矩：$T\cdot l_T=F\cdot l_F+G\cdot l_G$，方向由图定。若是圆弧门，先分解 $F_H$ 和 $F_V$：$F_H$ 等于竖直投影面压力，$F_V$ 等于曲面上方假想水体重量，再对铰点取矩。检查：拉杆力臂是垂直距离，不是杆长；压力中心比形心更深。}
\qt{子题：动量矩/叶轮功率。题干给叶轮半径、转速、入口出口速度三角形，求转矩或功率。}
\ans{控制体绕转轴写角动量矩：$M_z=\dot m(r_2V_{u2}-r_1V_{u1})$，功率 $P=M_z\omega$。速度三角形：绝对速度 $\bm V$、叶片速度 $\bm U=\omega r\,\bm e_\theta$、相对速度 $\bm W=\bm V-\bm U$。若无冲击入口，$\bm W$ 沿叶片入口角；若出口叶片角给定，先由 $W_2$ 方向和连续性求 $V_{u2}$。检查：只有切向分量 $V_u$ 产生轴功率，径向/轴向分量不直接贡献转矩。}
\qt{子题：PM 膨胀角完整写法。题干给超声速绕凸角、出口欠膨胀、求膨胀角或转角。}
\ans{先确认是凸角膨胀且超声速 $M_1>1$。Prandtl--Meyer 函数空气常写 $\nu(M)=\sqrt6\tan^{-1}\sqrt{(M^2-1)/6}-\tan^{-1}\sqrt{M^2-1}$。若已知 $M_1,M_2$，转角 $\delta=\nu(M_2)-\nu(M_1)$；若已知转角，先算 $\nu_2=\nu_1+\delta$ 反查 $M_2$，再用等熵式算 $p_2,T_2$。检查：PM 膨胀 $p_0$ 不变；压缩角不能用 PM，要用斜激波。}
\qt{子题：平板摩擦阻力与边界层尾厚。题干给 $U,L,b,\rho,\nu$，比较空气和水或单/双面阻力。}
\ans{先 $Re_L=UL/\nu$。层流全板：$\delta_L\approx5L/\sqrt{Re_L}$，$\bar C_f=1.328/\sqrt{Re_L}$。湍流全板：$\delta_L\approx0.37L/Re_L^{1/5}$，$\bar C_f=0.074/Re_L^{1/5}$。有转捩：$x_c=Re_{cr}\nu/U$，$x_c<L$ 则前层后湍，常用修正 $\bar C_f=0.074/Re_L^{1/5}-1742/Re_L$。阻力 $D=\bar C_f(\rho U^2/2)A_w$，单面 $A_w=Lb$，双面 $2Lb$。检查：边界层厚度不是阻力公式中的面积。}
\qt{子题：老师终课图的三类物质模型。题干问“为什么这章用这个方程”。}
\ans{静止流体：表面力垂直表面、大小与方向无关，微元平衡给静水压强。不可压无粘：忽略切应力，用 Euler 方程；若无旋，速度势满足 Laplace，Bernoulli 可沿流线或全场用。可压无粘：用 $Ma$ 判断密度变化，超声速扰动形成声波/激波/膨胀波。不可压有粘：用 N--S，低 $Re$ 可解析，高 $Re$ 用边界层或经验阻力。检查：先判模型再写方程，不能拿一个公式跨所有题。}
\subt{管路/泵阀/粗糙度题型总包（旧版v106/v108完整链）}
\form{管路总入口：看到水池/油箱/管径/长度/粗糙度/弯头/阀/泵/水轮机/压力表，就先编号截面。自由液面：$p=0$表压、$V\approx0$；管内截面：$V=Q/A$；损失：$h_L=\sum(\lambda L/d+\zeta)V^2/(2g)$；能量：$z_1+p_1/\rho g+\alpha_1V_1^2/2g+H_p=z_2+p_2/\rho g+\alpha_2V_2^2/2g+h_L+H_T$。}
\warn{易错：局部损失速度取该元件所在管段；突扩用$(V_1-V_2)^2/(2g)$；粗糙度只在湍流/过渡查$\lambda$时用；汽化压和泵入口压力必须用绝压；泵功率和水轮机功率效率方向相反。}
\qt{子题：两油箱变径管15分题完整链。题干：两油箱液面差$h$，前段直径$d_1$、后段$d_2$，入口出口在液面以下，中间突扩，给$s,\mu$，求短管流量和A/B压强，长管给$Q,L_1,L_2,e$反求液面差。}
\ans{短管：两液面列PVZ，$h=(0.5V_1^2+(V_1-V_2)^2+1.0V_2^2)/(2g)$且$Q=A_1V_1=A_2V_2$。A点从左液面到A：$H=p_A/\rho g+V_1^2/(2g)+0.5V_1^2/(2g)$；B点从B到右液面：$p_B/\rho g+V_2^2/(2g)=H-h+V_2^2/(2g)$，故常可得$p_B/\rho g=H-h$。长管：给$Q$先$V_i=Q/A_i$，再$Re_i=\rho V_id_i/\mu$，层流$\lambda=64/Re$，湍流用Colebrook/Moody由$e/d,Re$取$\lambda$，最后$h=h_e+h_x+h_d+h_{f1}+h_{f2}$。}
\qt{子题：离心泵吸水安装高度。题干：吸水管长$L$、直径$d$、粗糙度$e$、弯头/入口阀$\zeta$、流量$Q$、汽化压$p_v$，要求泵中心高出水面最大$h$。}
\ans{从水面到泵入口中心列PVZ：$p_a/\rho g=z_B+p_B/\rho g+V^2/(2g)+\sum(\lambda L/d+\zeta)V^2/(2g)$。安全条件：$p_{B,abs}\ge p_v+\Delta p_{\rm safe}$。未知顺序：$Q\to V\to Re\to e/d\to\lambda\to h_L\to z_B^{\max}$。检查：题给“大气压/汽化压/至少高于汽化压20kPa”全是绝压条件，不可用表压0代替。}
\qt{子题：水轮机输出功率。题干：上下游水面高差、管径管长、粗糙度、平均流速或流量、下游压力表，忽略或计局部损失，求水轮机输出功率。}
\ans{先从上游自由液面到下游截面列能量，水轮机从流体取能：$H_T=z_1+p_1/\rho g+V_1^2/2g-z_2-p_2/\rho g-V_2^2/2g-h_L$。若截面是自由液面则$p=0,V=0$；若是管内压力表，$p$为表压水头。流量$Q=AV$，输出功率$P_{\rm out}=\eta\rho gQH_T$。检查：若求“水流给水轮机的功率”不乘效率；若求“轴输出”乘$\eta$。}
\qt{子题：串联/并联管路。题干：多管段、不同直径、分支汇合、给总流量或两端水头差。}
\ans{串联：各段$Q$相同，损失相加，$h_L=\sum h_{Li}$。并联：各支路两端总水头差相同，$h_{La}(Q_a)=h_{Lb}(Q_b)$，节点连续$Q=Q_a+Q_b$。每段单独算$V_i,Re_i,\lambda_i$，局部损失也用本段$V_i$。检查：并联不是把损失相加；若未知流量分配，用试算法/方程组，先假设$Q_a$再由水头相等修正。}
\qt{子题：粗糙度/Colebrook/Moody 写法。题干给镀锌钢管、铸铁管、粗糙度$e$或$e/d$，要求沿程损失或压降。}
\ans{先$Re=Vd/\nu$。层流直接$\lambda=64/Re$，粗糙度不用。湍流写“由$Re$和相对粗糙度$e/d$查Moody图得$\lambda$”；若题给Colebrook，写$1/\sqrt\lambda=-2\log(e/(3.7d)+2.51/(Re\sqrt\lambda))$迭代。压降$\Delta p=\rho g\lambda(L/d)V^2/(2g)=\lambda(L/d)\rho V^2/2$。检查：Darcy摩擦系数$\lambda$不是Fanning系数，若公式用$f$看清差4倍。}
\qt{子题：管路压力表/测压管读数。题干：管内某点接压强计或U形压差计，要求该点压强或压差。}
\ans{若接开口测压管，读数$h_p=p/\rho g$为表压水头。若U形压差计，两侧同流体、测压液密度$\rho_m$，常用$\Delta p=(\rho_m-\rho)gh$；若一侧接空气可近似$\Delta p=\rho_mgh$。接入能量方程前统一成$p/\rho g$。检查：测压计只给两个测点的静压差，不含速度头；不要把总水头和静压水头混用。}
\qt{子题：管道受力/弯头支座反力。题干：弯管、喷嘴、变径管，给入口出口压力和流量，求固定支座受力。}
\ans{取管内流体为CV，先求$\dot m=\rho Q$和$\bm V_1,\bm V_2$。动量方程：$\sum\bm F=\dot m(\bm V_2-\bm V_1)$。外力含入口压力$p_1A_1$、出口压力$p_2A_2$、重力、壁面对流体反力$\bm R$。求出$\bm R$后，题问“流体对管道/支座”取反。检查：出口若自由射流$p_2=0$表压；压力力永远指向CV内部。}
\qt{子题：水池底部放水管/普通出流。题干：水池到大气出口，给水头$H$、管长直径、入口/弯头/阀/出口损失，求$Q$。}
\ans{从水池自由液面到出口截面列PVZ：$H=V^2/(2g)+\sum(\lambda L/d+\zeta)V^2/(2g)$。若$\lambda$未知，先猜$V$或$\lambda$，由$Re$和$e/d$查$\lambda$，迭代到$Q=AV$稳定。短管可忽略沿程，只保留入口/出口/局损。检查：出口速度头通常保留，出口局损$\zeta=1$要看题目是否已把出口截面取在自由射流处，避免重复。}
\qt{子题：泵阀管路第一行怎么写。题干复杂到不会下手时。}
\ans{先写一句：取上游自由液面1、泵前/后截面2/3、下游自由液面4为控制截面，沿管线列扩展Bernoulli。再列未知链：$Q\to V_i=Q/A_i\to Re_i\to\lambda_i\to h_{Li}\to H_p/H_T/p_i$。最后写检查：流向从高总水头到低总水头；泵使总水头升高，阀门/弯头/摩擦使总水头降低；所有压力结果说明表压或绝压。}
\subt{组合题拆题链：管路--动量--势流--可压--黏性接口}
\form{组合题总原则：先分段，再给每段贴模型标签。液面--管路段用PVZ；喷嘴/弯管受力段用CV动量；势流外场先求速度再用Bernoulli求压；可压喷管先用$M$链判状态，跨激波必须重置总压；黏性层流先由N--S删项得速度，再积分求$Q,F$；边界层先算$Re$再取$C_f,\delta$。接口量只传$Q,V,p,F$等物理量，不传错方程常数。}
\qt{子题：管路末端接喷嘴/射流冲板。题干：水池经管道到喷嘴，喷流打固定板或叶片，求流量、喷嘴反力、板受力。}
\ans{先用母题1从水池到喷嘴出口求$Q,V_e$和出口压力；若出口自由射流，$p_e=0$表压。再取喷流/板为CV用动量：固定平板法向力近似$F=\rho QV_e$；若射流折转角$\theta$，用矢量式$\sum F_x=\dot m(V_{2x}-V_{1x}),\sum F_y=\dot m(V_{2y}-V_{1y})$。检查：管路损失影响$V_e$，但打板力不再用能量方程直接算。}
\qt{子题：势流求压后再求结构受力。题干：圆柱/半圆柱/气膜馆外绕流，内部压力给定，求升力或是否掀起。}
\ans{先由势流母题4求表面速度，例如圆柱无环量$V_\theta=-2U\sin\theta$；再由定常Bernoulli求外压$p_{\rm out}=p_\infty+\rho(U^2-V_s^2)/2$；净压$p_{\rm in}-p_{\rm out}$沿表面积分，竖向分量乘$\sin\theta$或按投影取。检查：势流给的是理想外压分布；若题问真实阻力，必须说明黏性分离会改变阻力。}
\qt{子题：Laval喷管题再接管外波系。题干：已知$A_e/A_t,p_0,p_b$，问管内是否激波、出口是否欠/过膨胀、管外有何波。}
\ans{先按母题5求设计超声支出口压$p_e$和出口$M_e$。若$p_b=p_e$设计匹配；若$p_b>p_e$，出口外压较高，管外出现压缩波/斜激波或管内激波；若$p_b<p_e$，出口欠膨胀，管外出现膨胀扇，PM继续算到$p_b$。检查：判断管内激波先比较临界背压；判断管外波系看内部出口静压 $p_e$ 与外界背压 $p_b$，二者不是同一个量。}
\qt{子题：黏性层流速度分布再接流量/阻力/功率。题干：先要求$u(y)$或$u(r)$，再问$Q,\tau,F,P$。}
\ans{先用母题6删N--S得速度分布；再按定义积分$Q=\int_Au\,dA$，平均速度$\bar u=Q/A$，壁剪$\tau_w=\mu(\partial u/\partial n)_w$，阻力$F=\tau_wA_w$，拖动功率$P=FU$。检查：不要先背$Q$公式再反推速度；题要“证明”时必须从微分方程和边界条件开始。}
\subt{边界层积分法完整题链。题干：给零压梯度平板、速度剖面 $u/U=f(\eta),\eta=y/\delta$，求 $\delta^*,\theta,\delta(x),C_f,D$。}
\ans{第一步写定义：$\delta^*=\int_0^\delta(1-u/U)dy=\delta I_1$，$\theta=\int_0^\delta(u/U)(1-u/U)dy=\delta I_2$。第二步壁剪：$\tau_w=\mu(\partial u/\partial y)_0=\mu U f'(0)/\delta$。第三步 Karman 积分式零压梯度：$\tau_w/(\rho U^2)=d\theta/dx=I_2d\delta/dx$，得 $\delta d\delta/dx=\nu f'(0)/(UI_2)$，积分且$\delta(0)=0$。第四步局部 $C_f=2\tau_w/(\rho U^2)$，单面阻力 $D=b\rho U^2\theta(L)$ 或 $C_D=2\theta(L)/L$。检查：$I_1,I_2$ 是把$f(\eta)$在0到1积分，不是把 $y$ 当常数。}
\qt{子题：边界层转捩混合板。题干：给 $Re_{cr}$，要求前段层流、后段湍流的总阻力。}
\ans{先算 $x_c=Re_{cr}\nu/U$。若 $x_c\ge L$ 全层流；若 $x_c<L$，写总阻力为前层流积分加后湍流积分。考试常用修正式 $\bar C_f=0.074/Re_L^{1/5}-1742/Re_L$，再 $D=\bar C_f(0.5\rho U^2)A_w$。若题要求严格过程，写 $D=b\int_0^{x_c}\tau_{w,l}dx+b\int_{x_c}^{L}\tau_{w,t}dx$。检查：转捩点由 $Re_x=Ux/\nu$ 定，不是由 $Re_L$ 直接定厚度。}
\qt{子题：阻力危机/气球系留角。题干：球、气球、圆柱外绕，给风速和阻力系数图，求阻力、浮力、绳角。}
\ans{先算 $Re=\rho Ud/\mu$，由 $Re$ 和形状查 $C_D$；阻力 $D=C_D(0.5\rho U^2)A_{\rm front}$，球 $A_{\rm front}=\pi d^2/4$。浮力 $B=\rho_{\rm air}gV$，重力 $G=\rho_{\rm obj}gV$；稳态力平衡：水平 $T\sin\alpha=D$，竖直 $T\cos\alpha=B-G$，故 $\tan\alpha=D/(B-G)$。检查：阻力面积是迎风面积，不是表面积；阻力危机时 $C_D$ 可能突然下降，不能用固定0.44。}
\subt{相似准则冲突题。题干：模型/原型、同流体缩尺、要求保持 Re 或 Fr，问速度、力、能否完全相似。}
\ans{先写几何比 $\lambda_L=L_m/L_p$。若粘性主导，令 $Re_m=Re_p$，得 $V_m/V_p=\nu_m L_p/(\nu_p L_m)$；同流体即 $V_m/V_p=1/\lambda_L$。若重力自由面主导，令 $Fr_m=Fr_p$，得 $V_m/V_p=\sqrt{L_m/L_p}=\sqrt{\lambda_L}$。两者同流体同时满足要求 $1/\lambda_L=\sqrt{\lambda_L}$，除非 $\lambda_L=1$，否则冲突。检查：船模/明渠优先 Fr，管内阻力优先 Re，高速气体还要 Ma。}
\qt{子题：Rayleigh 量纲法。题干：变量少，要求推出 $Y$ 与若干变量的幂律关系。}
\ans{写 $Y=Cx_1^ax_2^b\cdots$，把 $Y,x_i$ 全部换成 $M,L,T$ 指数，按 $M,L,T$ 三行列方程求 $a,b,\cdots$。例：单摆周期若只与 $l,g$ 有关，设 $T=Cl^ag^b$，量纲得 $a=1/2,b=-1/2$，$T=C\sqrt{l/g}$。检查：Rayleigh 适合变量少；变量多时改 Buckingham $\pi$，避免少掉参数。}
\subt{水击/水波/明渠扰动。题干：阀门突然关闭、长管水锤；或给波长水深；或问扰动能否传上游。}
\ans{水击快速关阀首式：$\Delta p=\rho a\Delta V$，压升水头 $\Delta H=a\Delta V/g$；若给管壁弹性，$a\approx\sqrt{K/\rho}/\sqrt{1+KD/(Ee)}$。水波：深水 $h>\lambda/2$，$c=\sqrt{g\lambda/(2\pi)}$；浅水 $h<\lambda/20$，$c=\sqrt{gh}$。明渠扰动：$Fr=V/\sqrt{gL}$，$Fr<1$ 缓流可影响上游，$Fr>1$ 急流不能传上游。检查：水击是液体/管壁弹性问题，不是普通不可压 Bernoulli。}
\qt{子题：自由涡/强迫涡液面。题干：旋涡、旋转水桶、流线弯曲，求压差或自由面形状。}
\ans{径向平衡统一写 $dp/dr=\rho v_\theta^2/r$。强迫涡 $v_\theta=\omega r$，积分 $p/\rho+\!gz-\omega^2r^2/2=C$，自由面 $p=p_a$，故 $z=\omega^2r^2/(2g)+C$。自由涡 $v_\theta=C/r$，积分 $p/\rho+\!gz+C^2/(2r^2)=C_0$，自由面中心凹陷，理论 $r\to0$ 发散。检查：强迫涡像刚体旋转，涡量非零；自由涡除中心外无旋，环量常数。}
\qt{子题：曲线流动法向压强梯度。题干：弯曲流线半径 $R_c$、速度 $V$，问内外侧压力大小。}
\ans{沿法向指向曲率中心写 $dp/dn=\rho V^2/R_c$ 的符号要按 $n$ 方向定义。物理判断：需要向心加速度，压力必须外侧高、内侧低，压力梯度提供向心力。若是自由液面，沿面 $p=p_a$，由 $p/\rho+gz+\int V^2/R_c\,dn=C$ 求水面高差。检查：这不是沿流线 Bernoulli 的切向压强变化，而是法向动量平衡。}
\subt{斜激波题完整链。题干：给来流 $M_1$、楔角/偏转角 $\theta$ 或波角 $\beta$，求波后状态。}
\ans{先判压缩角且 $M_1>1$。若给 $\beta$，先 $M_{n1}=M_1\sin\beta$；用正激波关系求 $M_{n2},p_2/p_1,T_2/T_1,p_{02}/p_{01}$；再 $M_2=M_{n2}/\sin(\beta-\theta)$。若给 $\theta$，用 $\theta$--$\beta$--$M$ 关系或表求弱解 $\beta$，再按上链。检查：斜激波只处理法向分量，切向速度不变；压缩角太大无附体弱解，改脱体激波。}
\qt{子题：正激波表连算。题干：给 $M_1,p_1,T_1$ 或给喷管中某截面波前马赫数，求波后静压/总压。}
\ans{直接写 $M_1>1$，查正激波表或用公式得 $M_2,p_2/p_1,T_2/T_1,\rho_2/\rho_1,p_{02}/p_{01}$。再算 $p_2,T_2,\rho_2$；后续若继续等熵流动，必须以 $p_{02},T_{02}=T_{01}$ 为新的总参数。检查：不能用“知道 $M_2$ 就随便全算”，因为 $M_2$ 只给波后速度状态，仍要乘波前 $p_1,T_1$ 或用新总压。}
\qt{子题：Fanno 等截面摩擦管。题干：绝热、等截面、有摩擦、给管长或极限管长，问出口状态。}
\ans{识别词是等截面、绝热、摩擦、可压。摩擦使亚声速加速到 $M=1$，超声速减速到 $M=1$，总温 $T_0$ 不变，总压下降。流程：由入口 $M_1$ 查 Fanno 表得 $4fL_1^*/D$；给实际 $L$，算 $4fL/D$，出口剩余 $4fL_2^*/D=4fL_1^*/D-4fL/D$，反查 $M_2$，再查 $T/T^*,p/p^*,p_0/p_0^*$ 比值。检查：Fanno 不是等熵，不能用面积--马赫式代替。}
\qt{子题：可压缩文丘里/质量流量反求。题干：气体通过收缩段，给 $p_0,T_0,A$ 和下游压，求 $\dot m$ 或是否阻塞。}
\ans{先看最小面积截面是否可能临界。若 $p_b/p_0>0.528$，未阻塞，取出口/喉部静压由背压控制，用等熵压比求 $M$，再 $\dot m=\rho MAV=\rho A M\sqrt{\gamma RT}$。若 $p_b/p_0\le0.528$，阻塞，最小截面 $M=1$，$\dot m_{\max}=A^*p_0\sqrt{\gamma/R}/\sqrt{T_0}(2/(\gamma+1))^{(\gamma+1)/(2(\gamma-1))}$，空气可用 $0.0404A^*p_0/\sqrt{T_0}$。检查：阻塞后继续降背压，质量流量不再增大。}
\subt{Rankine 半体/源汇偶极综合题。题干：均匀流加点源/点汇/偶极，求驻点、分界流线、实体形状。}
\ans{先写 $\psi$，因为边界通常是流线。均匀流 $Ur\sin\theta$，点源 $Q\theta/(2\pi)$，点汇取负，偶极 $-M\sin\theta/r$。驻点由速度分量为0定；分界线取经过驻点的 $\psi=\psi_s$。Rankine 半体常有驻点距源 $a=Q/(2\pi U)$，半体宽度由远处 $\psi=Q/2$ 定。检查：求形状用 $\psi=C$ 比用 $\phi$ 稳；镜像壁面题把壁面也当流线。}
\qt{子题：圆柱有环量/机翼升力链。题干：圆柱绕流叠加环量，求驻点位置、压力分布、升力。}
\ans{表面切向速度 $V_\theta=-2U\sin\theta+\Gamma/(2\pi a)$，驻点令 $V_\theta=0$。压力系数 $C_p=1-(V_\theta/U)^2$。升力单位长度 $L'=\rho U\Gamma$，方向由来流和环量右手关系判；阻力在理想势流中为0。检查：若 $|\Gamma|>4\pi Ua$，驻点离开圆柱表面；真实机翼环量由Kutta条件选定，不是随便给。}
\subt{Kelvin/Lagrange/Helmholtz 概念证明题。题干：证明无旋保持、涡管强度守恒、解释势函数单值。}
\ans{Kelvin：理想无粘、正压流体、保守体力下，随体闭合线环量 $\Gamma=\oint\bm V\cdot d\bm l$ 守恒。Lagrange：若初始 $\Omega=0$，任取随体小闭合线初始 $\Gamma=0$，以后仍 $\Gamma=0$，由 Stokes 定理得任意小面涡量为0。Helmholtz：涡管侧面 $\bm\Omega\cdot\bm n=0$，两截面涡通量相等，且涡线随体运动。检查：必须先写适用假设；有粘性或非保守体力时结论不可直接用。}
\qt{子题：流线/迹线/脉线。题干：给非定常速度场，问三种线或某时刻流线。}
\ans{流线是固定时刻速度场切线：$dx/u=dy/v=dz/w$，非定常题先代入 $t=t_0$ 再积分。迹线是单个质点轨迹：解 $dx/dt=u(x,y,z,t)$ 且带初始位置。脉线是同一注入点不同时刻释放质点在同一时刻的位置集合。检查：定常流三者重合；非定常流不能把流线微分方程当质点运动方程。}
\subt{旧版剩余短问转过程分。题干问“为什么/区别/判断/物理意义”。}
\ans{先写“模型+原因+后果+限制”。例：边界层存在：高 $Re$ 外流黏性集中近壁，壁面无滑移使速度从0过渡到外流，故有薄层；分离：逆压梯度使近壁低能流体减速，$\tau_w=\mu(\partial u/\partial y)_w\to0$ 后反向，尾迹低压导致压差阻力；阻力危机：球/圆柱边界层转捩为湍流，抗逆压梯度增强，分离点后移，$C_D$骤降；达朗贝尔佯谬：无粘势流前后压力对称，阻力为0，真实阻力来自黏性分离；激波总压下降：绝热但不可逆，熵增，总温近似不变，总压损失。}
\lookup{旧版查表题转成完整答题句。}
\ans{写法必须是“查什么、用什么输入、读什么输出、回代哪里”：Moody 输入 $Re,\varepsilon/d$ 读 $\lambda$ 回 $h_f$；阻力图输入形状和 $Re$ 读 $C_D$ 回 $D=C_DqA$；面积--马赫输入 $A/A^*$ 读两支 $M$ 再由背压选；正激波输入 $M_1$ 读 $M_2,p_2/p_1,T_2/T_1,p_{02}/p_{01}$，激波后继续等熵必须用新 $p_{02}$；斜激波输入 $M_1,\theta$ 读弱解 $\beta$；PM 输入 $M$ 读 $\nu(M)$，膨胀用 $\nu_2=\nu_1+\delta$。}
\err{最后核验：若一道题看起来没模板，先问“它最终求什么”。求 $Q/H/p/P$ 回母题1；求 $F/M$ 回母题2/3；求 $\phi,\psi,\Gamma$ 回母题4；求 $M,\dot m,p_b$ 回母题5；求 $u,\tau,Q$ 回母题6；求 $D,\delta,C_f,C_D$ 回母题7；求 $P\bm n,\Omega,\phi,\psi$ 回母题8；求比例/波速/水击回母题9/16。}
\qt{子题：孔口/文丘里/皮托测流。题干给水箱孔口、收缩截面、压差计、皮托管或流量系数，求 $Q$、$V$、压差。}
\ans{孔口先从自由液面到射流收缩截面列 Bernoulli：理想 $V=\sqrt{2gH}$，实际 $Q=C_dA\sqrt{2gH}$，若给收缩系数 $C_c$、速度系数 $C_v$，则 $C_d=C_cC_v$。文丘里：连续 $Q=A_1V_1=A_2V_2$，能量 $p_1/\rho g+V_1^2/2g=p_2/\rho g+V_2^2/2g+h_L$，忽略损失时由 $\Delta p$ 求 $Q$；有流量系数则乘 $C_d$。皮托管：停滞压差 $\Delta p=\rho V^2/2$，故 $V=\sqrt{2\Delta p/\rho}$。检查：压差计读数要先换成 $\Delta p$；孔口题出口表压为0，但速度头不能丢。}
\qt{子题：声速、马赫锥、听到飞机声音。题干给高度、温度或声速、飞行速度，问 Ma、锥角、延迟听声距离。}
\ans{先算声速 $a=\sqrt{\gamma RT}$；若题给高度需先取该高度温度。$Ma=V/a$。若 $Ma<1$，扰动可向前传，无马赫锥；若 $Ma>1$，马赫角 $\sin\mu=1/Ma$。观察者听到声爆时，飞机已飞过距离常由几何给：若高度为 $H$，水平滞后距离 $x=H\cot\mu=H\sqrt{M^2-1}$。检查：声速用当地静温，不用滞止温；角度 $\mu$ 是马赫线与飞行方向夹角。}
\qt{子题：体积弹性模量/气体密度变化。题干问水被加压、气体等温压缩、密度相对变化。}
\ans{液体体积弹性模量 $E=-\Delta p/(\Delta V/V)$，因 $\rho=m/V$，小变化有 $\Delta\rho/\rho\approx-\Delta V/V=\Delta p/E$。水从 $p_1$ 到 $p_2$：$\Delta\rho/\rho=(p_2-p_1)/E$。理想气体等温：$p/\rho=RT$ 常数，故 $\rho_2/\rho_1=p_2/p_1$，相对变化 $(\rho_2-\rho_1)/\rho_1=p_2/p_1-1$。检查：液体压缩很小，气体压缩可很大；压力必须用绝压。}
\qt{子题：浮体稳定/倾角小扰动。题干给漂浮体、重心、浮心、排水体积、水线面惯性矩，问是否稳定或复原力矩。}
\ans{先由浮力等于重量定吃水/排水体积：$\rho gV=G$。小倾角稳定看定倾中心 $M$：$BM=I_w/V$，$GM=BM+KB-KG$。若 $GM>0$ 稳定，$GM=0$ 随遇，$GM<0$ 不稳定。小角复原力矩 $M_R=G\,GM\sin\theta\approx G\,GM\,\theta$。检查：$I_w$ 是水线面关于倾斜轴的二次矩，不是实体截面惯性矩；浮心 $B$ 随倾斜移动，重心 $G$ 不随流体变。}
\qt{子题：Reynolds 输运定理/系统到控制体。题干让“由系统守恒推出控制体形式”或给移动控制面。}
\ans{先写通式：$\frac{dB_{\rm sys}}{dt}=\frac{\partial}{\partial t}\int_{CV}\rho b\,dV+\int_{CS}\rho b(\bm V-\bm V_{CS})\cdot\bm n\,dA$。质量取 $b=1$ 得连续方程；动量取 $b=\bm V$ 得 $\sum\bm F=\partial_t\int\rho\bm VdV+\int\rho\bm V[(\bm V-\bm V_{CS})\cdot\bm n]dA$。定常固定CV简化为出流减入流。检查：运动控制面必须用相对速度 $\bm V-\bm V_{CS}$；法向 $\bm n$ 取外法线，流入项自然为负。}
\end{multicols}
\end{document}
