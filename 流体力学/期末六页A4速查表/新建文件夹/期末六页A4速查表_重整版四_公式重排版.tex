...
\subq{Hagen-Poiseuille 圆管层流计算：已知 L, d, $\mu$, $\rho$ 及流量 Q，求压降与壁面剪力}
1. \key{验证层流}：算流速 $V = \frac{4Q}{\pi d^2}$，算雷诺数 $Re = \frac{\rho V d}{\mu}$。若 $Re < 2300$，则为层流。
2. \key{计算压降 $\Delta p$}：带入公式：$\Delta p = \frac{128 \mu L Q}{\pi d^4} = \frac{32 \mu L V}{d^2}$。
3. \key{求壁面剪应力 $\tau_w$}：带入力平衡公式（或者剪力公式）：$\tau_w = \frac{d\Delta p}{4L} = \frac{8\mu V}{d}$。
