...
\infotxt{%
1. \key{描述方法}：拉格朗日法跟踪质点随时间轨迹；欧拉法在空间固定场点研究流速、压强等要素。\\
2. \key{流线与迹线}：流线是同一时刻不同质点速度方向的切线（瞬时场线，欧拉观点）；迹线是同一质点在不同时间段的轨迹（时间曲线，拉格朗日观点）。\warn{定常流动中流线与迹线重合}，非定常通常不重合。\\
3. \key{均匀流与渐变流}：均匀流指位变加速度为0（沿程流速不随空间变化）。\warn{恒定均匀流的时变与位变加速度均为0}，流体质点无惯性力，作匀速直线运动，其过水断面是平面。\\
4. \key{连续性与本构方程}：连续性方程源自质量守恒，不涉及粘性或受力，故\warn{粘性流体与理想流体的连续性方程形式完全相同}。牛顿流体本构中，应力与变形速率张量为线性关系，且由于流体各向同性，该线性关系\warn{与应力作用方向无关}。\\
5. \key{动压强与控制方程}：动压强$p = -(\sigma_{xx}+\sigma_{yy}+\sigma_{zz})/3$是数量场，但不是任意方位面上的真正压应力。无粘流体Euler方程由于含有对流项$(\bm V\cdot\nabla)\bm V$，\warn{同样是非线性偏微分方程}（N-S方程亦然）。\\
6. \key{旋涡与速度环量}：涡量是速度旋度（$\bm\Omega=\nabla\times\bm V$），等于微团绕该点平均旋转角速度的\warn{两倍}。涡管在流体内部不能产生或消失，要么闭合，要么终止于自由面或固壁。速度环量随时间变化的根源为：\warn{质量力无势、流体斜压、粘性作用}（粘性是旋涡产生和扩散的根源）。\\
7. \key{势函数与流函数}：只要是二维不可压流动（\warn{无论定常与否、有无粘性}），\warn{必然存在流函数$\psi$}。只有\warn{无旋流动}才存在势函数$\phi$。有流函数不一定有势函数（可能有旋）；有势函数不一定有流函数（可能三维）。等势线与等流函数线正交。
}
...
4. \key{管嘴出流真空区}：相同水头下管嘴流量为相同口径孔口流量的\warn{1.32倍}（收缩断面处\warn{存在真空区起抽戏作用}）。正常工作条件是管长$l=(3\sim4)d$，且真空度不超过限值。\\
5. \key{水击现象}：流量突变导致压强大幅波动。水击波速$c$受液体压缩性和管壁弹性缓冲。关闭时间$T_s$小于水击相长$T$时为直接水击，否则为间接水击，\warn{间接水击增压低于直接水击}。
}

\infoh{选择判断秒杀库-4.低雷诺数与粘性主导流动}
\infotxt{%
...
\infoh{选择判断秒杀库-5.边界层理论、流动分离与相似性}
\infotxt{%
1. \key{边界层外界线}：由名义厚度$\delta$标定的边界层粘性外界线\warn{不是流线}（因排挤效应沿程增厚，有外部流体跨越卷吸进入）。边界层积分求解\warn{必须同时使用}名义厚度$\delta$、排挤厚度$\delta^*$和动量损失厚度$\theta$。\\
2. \key{流动分离判据}：高Re绕流在后部减速段存在\warn{逆压梯度}（$dp/dx>0$）；壁面剪力降为零（$\tau_w=0$即$\partial u/\partial y|_w=0$）是分离的临界条件。分离后尾流区压强降低，压差阻力剧增。\\
3. \key{阻力危机}：球/圆柱在临界$Re$附近边界层转捩为湍流，抗逆压梯度能力增强，分离点后移、尾迹变窄，使\warn{阻力系数$C_D$骤降}。\\
4. \key{力学相似律}：几何相似是前提，动力相似是保证，运动相似是目的。雷诺相似（粘性主导，同流体$V_rL_r=1$）和弗劳德相似（重力主导，同流体$V_r=\sqrt{L_r}$）通常互相冲突，模型设计应保证主导力相似。在阻力平方区发生\warn{自模化}时，只需满足几何相似即可。
}

\infoh{选择判断秒杀库-6.湍流与时均封闭模型}
\infotxt{%
...
4. \key{Prandtl-Meyer膨胀波}：超声速流绕凸角流动（流道向外打开）时产生膨胀扇。膨胀波内流动为等熵过程，总压 $p_0$ and $T_0$ \warn{保持不变}；气体通过后马赫数增大，\warn{静压、静温、密度均下降}。
}

\infoh{选择判断秒杀库-9.期末真题判断与选择专练}
\infotxt{%
