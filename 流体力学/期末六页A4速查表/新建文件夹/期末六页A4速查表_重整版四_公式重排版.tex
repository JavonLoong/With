...
\columnbreak
\infoh{选择判断秒杀库-1.第1章：流体的物理性质}
\infotxt{%
1. \key{连续介质模型}：将流体视为由质点连续无间隙分布的介质，其物理量连续分布。当研究的流场特征尺度远大于分子平均自由程时适用。\\
2. \key{粘性与温度}：温度升高，\warn{液体粘度下降}（分子引力减小，分子受热膨胀间距变大）；\warn{气体粘度上升}（分子热运动增强，动量交换加剧）。粘性只在相对运动时表现，静止时为0。\\
3. \key{压缩性与声速}：不可压缩流体中声速为无穷大；气体在流速远小于音速（$Ma<0.3$）时可视为不可压缩。\\
4. \key{表面张力}：源于界面层分子受力不平衡，切向作用于分界线，使液体表面有收缩趋势。
}

\infoh{选择判断秒杀库-2.第2章：流体运动学}
\infotxt{%
1. \key{运动描述法}：拉格朗日法跟踪流体质点轨迹随时间的变化（对应迹线）；欧拉法研究固定空间点处的流速、压力等物理量（对应流线）。\\
2. \key{流线与迹线}：流线是瞬时切线场；迹线是质点轨迹。在\warn{定常流动中流线与迹线完全重合}，非定常流动中通常不重合。\\
3. \key{加速度构成}：随体导数由当地导数（时变，$\partial/\partial t$）与迁移导数（位变，$(\bm V\cdot\nabla)\bm V$）组成。定常均匀流的时变与位变加速度均为0。\\
4. \key{流体微团运动与旋涡}：微团运动可分解为平动、转动、线变形和角变形。旋涡强度的量度为涡量（速度旋度 $\bm\Omega=\nabla\times\bm V$），其大小等于微团绕该点平均旋转角速度的\warn{两倍}。
}

\infoh{选择判断秒杀库-3.第3章：流体动力学基本原理}
\infotxt{%
1. \key{连续性方程}：源自质量守恒，仅涉及运动学，故\warn{粘性流体与理想流体的连续性方程形式完全相同}。\\
2. \key{动能与动量修正系数}：由于断面流速不均匀而引入。流速分布越不均匀，修正系数越大。层流的修正系数大于紊流（层流速度剖面呈抛物线分布，动能修正系数 $\alpha=2.0$；紊流剖面扁平均匀，$\alpha\approx 1.05\sim 1.10$）。\\
3. \key{孔口与管嘴出流}：相同水头下，管嘴流量为孔口的 1.32 倍，原因在于管嘴收缩断面处\warn{存在真空区起抽吸作用}。工作条件是管长 $l=(3\sim 4)d$。
}

\infoh{选择判断秒杀库-4.第4章：理想流体动力学}
\infotxt{%
1. \key{理想流体}：忽略粘性的流体（$\mu=0$）。其控制方程为 Euler 方程，由于对流加速度项存在，\warn{同样是非线性偏微分方程}（N-S方程亦然）。\\
2. \key{伯努利方程条件}：沿同一流线成立的条件是：定常、不可压、无粘理想流体且无摩擦损失。若流场全场无旋，则伯努利常数全场相等，不限于同一流线。\\
3. \key{开尔文定理与环量变化}：在理想、正压流体且质量力有势时，沿闭合物质回路的速度环量不随时间变化。粘性是产生与扩散旋涡的根本原因。
}

\infoh{选择判断秒杀库-5.第5章：二维无旋与有旋流动}
\infotxt{%
1. \key{流函数存在条件}：\warn{二维}（平面或轴对称）且\warn{不可压缩}流动。源于连续性方程（质量守恒），\warn{与有无粘性、有旋无旋、定常与否无关}。\\
2. \key{势函数存在条件}：流动\warn{无旋}（$\nabla \times \bm V = 0$）。源于无旋场势函数定义，\warn{与维数、压缩性、有无粘性、定常与否无关}。\\
3. \key{混淆要点}：二维不可压有旋流动有流函数但无势函数；三维无旋流动有势函数但无二维流函数；有粘流动满足局部无旋时也可以存在势函数。\\
4. \key{复势与拉普拉斯}：若流动既是二维不可压又是无旋的，势函数与流函数均满足 Laplace 方程，且等势线与等流函数线正交。\\
5. \key{圆柱绕流与升力}：理想流体无环量绕圆柱无阻力也无升力（达朗贝尔佯谬）；若有速度环量 $\Gamma$，则产生与来流垂直的升力 $L=\rho_\infty V_\infty \Gamma$（库塔-儒可夫斯基定理）。
}

\infoh{选择判断秒杀库-6.第6章：水波动力学}
\infotxt{%
1. \key{自由表面边界条件}：运动学边界条件要求表面流体质点始终在表面上；动力学边界条件要求自由表面压强等于大气压（等压面）。\\
2. \key{色散关系与波速}：波速随波长（或频率）变化的现象称为色散。深水波是强色散波（波速 $c=\sqrt{g\lambda/2\pi}$）；浅水长波是非色散波（波速 $c=\sqrt{gh}$）。\\
3. \key{水波分类}：由水深与波长比 $h/\lambda$ 决定：$h/\lambda > 0.5$ 为深水波（波浪运动无法传至海底）；$h/\lambda < 0.05$ 为浅水波（长波）。
}

\infoh{选择判断秒杀库-7.第7章：气体动力学基础}
\infotxt{%
1. \key{声速与马赫数}：声速 $c=\sqrt{\gamma RT}$ 仅是温度的函数。马赫数 $Ma=V/c$ 反映可压缩程度，当 $Ma < 0.3$ 时可近似为不可压缩流动。\\
...
2. \key{低雷诺数流动（$Re < 1$ 斯托克斯流）}：忽略惯性项，由线性 Stokes 方程控制，**绝不能使用无粘 Euler 方程**。小球总阻力 $D = 3\pi \mu d U$，其中\warn{摩擦阻力占 2/3}，压差阻力占 1/3，无任何流动分离。在极低雷诺数下，流线型物体因湿面积大导致摩擦阻力大，总阻力可能大于圆球。\\
3. \key{粘性流动解析解}：Couette流（剪切驱动，无压梯度下速度呈线性分布）；Poiseuille流（压力驱动，速度呈抛物线分布）。两层粘性流体交界面上，\warn{剪切应力与速度必须连续}。\\
4. \key{动力相似律与自模化}：重力主导选用 Fr 相似，粘性主导选用 Re 相似，二者通常冲突。在阻力平方区发生\warn{自模化}时，阻力系数与 Re 无关，此时只需满足几何相似即可。
}

\infoh{选择判断秒杀库-9.第9章：湍流}
\infotxt{%
1. \key{转捩与稳定性}：流态由层流向湍流转捩的临界雷诺数受壁面粗糙度和外界干扰影响，\warn{不是确定的常数}。\\
2. \key{雷诺应力与方程不封闭}：不可压湍流时均动量方程（RANS）中，由于引入了脉动动量交换产生的\warn{雷诺应力项而导致方程不封闭}，必须引入半经验湍流模型。\\
3. \key{切应力分布}：紊流圆管中总切应力（粘性切应力加雷诺应力之和）沿过流断面同样呈\warn{线性分布}，管轴处为0，管壁处最大。
}

\infoh{选择判断秒杀库-10.第10章：边界层理论基础}
\infotxt{%
1. \key{边界层外界线}：由名义厚度 $\delta$ 标定的边界层粘性外界线\warn{不是流线}（因排挤效应沿程增厚）。边界层积分求解\warn{必须同时使用}名义厚度 $\delta$、排挤厚度 $\delta^*$ 和动量损失厚度 $\theta$。\\
2. \key{流动分离判据}：高Re绕流在后部减速段存在\warn{逆压梯度}（$dp/dx>0$）；壁面剪力降为零（$\tau_w=0$即$\partial u/\partial y|_w=0$）是分离的临界条件。分离后压差阻力剧增。\\
3. \key{阻力危机}：球/圆柱在临界 $Re$ 附近边界层由层流转捩为湍流，抗逆压梯度能力增强，分离点后移、尾迹变窄，使\warn{阻力系数 $C_D$ 骤降}。
}

\infoh{选择判断秒杀库-11.第11章：磁流体动力学}
\infotxt{%
1. \key{磁流体动力学假设}：将流体视为导电的连续介质，其运动受磁场控制，控制方程为流体力学方程与 Maxwell 电磁方程组的耦合。\\
2. \key{洛伦兹力作用}：导电流体在磁场中运动受洛伦兹力 $\bm J \times \bm B$ 作用，该力通常起阻碍流动、使速度剖面扁平化（如 Hartmann 流）的作用。\\
3. \key{阿尔芬波}：磁场与导电流体相互耦合时，在垂直于磁场方向传播的横波，波速为 $V_A = B / \sqrt{\mu_0 \rho}$。
}

\infoh{选择判断秒杀库-12.前半学期（第1-4章）期中综合秒杀库}
\infotxt{%
1. \key{静压强特性}：静止流体中任一点的静压强大小与方向无关，垂直指向受压面。\\
2. \key{静力平衡与等压面}：不可压常密度流体\warn{只有在有势质量力作用下}才能处于静止平衡。等压面同时是等势面，质量力方向必定垂直于等压面（如匀速旋转抛物面，匀加速斜平面）。\\
3. \key{测管水头}：连通的同一静止液体内各点测管水头 $z+p/\rho g$ 恒定相等。流动流体中测管水头沿程由于摩擦阻力而下降。\\
4. \key{压力中心与形心}：平面受压面的压力中心\warn{总是低于形心}，因为静水压强随深度线性递增，下部压力所占的权重更大。\\
5. \key{控制体动量方程}：求解弯管反力或喷嘴螺栓受力时，必须正确划定控制体（CV），列出表面上的压力、重力及管壁反力，动量通量流出为正。
}
