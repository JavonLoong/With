...
3. \key{喷管核心结论}：\\
{\fontspec{SimSun}①} \key{加速原理}：由面速关系 $\frac{dA}{A} = (Ma^2-1)\frac{dV}{V}$ 决定。亚声速（$Ma<1$）在渐缩段（$dA<0$）加速；若想进一步加速至超声速（$Ma>1$），必须先在喉部（$dA=0$）达到声速 $Ma=1$（临界），再在渐扩段（$dA>0$）继续加速。故渐缩喷管出口最大只能达声速，渐缩渐扩（拉瓦尔）喷管出口可达超声速。\\
{\fontspec{SimSun}②} \key{临界流量}：最大质量流量（$\dot{m}_{\text{max}} = K \frac{p_0 A^*}{\sqrt{T_0}}$）唯一由喉部面积 $A^*$ 和上游滞止状态（即静止容器内的滞止压强 $p_0$ 和滞止温度 $T_0$）决定。喉部一旦达到声速（阻塞），下游扰动无法逆上传播，因此即使下游管道存在激波或出口有膨胀波，也不改变此最大流量。
}
