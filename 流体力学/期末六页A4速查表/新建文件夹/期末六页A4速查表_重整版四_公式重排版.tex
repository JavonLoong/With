...
2. \key{N-S方程}：不可压粘性流体运动微分方程，建立在动量守恒上。液体静止时，N-S方程\warn{退化为欧拉平衡方程}。\\
3. \key{边界条件}：理想流体为可滑移（法向速度为0，切向可滑动）；实际粘性流体为\warn{完全无滑移}（法向与切向流速均与固体壁面相同）。\\
...
3. \key{速度环量}：沿有向闭合曲线的速度积分。环量随时间变化（开尔文定理不成立）的三个因素为：\warn{质量力无势、流体斜压（密度依赖于温度等其他变量）、粘性作用}。粘性是产生和扩散旋涡的根源。\\
4. \key{卡门涡街}：绕圆柱在$Re=50\sim300$时产生周期性脱落的双列交错旋涡（$50\sim 150$最稳定），$Re>300$时脱落呈随机性并转为紊流。\\
5. \key{势函数与流函数}：不可压平面无旋流既有速度势又有流函数，它们都满足拉普拉斯方程，是\warn{调和函数/共轭调和}（满足柯西-黎曼条件）。\warn{等势线与等流函数线正交}。势函数和流函数在内部\warn{没有极大值或极小值}。只要是二维不可压流动，\warn{必然存在流函数}。
}

\infoh{选择判断秒杀库-6.流动阻力与层流紊流}
\infotxt{%
1. \key{层流损失}：圆管层流沿程水头损失与断面平均流速的\warn{一次方}成正比。\\
2. \key{紊流与雷诺应力}：紊流伴随脉动和掺混，断面流速呈对数分布。雷诺数很大时，\warn{分子粘性切应力可忽略，但由脉动引起的紊流附加切应力（雷诺应力）起控制作用，绝不能忽略}。时均方程不封闭，必须引入紊流模型（如混合长度）。\\
...
2. \key{紊流光滑区}：粗糙突起完全被掩盖在粘性底层内，阻力系数$\lambda$仅与雷诺数$Re$有关，与粗糙度无关。\\
3. \key{紊流过渡区}：粘性底层与粗糙高度相当，阻力系数$\lambda$与雷诺数$Re$和相对粗糙度均有关。\\
4. \key{紊流粗糙区（阻力平方区）}：粗糙突起完全暴露在紊流核心中，\warn{阻力系数$\lambda$仅与相对粗糙度$k_s/d$有关，与雷诺数无关}，损失与流速平方成正比。
}

\infoh{选择判断秒杀库-8.管嘴出流与水击现象}
\infotxt{%
1. \key{管嘴出流}：相同水头下管嘴流量为相同孔口流量的\warn{1.32倍}（管嘴收缩断面处\warn{存在真空区起抽吸作用}）。管嘴正常工作条件是管长$l=(3\sim4)d$，且真空度不超过限值。\\
2. \key{水击现象}：流量突变导致压强大幅交替波动。水击波速$c$受液体压缩性和管壁弹性缓冲，管壁弹性越好，波速越慢。关闭时间$T_s$小于水击相长$T$时为直接水击，否则为间接水击，\warn{间接水击增压低于直接水击}。
}

\infoh{选择判断秒杀库-9.力学相似与自模区}
\infotxt{%
1. \key{力学相似}：几何相似是前提，动力相似是保证（相应点受力比值相同），运动相似是目的。雷诺准则和弗劳德准则通常互有冲突，模型设计时应保证主导力的相似。\\
2. \key{自模区}：雷诺数很大进入阻力平方区时，流动进入\warn{自模化区}，\warn{只需保证几何相似即可}，雷诺数准则不需严格满足。
}

\infoh{选择判断秒杀库-10.气体动力学与激波}
\infotxt{%
1. \key{正激波物性}：气体通过正激波（激波参考系）：静压$p\uparrow$、静密$\rho\uparrow$、静温$T\uparrow$、静焓$h\uparrow$；\warn{总温$T_0$不变、总焓$h_0$不变}（绝热流）；\warn{总压$p_0\downarrow$}（因有熵增）；\warn{临界截面积$A^*$不变}（因流率守恒）。\\
2. \key{斜激波物性}：气体通过斜激波时，\warn{切向速度分量$V_t$保持不变}，只有法向速度分量$V_n$突跃减速。\\
3. \key{马赫数临界}：气流在渐缩管道中最大速度为临界声速；在渐扩管道中亚声速流减速、超声速流加速。
}
