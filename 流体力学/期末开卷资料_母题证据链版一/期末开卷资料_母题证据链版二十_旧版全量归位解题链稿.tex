...
\newcommand{\boxmini}[1]{\colorbox{warnbg}{\begin{minipage}{\dimexpr\linewidth-2\fboxsep\relax}\fontsize{2.60}{2.74}\selectfont #1\end{minipage}}\par}
...
\newenvironment{vfourWrap}{\par\begingroup\fontsize{2.60}{2.74}\selectfont\setlength{\fboxsep}{0.38pt}}{\par\endgroup}
\begin{document}
\fontsize{2.90}{3.02}\selectfont
\begin{center}
{\heiti\fontsize{6.1}{6.35}\selectfont 流体力学期末开卷资料：母题证据链版二十}\par
{\fontsize{3.18}{3.34}\selectfont 旧版 v106/v108/重整版五全量归位：所有内容必须落回“母题原题--子题变式--解题链--检查”，不另开孤立公式仓库。}
\end{center}
\vspace{-3.4pt}
\begin{multicols}{3}
...
\ans{\textbf{v20回填原则：}旧版里凡是“管路总包、可压概念、壁面律、量纲相似、外绕阻力、短问证明、查表动作、易错检查”的孤立块，都必须在对应母题里变成可落笔链：\textbf{看到什么}、\textbf{第一句写什么}、\textbf{未知量顺序}、\textbf{最后检查什么}。若考试只记住目录：液面管路找1，静水找2，CV受力找3，势流圆柱找4，Laval/激波找5，粘性解析找6，边界层阻力找7，速度应力找8，量纲水击找9，证明解释找10/12。}
\qt{子题：每道计算题的标准落笔骨架。}
...
\qt{母题1回填：管路/泵阀/粗糙度题型总包（旧版v106/v108完整链）}
...
\qt{母题3/4/5/6回填：组合题拆题链（跨母题时先拆再合）}
...
\ans{先用母题21从水池到喷嘴出口求$Q,V_e$和出口压力；若出口自由射流，$p_e=0$表压。再取喷流/板为CV用动量：固定平板法向力近似$F=\rho QV_e$；若射流折转角$\theta$，用矢量式$\sum F_x=\dot m(V_{2x}-V_{1x}),\sum F_y=\dot m(V_{2y}-V_{1y})$。检查：管路损失影响$V_e$，但打板力不再用能量方程直接算。}
...
\qt{母题7回填：边界层积分法完整题链。题干：给零压梯度平板、速度剖面 $u/U=f(\eta),\eta=y/\delta$，求 $\delta^*,\theta,\delta(x),C_f,D$。}
...
\qt{母题9回填：相似准则冲突题。题干：模型/原型、同流体缩尺、要求保持 Re 或 Fr，问速度、力、能否完全相似。}
...
\qt{母题16回填：水击/水波/明渠扰动。题干：阀门突然关闭、长管水锤；或给波长水深；或问扰动能否传上游。}
...
\qt{母题5回填：斜激波题完整链。题干：给来流 $M_1$、楔角/偏转角 $\theta$ 或波角 $\beta$，求波后状态。}
...
\qt{母题4回填：Rankine 半体/源汇偶极综合题。题干：均匀流加点源/点汇/偶极，求驻点、分界流线、实体形状。}
...
\qt{母题18回填：Kelvin/Lagrange/Helmholtz 概念证明题。题干：证明无旋保持、涡管强度守恒、解释势函数单值。}
