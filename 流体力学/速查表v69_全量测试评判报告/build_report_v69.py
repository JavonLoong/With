...
base.CHEATSHEET_PDF = ROOT / "期末六页A4速查表" / "期末六页A4速查表_最终主用版_已修正_v69.pdf"
...
bump_rule(
    "概念简答/定义解释",
    0.78,
    " v67 已补长论述结构，v69 补读图/收口检查；教材原话仍可能扣分。",
)
bump_rule(
    "流体运动学/连续/流线迹线",
    0.78,
    " v67 已补推导题固定结构，v69 补截面和边界条件收口检查。",
)
bump_rule(
    "水波/水击/课程概念缺口",
    0.76,
...
def replace_version(text: str) -> str:
    text = re.sub(r"(?<![A-Za-z0-9])v10(?![A-Za-z0-9])", "v69", text)
    text = re.sub(r"(?<![A-Za-z0-9])V10(?![A-Za-z0-9])", "V69", text)
    text = re.sub(r"(?<![A-Za-z0-9])v6(?![A-Za-z0-9])", "v69", text)
    text = re.sub(r"(?<![A-Za-z0-9])V6(?![A-Za-z0-9])", "V69", text)
    return text


def estimate_v69(rule: dict, question_text: str, group: str, max_score: float) -> tuple[float, str, str]:
    est, basis, confidence = old_estimate(rule, question_text, group, max_score)
    return est, replace_version(basis), confidence


base.estimate = estimate_v69


def postprocess_v69() -> None:
    literal_replacements = [
        ("期末六页A4速查表_v6_结构导航版.pdf", "期末六页A4速查表_最终主用版_已修正_v69.pdf"),
        ("期末六页A4速查表_v6_结构导航版.tex", "期末六页A4速查表_最终主用版.tex"),
        ("期末六页A4速查表_最终主用版_已修正_v10.pdf", "期末六页A4速查表_最终主用版_已修正_v69.pdf"),
        ("只靠 v69 得分", "只靠 v69 过程覆盖估计得分"),
        ("只靠 v69 折算得分", "只靠 v69 过程覆盖估计得分"),
    ]
    note = (
