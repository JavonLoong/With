...
\infotxt{%
1. \key{连续介质与物理性质}：将流体视为质点连续无间隙介质。温度升高，\warn{液体粘度下降}，\warn{气体粘度上升}。重力场下\warn{等温大气的气压随高度呈指数级下降}（$p=p_0 e^{-gz/RT}$），而非线性。因各向同性，牛顿流体本构中应力与变形速率张量的比例系数（粘度）与\warn{应力作用方向无关}。水滴与水中气泡内部压强总是大于外部（$\Delta p = 2\sigma/R$），极细测压管读数须修正毛细高度（$h = \frac{4\sigma}{\rho g d}$）。牙膏、油漆、煤泥水低于屈服应力时不流动，属于宾汉塑性流体。不可压声速为无穷大；$Ma < 0.3$ 可忽略压缩性。\\
2. \key{流线迹线与速度梯度}：在\warn{定常流动中流线与迹线完全重合}，且当地加速度为0。自转角速度为涡量的\warn{一半}，是纯运动学量（与粘性无关）。速度梯度张量可分解为对称张量（形变速率）与反对称张量（旋转张量）。速度梯度不为零时不一定会变形，如\warn{刚体旋转有速度梯度但应变率为零}。\\
3. \key{三大基本方程基本概念}：连续性方程源自质量守恒，粘性与理想流体形式完全相同。动能/动量修正系数因流速不均匀产生，层流修正系数大于紊流。管嘴出流量大于孔口，因收缩断面处存在真空起抽吸作用。渐变流流线近似平行直线，\warn{过流断面上静压分布仍满足静力学规律}（$z+p/\rho g = \text{const}$）。明渠中堰流无上部约束，闸孔出流受下闸门约束，以 \warn{$e/H \le 0.65$} 作为闸孔出流划分界限。\\
...
4. \key{当量直径与动力相似}：当量直径 $d_e = 4 R_h = 4 A / \chi$（水力半径 $R_h = A / \chi$，即过流面积比湿周），作用是将非圆截面折算为圆管阻力计算。重力主导选 Fr 相似，粘性主导选 Re 相似。模型试验中通常无法在普通风洞/水槽中\warn{同时满足雷诺数 $Re$ 与弗劳德数 $Fr$ 相似}（因两者的速度缩放比例要求冲突）。阻力平方区自模化时阻力系数与 Re 无关。欧拉数 $Eu = \Delta p / (\rho V^2)$ 是压力与惯性力之比，$Eu \gg 1$ 代表压力起主导作用。
}
