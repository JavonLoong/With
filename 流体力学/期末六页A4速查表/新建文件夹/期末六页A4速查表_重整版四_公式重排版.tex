...
\columnbreak
\infoh{选择判断秒杀库-1.流体物理性质与流体静力学}
\infotxt{%
1. \key{易流动性}：静止流体绝不能承受剪切力；运动状态下质点间有相对运动时才能抵抗剪切力。运动流体抵抗剪切变形的能力体现在\warn{剪切变形速率}而非变形大小。\\
2. \key{粘性与温度}：温度升高，\warn{液体粘度下降}（分子引力为主，受热膨胀分子间距变大），\warn{气体粘度上升}（分子运动动量交换为主）。粘性只在运动时表现，静止时为0。\\
3. \key{连续介质与不可压}：连续介质假设视流体由质点连续无间隙组成，物理量连续分布。不可压假设忽略密度变化，气体在流速远小于音速（$Ma<0.3$）时可视为不可压缩。\\
4. \key{静压强特性}：静止流体中一点的静压强\warn{大小与作用面方位无关}，方向垂直指向受压面（各向同性，只承受压应力）。\\
5. \key{静力平衡与等压面}：常密度不可压流体\warn{只有在有势质量力作用下}才能维持平衡。等压面同时是等势面，质量力\warn{必垂直于等压面}。如旋转液体等压面为抛物面，匀加速为斜平面。\\
6. \key{测管水头与压力中心}：连通的同一静止液体内各点测管水头$z+p/\rho g$恒定相等。平面受压的压力中心\warn{总是低于形心}（静水压随深度递增，下部压力权重高）。
}

\infoh{选择判断秒杀库-2.流体运动学、动力学与有势流动}
\infotxt{%
1. \key{定常与非定常}：定常流动时变加速度为零，流场中流速与压力不随时间改变，此时\warn{流线与迹线完全重合}。非定常流动中二者通常不重合。\\
2. \key{流函数存在的条件}：\warn{二维}（平面或轴对称）且\warn{不可压缩}流动，其本质是质量守恒方程（连续性方程 $\frac{\partial u}{\partial x} + \frac{\partial v}{\partial y} = 0$）。**与流动定常与否、有无粘性、有旋无旋完全无关**。\\
3. \key{势函数存在的条件}：流动\warn{无旋}（旋度 $\nabla \times \bm V = 0$）。**与流动的维数、压缩性、有无粘性、定常与否完全无关**。\\
4. \key{混淆防坑指南}：\\
  (1) \warn{有流函数不一定有势函数}：二维不可压有旋流动（如圆管层流）存在流函数，但无势函数。\\
  (2) \warn{有势函数不一定有流函数}：三维无旋流动（如三维势流）存在势函数，但因是三维流动，无普通的二维流函数。\\
  (3) \warn{有粘流动也可能有势函数}：只要局部无旋（$\nabla \times \bm V = 0$），粘性流动也可定义势函数。非定常流动同样可同时存在流函数与势函数。\\
  (4) \warn{拉普拉斯方程正交性}：当二者同时存在时，均满足拉普拉斯方程（$\nabla^2\phi=0, \nabla^2\psi=0$），且等势线与等流函数线正交。\\
5. \key{连续性与本构方程}：连续性方程仅源自质量守恒，故\warn{粘性流体与理想流体的连续性方程形式完全相同}。牛顿流体本构中，应力与变形速率张量为线性关系，且由于流体各向同性，该线性关系\warn{与应力作用方向无关}。\\
6. \key{动压强与控制方程}：动压强$p = -(\sigma_{xx}+\sigma_{yy}+\sigma_{zz})/3$是数量场，但不是任意方位面上的真正压应力。无粘流体Euler方程由于含有对流项$(\bm V\cdot\nabla)\bm V$，\warn{同样是非线性偏微分方程}（N-S方程亦然）。\\
7. \key{旋涡与速度环量}：涡量是速度旋度（$\bm\Omega=\nabla\times\bm V$），等于流体微团绕该点平均旋转角速度的\warn{两倍}。速度环量随时间变化的根源为：\warn{质量力无势、流体斜压、粘性作用}（粘性是旋涡产生和扩散的根源）。
}

\infoh{选择判断秒杀库-3.管路能量综合与流动阻力}
\infotxt{%
1. \key{圆管层流损失}：圆管层流沿程水头损失与断面平均流速的\warn{一次方}成正比。无论层流还是紊流，圆管过流断面上的切应力均呈\warn{线性分布}，管轴处切应力为0，管壁处切应力最大。\\
2. \key{粘性底层}：紧贴壁面的薄层，脉动消失，粘性力主导，速度呈\warn{线性分布}。雷诺数越大，该底层越薄。\\
3. \key{尼古拉兹阻力区}：层流区与紊流光滑区阻力系数$\lambda$仅与雷诺数有关，与粗糙度无关；紊流过渡区与两者均有关；紊流粗糙区（阻力平方区）\warn{仅与相对粗糙度有关，与雷诺数无关}，损失与流速平方成正比。\\
4. \key{管嘴出流真空区}：相同水头下管嘴流量为相同口径孔口流量的\warn{1.32倍}（收缩断面处\warn{存在真空区起抽吸作用}）。正常工作条件是管长$l=(3\sim4)d$，且真空度不超过限值。\\
5. \key{水击现象}：流量突变导致压强大幅波动。水击波速$c$受液体压缩性和管壁弹性缓冲。关闭时间$T_s$小于水击相长$T$时为直接水击，否则为间接水击，\warn{间接水击增压低于直接水击}。
}

\infoh{选择判断秒杀库-4.雷诺数与粘性/惯性流动}
\infotxt{%
1. \key{雷诺数的物理本质}：雷诺数 $Re = \rho V L / \mu$ 是惯性力与粘性力（剪切力）之比。\\
  (1) \warn{惯性力表现形式}：随体/对流加速度项引起的动量变化力，微元体尺度为 $\rho (\bm V\cdot\nabla)\bm V \sim \rho V^2 / L$。\\
  (2) \warn{粘性力表现形式}：由于剪切变形引起的粘性摩擦力，微元体尺度为 $\mu \nabla^2\bm V \sim \mu V / L^2$。\\
  (3) 二者之比 $(\rho V^2/L) / (\mu V/L^2) = \rho V L / \mu = Re$。\\
2. \key{低雷诺数流动（$Re < 1$ 蠕动流/斯托克斯流）特性}：\\
  (1) \warn{控制方程}：忽略了惯性项（质点加速度），由线性 Stokes 方程控制，**绝不能使用无粘 Euler 方程**。\\
  (2) \warn{阻力组成}：Stokes 小球总阻力 $D = 3\pi \mu d U$（阻力系数 $C_D=24/Re$），其中\warn{摩擦阻力占 2/3}，压差阻力占 1/3。\\
  (3) \warn{对称与无分离}：流线具有前后完全对称性，无任何流动分离。\\
  (4) \warn{流线型与圆球阻力}：在极低雷诺数下，摩擦阻力占绝对主导，流线型物体因湿表面积大，总阻力可能大于圆球。\\
3. \key{高雷诺数流动（$Re \gg 1$ 惯性/边界层流动）特性}：\\
  (1) \warn{控制方程非线性}：惯性项占主导。即使是无粘理想流体 Euler 方程，由于对流项存在，**同样是非线性偏微分方程**。\\
  (2) \warn{边界层与厚度}：近壁粘性起作用的薄层。由名义厚度标定的粘性外界线不是流线。积分求解须同时用 $\delta, \delta^*, \theta$ 三个厚度。\\
  (3) \warn{流动分离与逆压}：后部减速段存在逆压梯度（$dp/dx > 0$），壁面剪力降为零（$\tau_w=0$即$\partial u/\partial y|_w=0$）是分离的临界条件，分离后压差阻力剧增。\\
  (4) \warn{阻力危机}：临界雷诺数下边界层转捩为湍流，抗逆压梯度能力增强，分离点后移、尾迹变窄，使\warn{阻力系数 $C_D$ 骤降}。\\
  (5) \warn{相似律与自模化}：雷诺相似与弗劳德相似通常互相冲突。在阻力平方区发生\warn{自模化}时，阻力系数仅与相对粗糙度有关，此时只需满足几何相似即可。
}

\infoh{选择判断秒杀库-5.湍流与时均封闭模型}
\infotxt{%
1. \key{湍流发生与转捩}：流态转捩的临界雷诺数受壁面粗糙度和外界干扰影响，\warn{不是确定的常数}。\\
2. \key{雷诺应力与不封闭}：紊流中脉动引起的附加切应力（雷诺应力）起控制作用。不可压湍流时均动量方程（RANS）因脉动引起的\warn{雷诺应力项而未封闭}，须引入半经验湍流模型。\\
3. \key{雷诺应力分布}：紊流圆管切应力同样是呈线性分布的。管轴处时均切应力为0，管壁处最大。\\
4. \key{动能修正系数}：层流管流速度分布极不均匀，动能修正系数$\alpha=2.0$；紊流分布较均匀，$\alpha\approx 1.05\sim 1.10$，常近似为1.0。
}

\infoh{选择判断秒杀库-6.可压缩等熵流动与拉瓦尔管}
\infotxt{%
1. \key{声速与马赫数}：声速$c=\sqrt{\gamma RT}$仅是温度的函数。马赫数$Ma=V/c$反映可压缩性，当气流速度远小于音速（$Ma<0.3$）时可视为不可压缩。\\
2. \key{滞止状态与坐标系}：一维等熵可压缩气流的滞止压强$p_0$和滞止温度$T_0$等参考状态\warn{与坐标系的选取有关}（因流速具相对性，不同运动框架下测得的滞止量不同）。\\
3. \key{喉部临界与阻塞}：当喷管喉部达到临界声速（$Ma=1$）时发生\warn{阻塞}（壅塞）。一旦发生阻塞，喷管通过的质量流量达到最大值$\dot{m}_{\max}$，且仅由喉部面积和上游滞止状态（$p_0, T_0$）唯一决定，\warn{完全不受下游背压变化影响}。\\
4. \key{波流形态影响}：一旦喉部阻塞，上游滞止状态相同时，无论是在管内发生正激波还是在出口处存在膨胀波，其质量流量均\warn{相同}。
}

\infoh{选择判断秒杀库-7.正激波、斜激波与膨胀波}
\infotxt{%
1. \key{正激波状态变化}：气体通过正激波后，法向减速（超声速变为亚声速）。其\warn{静压、密度、静温、静焓均上升}；总温、总焓、临界横截面积 $A^*$ 保持不变（绝热流，质量流率守恒）；但由于熵增，\warn{总压（滞止压强）下降}。\\
2. \key{斜激波物性}：气体通过斜激波时，\warn{切向速度分量 $V_t$ 保持不变}，法向速度分量 $V_n$ 减速。波后总压下降，波后流动可能为超声速或亚声速。\\
3. \key{波前波后公式不互套}：激波是强非等熵过程，不能使用等熵公式贯穿波前波后；斜激波计算压力、温度比时必须使用法向马赫数 $M_{n1} = M_1 \sin\beta$，不能直接将 $M_1$ 代入正激波公式。\\
4. \key{Prandtl-Meyer膨胀波}：超声速流绕凸角流动（流道向外打开）时产生膨胀扇。膨胀波内流动为等熵过程，总压 $p_0$ 和总温 $T_0$ \warn{保持不变}；气体通过后马赫数增大，\warn{静压、静温、密度均下降}。
}

\infoh{选择判断秒杀库-8.期末真题判断与选择专练}
\infotxt{%
