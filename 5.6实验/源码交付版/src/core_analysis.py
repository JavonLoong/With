...
def find_ruler_band(frame: np.ndarray, ruler_roi: tuple[float, float] = (0.55, 0.78)) -> tuple[int, int]:
    gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
    h, w = gray.shape
    y0, y1 = int(h * ruler_roi[0]), int(h * ruler_roi[1])
    crop = gray[y0:y1]
...
def estimate_px_per_cm(frame: np.ndarray, out_dir: Path, ruler_roi: tuple[float, float] = (0.55, 0.78)) -> tuple[float, float, dict[str, float]]:
    h, w = frame.shape[:2]
    y0, y1 = find_ruler_band(frame, ruler_roi)
    crop = frame[y0:y1, :]
...
def extract_waterline(frames: list[np.ndarray], waterline_roi: tuple[float, float] = (0.32, 0.68)) -> tuple[np.ndarray, np.ndarray, tuple[int, int]]:
    ...
    y0, y1 = int(h * waterline_roi[0]), int(h * waterline_roi[1])
    xs = np.arange(w)
    lines = []
    for frame in frames:
...
def analyze_group(group_dir: Path, config: dict = None) -> dict[str, object]:
    if config is None: config = {}
    ruler_roi = tuple(config.get("ruler_roi", (0.55, 0.78)))
    waterline_roi = tuple(config.get("waterline_roi", (0.32, 0.68)))
    ...
    px_per_cm, px_spread, cal_diag = estimate_px_per_cm(frames[min(n - 1, n // 2)], group_dir, ruler_roi)
    dx_cm = 1.0 / px_per_cm
    xs_px, ylines, roi = extract_waterline(frames, waterline_roi)
