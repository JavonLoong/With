...
\infotxt{%
1. \key{连续介质与物理性质}：将流体视为质点连续无间隙介质。温度升高，\warn{液体粘度下降}，\warn{气体粘度上升}。重力场下\warn{等温大气的气压随高度呈指数级下降}（$p=p_0 e^{-gz/RT}$），而非线性。因各向同性，牛顿流体本构中应力与变形速率张量的比例系数（粘度）与\warn{应力作用方向无关}。不可压声速为无穷大；$Ma < 0.3$ 可忽略压缩性。\\
