...
\infotxt{%
1. \key{基本性质与压力定义}：任何流体\warn{都不能承受切应力而处于平衡状态}。液体的粘度随温度升高而降低，气体的粘度随温度升高而增加。理想流体是为处理问题方便而人为引入的理想模型（真实流体都是有粘性的），当动力粘性系数/速度变化率很小（剪应力极小）时可忽略不计。理想流体压力是唯一的表面力，作用力的大小与方向无关，方向垂直于作用面并指向内法线方向；粘性流体压力是\warn{三个主应力的算术平均值的负值}（即 $p = -(\sigma_x+\sigma_y+\sigma_z)/3$），忽略流体粘性时粘性流体的压力就蜕化为理想流体的压力。水滴与水中气泡内部压强总是大于外部（$\Delta p = 2\sigma/R$），极细测压管读数（液面高度）不直接是真实静压，须修正毛细上升（$h = \frac{4\sigma}{\rho g d}$）。牙膏、油漆、煤泥水低于屈服应力时不流动，属于宾汉塑性流体。\\
2. \key{流场描述与运动学}：在定常流场中流线、迹线、脉线完全重合。定常与非定常流动\warn{与坐标系选取有关}，只有当坐标系固定在物体上时周围流场才是定常流场（静止坐标系中为非定常）。在非定常流场中固定点连续施放颜色液形成的线是\warn{脉线}（而非流线或迹线）。染色流体质点随时间增长变形形成的抛物线代表\warn{流体线}（而非流线或速度分布）。在定常流场中速度不随时间变化，但\warn{还可能有迁移（位变）加速度}，故质点加速度不恒为零。刚体旋转有速度梯度但应变率为零。\\
3. \key{三大基本方程基本概念}：连续性方程 $\nabla\cdot\bm V=0$ \warn{适合不可压缩流体的任何流动}（定常/非定常均成立）。动量方程中的外力包含粘性切应力，\warn{既适用于理想流体，也适用于粘性流体}。静止流体等压面是水平面，不代表只受重力，\warn{除重力外还可以有垂直方向的惯性力}（等压面必垂直于合质量力方向）。渐变流过流断面上静压分布仍满足静力学规律。以 $e/H \le 0.65$ 划分堰流与闸孔出流。\\
...
4. \key{圆柱绕流与升力}：有速度环量 $\Gamma$ 产生升力 $L=\rho V \Gamma$（库塔-儒可夫斯基定理）。\key{K-J假设（后缘条件）}：在理想流体假设下流过平板（或机翼等）时，首尾端点速度绝对值为无穷大，与实际流动不符。为在理想流体假设下模拟真实流动，库塔-儒克夫斯基提出假设：在平板（或机翼等）有攻角绕流中，一定存在速度环量，其大小恰好能使背面的驻点移至后缘，使后缘端点的速度保持为有限值。
}
...
2. \key{雷诺应力与混合长度}：RANS方程因脉动应力不封闭。紊流圆管中总切应力沿断面呈线性分布。Prandtl\key{混合长度理论的物理意义}：将湍流中微团的脉动与气体分子的运动相比拟。假定流动微团横向移动混合长度 $l$ 后与周围流体混合并传递时均物理量。\\
...
\infotxt{%
1. \key{边界层外界线与摩擦阻力}：由名义厚度 $\delta$ 标定的外界线不是流线。排挤厚度 $\delta^*$ 和动量损失厚度 $\theta$ 均显著小于 $\delta$。平板层流阻力：积分求出摩擦阻力。\key{摩擦阻力定义}：当物体与流体有相对运动时与流体接触的物体表面要受到流体剪应力作用，剪应力的合力称为摩擦阻力。\\
2. \key{流动分离与卡门涡街}：绕流钝体两侧边界层因逆压与粘性脱流形成卡门涡街，频率 $f = St \frac{U}{d}$，阻力危机前 $St \approx 0.2$。流动分离的两个条件：存在逆压梯度区；壁面及粘性对流体的阻滞作用。高Re流线体后部仍会出现逆压区，在大攻角等工况下仍可能发生分离。\\
