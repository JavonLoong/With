...
\infotxt{%
1. \key{连续介质与物理性质}：将流体视为质点连续无间隙介质。温度升高，\warn{液体粘度下降}，\warn{气体粘度上升}。重力场下\warn{等温大气的气压随高度呈指数级下降}（$p=p_0 e^{-gz/RT}$），而非线性。不可压声速为无穷大；$Ma < 0.3$ 可忽略压缩性。\\
...
4. \key{无粘流体动力学}：理想流体无粘性（$\mu=0$），其控制方程 Euler 方程同样是非线性偏微分方程。伯努利方程沿流线成立条件为定常、不可压、无粘。若全场无旋，则伯努利常数全场相等。三维理想流体中因涡线拉伸弯曲，\warn{涡量沿流线通常不守恒}，仅二维理想不可压中守恒。\\
5. \key{水击现象防范}：水击是流速突变导致\warn{流体可压缩性与管壁弹性}引起的高频压力波，绝对不可压流体不会发生水击。防护包括延长关阀时间、加装安全阀/调压塔。
}
...
3. \key{混淆要点}：二维不可压有旋流动有流函数但无势函数；三维无旋流动有势函数但无二维流函数；有粘流动在局部无旋时也可以存在势函数。二维无旋流动中，\warn{流线与等势线总是正交的}。\\
...
3. \key{粘性流动解析解}：Couette流（剪切驱动，无压梯度下速度呈线性分布）；Poiseuille流（压力驱动，速度呈抛物线分布）。两层粘性流体交界面上，\warn{剪切应力与速度必须连续}。圆管完全发展段中速度剖面恒定，\warn{沿程压力梯度 $\frac{dp}{dx}$ 为常数}。\\
4. \key{当量直径与动力相似}：当量直径 $d_e = 4 R_h = 4 A / \chi$（水力半径 $R_h = A / \chi$，即过流面积比湿周），作用是将非圆截面折算为圆管阻力计算。重力主导选 Fr 相似，粘性主导选 Re 相似。阻力平方区自模化时阻力系数与 Re 无关。欧拉数 $Eu = \Delta p / (\rho V^2)$ 是压力与惯性力之比，$Eu \gg 1$ 代表压力起主导作用。
}
...
2. \key{流动分离与卡门涡街}：绕流钝体（如圆柱）两侧边界层因\warn{逆压梯度与粘性摩擦}脱流，剪切层不稳定，交替脱落形成排布整齐、旋转方向相反的涡列即卡门涡街。其释放频率 $f = St \frac{U}{d}$ 与流速 $U$ 成正比，与直径 $d$ 成反比，阻力危机前 $St \approx 0.2$。\\
3. \key{阻力危机}：球/圆柱在临界 $Re$ 附近边界层由层流转捩为湍流，抗逆压梯度能力增强，分离点后移、尾迹变窄，使\warn{阻力系数 $C_D$ 骤降}。
}

\infoh{选择判断秒杀库-7.期末真题判断与选择专练}
\infotxt{%
