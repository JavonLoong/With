...
\setlength{\columnsep}{1.45mm}
\setlength{\columnseprule}{0.13pt}
\setlength{\tabcolsep}{1.1pt}
\setlength{\fboxsep}{0.42pt}
\renewcommand{\baselinestretch}{0.64}
...
\newcommand{\mt}[1]{\vspace{0.35pt}{\fontsize{5.85}{6.05}\selectfont\bfseries\color{white}\colorbox{h}{\strut\ 母题：#1\ }}\par\vspace{0.15pt}}
\newcommand{\s}[1]{{\fontsize{4.85}{5.05}\selectfont\bfseries\color{b}$\blacktriangleright$ 子题：#1}\par}
...
\newcommand{\card}[2]{\vspace{0.18pt}\noindent\fcolorbox{grid}{white}{\begin{minipage}{0.972\linewidth}\fontsize{4.55}{4.78}\selectfont\textbf{\color{h}子题：#1}\par #2\end{minipage}}\par\vspace{0.18pt}}
...
\newcommand{\cvfig}{\tinyflow{\begin{tikzpicture}[>=Latex,scale=.50,every node/.style={font=\tiny}]
...
\newcommand{\pvzfig}{\tinyflow{\begin{tikzpicture}[>=Latex,scale=.48,every node/.style={font=\tiny}]
...
\node at (1.25,.22){A细};\node at (3.25,.18){B粗};\node at (2.15,.82){突扩};\node at (.45,1.38){C};\node at (4.85,1.02){D};\end{tikzpicture}}}
\begin{document}
\fontsize{4.65}{4.86}\selectfont
...
\pvzfig
\card{两油箱/水池--变径管PVZ主例}{图形：两液面差 $h$，左液面到管轴 $H$，细管A、粗管B，中间突扩。第一行：$p/\rho g+\alpha V^2/2g+z$ 左 = 右 + 损失。液面 $p=0,V=0$。短管求$Q$：$h=0.5V_s^2/2g+(V_s-V_b)^2/2g+1.0V_b^2/2g$，$V_s=Q/A_s,V_b=Q/A_b$。求A：$H=p_A/\rho g+1.5V_s^2/2g$。求B：从B到右液面，若只剩出口损失，则 $p_B/\rho g=H-h$。长管加 $\lambda L V^2/(d2g)$，层流 $64/Re$，湍流用题给Colebrook。局部损失速度必须取所在管段。}\pipefig
...
\mt{考场四问总控：每题先填这4格}
\card{1 看到什么用哪块}{水池/管/泵/阀$\to$能量；静水/U管/闸门$\to$静水；弯管/喷流/叶片$\to$CV动量；源汇涡/镜像/圆柱$\to$势流；$Ma,p_0,T_0,p_b,A^*$或激波$\to$可压；速度分布/剪应力$\to$粘性；平板/外绕物$\to$边界层阻力。}
\card{2 第一行固定句}{“取……为研究对象，设1截面为……、2截面为……，取……为正，在……假设下列……方程。” 这一行必须写，后面才不乱。}
\card{3 未知量顺序}{几何$A,L,d,R$ $\to$ 速度$V=Q/A$ $\to$ 判据$Re,Ma$ $\to$ 查表/选支$\lambda,C_D,M,\beta,\nu$ $\to$ 目标$Q,p,F,P,h,\delta$。}
\card{4 最后收口}{压强写表压/绝压；力写“谁对谁”和方向；功率写水功率/轴功率和效率；喷管写阻塞/未阻塞/激波位置；边界层写单面/双面和单位。}
\card{5 查表动作}{Moody先写$Re,\epsilon/d$；正激波查$M_1$；斜激波查$M_1,\theta$或$\beta$；PM查$M$或$\nu$；外绕阻力查$Re\to C_D$。只写“查表”不给入口会丢过程分。}
...
\mt{考场翻页顺序：先定位再代数}
\card{不会时先翻这里}{第1页定大类；管路/静水/CV在第1页；势流和喷管激波在第2页；缺量反推和改皮在第3页；扩展母题在第4--5页；三行模板、常数、最后检查在第6页。}
\card{每题强制四格}{题干钩子、第一行、未知量顺序、最后收口。若某题一时不会算数，先把这四格写出来，至少保过程分。}
...
\mt{v94 考场定位地图：只认 7 个入口}
\begin{tabularx}{\linewidth}{|p{.07\linewidth}|p{.29\linewidth}|p{.29\linewidth}|X|}\hline
\textbf{入口} & \textbf{题干/图像长相} & \textbf{第一行写法} & \textbf{先去哪块}\\\hline
A 管路能量 & 水池、长管、泵、阀、弯头、压力表、汽蚀、水轮机 & 取1上游自由面、2测点/泵口、3出口，列修正Bernoulli & 母题1；管路扩展；骨架1--4\\\hline
B 静水受力 & 静止液体、U管、闸门、曲面、浮体 & 写$p=p_0+\rho gh$，先形心压强或曲面分解 & 母题2；静水扩展；骨架5--7\\\hline
C 动量反力 & 弯管、喷流撞板、喷嘴、叶片、叶轮 & 取流体为CV，画$x/y$，写外力对流体 & 母题3；动量扩展；骨架8--9\\\hline
D 势流叠加 & 源汇涡、镜像壁、圆柱、环量、机翼 & 先写$\psi=\sum\psi_i,\varphi=\sum\varphi_i$ & 母题4/5；势流扩展；骨架12--13\\\hline
E 可压喷管 & $Ma,p_0,T_0,A^*,p_b$、Laval、激波、PM & 标$0,* ,e,b$或波前1波后2 & 母题6--9；可压判别树；骨架14--18\\\hline
F 粘性阻力 & 两板、圆管层流、湍流壁面、平板边界层、外绕球/柱 & 先算$Re$，再选解析/经验/查表 & 母题10--13；粘性/边界层扩展；骨架19--24\\\hline
G 代数概念 & 速度场、应力张量、量纲、证明、物质模型 & 写对象+假设+主方程/定义 & 母题14--17；骨架25--26\\\hline
\end{tabularx}
\mt{v94 入口内再分流：别扫碎片，按目标量找}
\card{A 管路能量内分流}{求$Q$：设$Q\to V\to Re\to\lambda\to h_L$；求$p$：从已知压强截面列到未知截面；求泵/水轮机功率：先求$H_p/H_T$再$P=\rho gQH$并判断效率；求汽蚀高度：用$p_{in}\ge p_v+\Delta p$。}
\card{B 静水内分流}{求点压强：沿竖直深度或U管液柱加减；求平面合力：$F=\rho gh_cA$，压力中心更深；求曲面：拆$F_H,F_V$；求浮力：排开液体重，方向竖直向上。}
\card{C 动量内分流}{弯管：压力力+支座力+重力；喷流打板：自由射流表压0，速度方向变；喷嘴反力：先能量求速度/压强，再动量；叶轮：只取切向动量矩。最后按题问对象取反。}
\card{D 势流内分流}{给基本流：直接叠加；问壁面：令$\psi=C$或镜像；问驻点：速度为0；问压力：速度进Bernoulli；问升力：有环量用$L'=\rho U\Gamma$。}
\card{E 可压内分流}{收缩喷管：先比$p_b/p_0$和0.528；Laval：先面积比取支，再比$p_e,p_b$；正激波：$1\to2$且$p_0$降；斜激波：压缩角查$\theta-\beta-M$；PM：膨胀角用$\nu_2=\nu_1+\delta$。}
\card{F 粘性阻力内分流}{圆管层流：$64/Re,u_{max}=2\bar u$；湍流管：Moody；两板：N-S化成二阶ODE；边界层：先$Re_x/Re_L$；外绕阻力：查$C_D(Re)$；Stokes：仅$Re<1$。}
\card{G 代数概念内分流}{速度场：散度/旋度/物质导数；应力：$\bm t=P\bm n$；量纲：列变量选重复变量；概念证明：定义、条件、方程、物理意义、限制。}
\warnline{考试翻找规则：先找 A-G，再找“求什么量”。不要在几十个弱化动作里扫；那些旧碎片已隐藏，保留在源码中仅作备份。}
\iffalse
...
\card{管网+机械能母题}{图像：水箱/水泵/水轮机/长管/弯头阀门。关键词：输出功率、安装高度、局部损失、粗糙度。变量：1上游自由面，2机器前后截面，3出口。顺序：$Q\to V\to Re\to\lambda\to h_f,h_\zeta\to H_p/H_T/p$。变体：多管径分段算；表压用$p/\rho g$，自由面表压为0；泵给能量写左边，损失写右边。易错：机器功率$P=\rho gQH/\eta$，不是直接把kW当压强。}
\card{汽蚀/吸水高度母题}{图像：离心泵从水池吸水，泵在水面上方。关键词：汽化压、入口压至少高于汽化压、最大安装高度。变量：1水池面，2泵入口。顺序：由1到2列能量，$p_2\ge p_v+\Delta p_{safe}$，解$z_2-z_1=h$。公式链：$p_a/\rho g=z_2+p_2/\rho g+V^2/2g+h_f+h_\zeta$。变体：有进口阀/弯头加入$\zeta V^2/2g$；粗糙度用于查$\lambda$。易错：汽化压要用绝压，不能用表压。}
\card{静水曲面/闸门母题}{图像：弯管、玻璃管、圆弧闸门、斜板闸门。关键词：压力分布、合力、压力中心、力矩平衡。变量：x水平、z竖直向下，取受压面投影。顺序：先水平分力$F_x=p_cA_v$，再竖直分力=虚水重量，最后合成与力矩。变体：倾斜面用形心深度$h_c$；曲面竖直力用上方水体体积。易错：压力中心比形心更深；方向是水对壁，不是壁对水。}
\card{CV动量母题}{图像：喷流打板、弯管受力、消防水枪、渐缩喷嘴、叶片。关键词：作用力、支座力、冲击力、动量方程。变量：CV包住流体，1入口，2出口，外力对流体为正。顺序：连续求$\dot m$；列压力力/重力/支座力；用$\sum F=\dot m(V_2-V_1)$；题问固体受力则取反。变体：二维分量分别列；自由射流出口表压0；速度方向要投影。易错：压力力方向按外法线反向判断，支座力和流体对管力相反。}
\card{伯努利+非定常振荡母题}{图像：U形管、摆动水柱、液面高度差随时间变。关键词：非定常、忽略粘性、振荡周期。变量：沿流线从液面1到2，速度相同$V=\dot z$。顺序：非定常伯努利/能量法得到$L\ddot z+2gz=0$，再读周期。公式链：$z=z_0\cos\sqrt{2g/L}t$，$T=2\pi\sqrt{L/2g}$。变体：若有损失会阻尼；若截面不等要用连续换速度。易错：定常伯努利不能直接用于振荡加速度项。}
\card{势流叠加母题}{图像：均匀流+源/汇/涡/偶极，壁面镜像，半体/Rankine体。关键词：流函数、势函数、驻点、边界为流线。变量：$\psi=\sum\psi_i,\varphi=\sum\varphi_i$，极坐标$r,\theta$。顺序：写基元流；叠加；令速度或$\psi$满足边界；求驻点/边界形状；伯努利求压强。变体：壁面用镜像；半平面源汇分母由$2\pi$变$\pi$；点涡只改切向速度。易错：流线边界用$\psi=$常数，不是$\varphi=$常数。}
\card{圆柱/升力母题}{图像：圆柱绕流、叠加环量、上下速度不同。关键词：圆柱绕流、环量、升力、达朗贝尔佯谬。变量：半径$a$，表面$r=a$。顺序：无环量先写$v_r=0,v_\theta=-2U\sin\theta$；有环量加$\Gamma/(2\pi a)$；用伯努利得$C_p$；升力$L'=\rho U\Gamma$。变体：求驻点令表面切向速度0；机翼用环量给升力。易错：势流阻力为0不是现实阻力为0。}
\card{Laval喷管状态母题}{图像：大容器+收缩/扩张喷管+背压。关键词：背压、阻塞、临界、喉部、出口压力。变量：0滞止，*喉部，e出口内状态，b外界背压。顺序：用$p_b/p_0$判是否可能阻塞；若阻塞喉部$M=1$；用面积比求出口等熵$M_e$；比较$p_e$与$p_b$决定波系。变体：收缩喷管无超声速扩张段；Laval有亚/超双支；内激波后总压变。易错：$p_e$是管内出口静压，$p_b$是外界背压，二者不总相等。}
\card{管内正激波母题}{图像：Laval扩张段内有一条近似垂直激波。关键词：管内激波、波后亚声速、总压损失。变量：1激波前，2激波后，02为波后总压。顺序：先由面积比求波前$M_1$；正激波表/式得$M_2,p_2,T_2,p_{02}$；波后按新的$p_{02}$继续等熵到出口。变体：给背压反求激波位置要试算$A_s/A^*$。易错：激波跨越不是等熵，不能用同一个$p_0$算波后。}
\card{斜激波/膨胀扇母题}{图像：楔角、斜直波线、外凸转角多条扇形线。关键词：压缩角、膨胀角、弱解、Prandtl-Meyer。变量：来流$M_1$，偏转角$\theta$，激波角$\beta$。顺序：压缩角用斜激波，先$M_{n1}=M_1\sin\beta$再正激波法；膨胀角用$\nu(M_2)-\nu(M_1)=\delta$。变体：多折角逐段更新；脱体激波说明角度过大。易错：膨胀波$p_0$不变，激波$p_0$下降。}
\card{粘性层流解析母题}{图像：两平板、倾斜板、圆管、两层流体。关键词：充分发展、N-S简化、无滑移、剪应力连续。变量：x流向，y法向，板间距H。顺序：写假设$u=u(y)$；N-S化为$\mu u''=dp/dx-\rho g_x$；积分两次；代边界；求$Q,\tau$。变体：Couette加板速边界；Poiseuille靠压差；倾斜加$\rho g\sin\theta$。易错：压力梯度符号按x正向，剪应力是$\mu du/dy$。}
\card{湍流壁面/管流母题}{图像：速度剖面、壁面近区、管径、粗糙度。关键词：$u_*,y^+,u^+$、对数律、Moody、雷诺应力。变量：$u_*=(\tau_w/\rho)^{1/2}$。顺序：由$Re$判层/湍；若近壁给$y$求$y^+$选线性或对数律；若压降求$\lambda$。变体：光滑管可用Blasius，粗糙管必须查Moody。易错：$\bar u$平均速度，$u_{max}$最大速度，$u_*$摩擦速度，三者不能混。}
\card{边界层/平板阻力母题}{图像：平板前缘、边界层厚度、层流/湍流分段。关键词：$Re_x$、转捩、$C_f$、阻力。变量：x从前缘，L板长，b板宽。顺序：算$Re_L$与$Re_{cr}$；确定层流/湍流/混合；求平均$C_f$；$D=C_f(\rho U^2/2)A$。变体：双面乘2；尾部边界层厚度用$x=L$；给局部位置用局部$C_{fx}$。易错：板面积是湿表面积，外绕圆柱用迎风面积。}
\card{外绕流阻力/终速母题}{图像：球、圆柱、小球沉降、风吹物体。关键词：阻力系数、终端速度、Stokes、Re<1。变量：迎风面积A，特征长度d。顺序：先估Re；低Re用$F_D=3\pi\mu dV$，一般用$F_D=C_D\rho V^2A/2$；终速令重力-浮力=阻力。变体：Cd依赖Re需迭代；水中有浮力。易错：球迎风面积$\pi d^2/4$，不是表面积。}
\card{量纲/模型试验母题}{图像：原型-模型、风洞/水槽、变量列表。关键词：相似、Pi定理、Re/Fr/Ma。变量：重复变量常选$\rho,V,L$。顺序：列变量和量纲；选重复变量；构造无量纲群；决定主导准则；换算力/功率。变体：船舶重力波用Fr；管内黏性用Re；高速气体用Ma。易错：不能同时满足所有准则时要说明主导物理。}
\card{速度场运动学母题}{图像：给$u,v,w$或流线图。关键词：流线、迹线、散度、旋度、速度势、流函数。变量：平面流$u=\psi_y,v=-\psi_x$。顺序：$\nabla\cdot V$判不可压；$\nabla\times V$判无旋；无旋求$\varphi$，不可压平面求$\psi$；流线解$dy/dx=v/u$。变体：非定常迹线要解ODE；定常时流线=迹线。易错：有流函数不等于无旋，有势函数才无旋。}
\card{应力张量母题}{图像：给矩阵$P$和法向$\bm n$，求应力矢量/法向/切向。关键词：单位法向、应力矢量、夹角。变量：$\bm t=P\bm n$。顺序：先单位化法向；算$\bm t$；$t_n=\bm t\cdot\bm n$；切向$\bm t_s=\bm t-t_n\bm n$。变体：平面方程法向来自系数；切应力大小取模。易错：不单位化会把结果整体放大。}
\card{证明/概念母题}{图像：无图，问适用条件/推导/物理意义。关键词：证明、说明、为什么、条件。变量：先写模型假设。顺序：对象$\to$守恒律$\to$假设化简$\to$结论$\to$限制。变体：伯努利必须说明定常/无粘/沿流线或无旋全场；NS必须说明牛顿流体。易错：概念题不要只写口水话，必须落到方程。}
...
\mt{末页常数/判据：带用法的小表}
\card{水和单位}{液体能量题常用。$\rho\approx1000kg/m^3$，$g=9.81m/s^2$，$1mH_2O\approx9.81kPa$。先把kPa、Pa、m水柱统一。}
\card{空气和绝温}{可压题用。空气$\gamma=1.4,R=287J/(kg\cdot K)$，$T(K)=t+273.15$，$a=\sqrt{\gamma RT}$。状态方程必须绝压绝温。}
\card{管流Re判层湍}{看到管内流动先算$Re=Vd/\nu$。$Re<2300$层流，$>4000$湍流；层流$\lambda=64/Re$，湍流按$Re,\epsilon/d$查Moody。}
\card{可压缩判据}{看到$Ma$或喷管/激波不要用不可压。普通气流$Ma<0.3$才可近似不可压；喷管/激波题必须按等熵或激波链。}
\card{边界层转捩}{平板先算$x_{cr}=Re_{cr}\nu/U$，常取$Re_{cr}=5\times10^5$。若$x_{cr}<L$，前层后湍，阻力分段。}
\card{沿程损失}{长直管用：$h_f=\lambda(L/d)V^2/(2g)$，$\Delta p_f=\lambda(L/d)\rho V^2/2$。多段串联逐段相加。}
\card{局部损失}{弯头、阀门、入口出口用：$h_\zeta=\zeta V^2/(2g)$。速度选该部件所在管径；突然扩大可用$(V_1-V_2)^2/(2g)$。}
\card{总损失位置}{能量方程中损失永远非负，放在下游侧：上游能量+泵头=下游能量+损失+水轮机头。}
\card{喷管临界}{只有某截面$M=1$时才用：$T^*/T_0=0.833$，$p^*/p_0=0.528$，$\rho^*/\rho_0=0.634$。不要把0.528当所有背压结论。}
\card{质量流量}{所有流动可写$\dot m=\rho VA$；阻塞喷管必须用喉部临界状态求最大流量，不能拿出口任意状态代。}
\card{激波/膨胀方向}{压缩超声转角$\to$斜激波或正激波；膨胀外角$\to$PM扇。激波$p,T,\rho$升、$p_0$降；PM等熵$p_0$不变。}
\card{伯努利范围}{真实管路用修正Bernoulli+损失；势流用Bernoulli求压强；跨激波不能用同一条等熵Bernoulli。}
...
\mt{考场三行过程模板：每类题直接抄结构}
\card{能量题}{触发：水池/管/泵/阀/压力表。1 取截面列修正Bernoulli；2 代$V=Q/A$和$h_L=\sum(\lambda L/d+\zeta)V^2/2g$；3 解目标并写$\lambda$来源、表压/水头单位。}
\card{动量题}{触发：弯管/喷流/叶片。1 取流体为CV并画正方向；2 写压力、重力、支座力；3 列$\sum F=\dot m(\bm V_2-\bm V_1)$，最后按题问对象取反。}
\card{静水题}{触发：静止液体/U管/闸门。1 写$p=p_0+\rho gh$；2 平面用$F=\rho gh_cA$，曲面拆$F_H,F_V$；3 压力中心/力矩平衡，说明方向。}
\card{势流题}{触发：源汇涡/镜像/圆柱。1 写基本解并叠加$\varphi,\psi$；2 用$\psi=C$、不穿透或驻点定常数；3 由速度+Bernoulli求压强/力。}
\card{喷管题}{触发：$p_0,T_0,A^*,A_e,p_b$。1 标$0,* ,e,b$；2 用等熵和面积比求$M,T,p$或阻塞流量；3 比$p_e,p_b$判阻塞、激波、膨胀。}
\card{激波题}{触发：正激波/斜激波/楔角。1 标波前1、波后2；2 正激波或斜激波关系更新$M,p,T,p_0$；3 波后用新$p_{02}$继续等熵。}
\card{粘性解析}{触发：两板/圆管层流/速度分布。1 写假设并化简N-S；2 积分两次；3 代无滑移、速度连续、剪应力连续求常数和$\tau,Q$。}
\card{边界层}{触发：平板/外绕物/边界层厚度。1 算$Re_x$或$Re_L$；2 判层流/湍流/混合；3 选$\delta,C_f,D$，检查单面/双面面积。}
\card{相似}{触发：模型/风洞/量纲。1 列变量和基本量纲；2 选重复变量构造$\pi$群；3 写相似准则，按$F\sim\rho V^2L^2$等换算。}
\card{概念证明}{触发：证明/说明/物理意义。1 写对象和假设；2 写守恒方程或定义；3 化简到结论并写适用限制/反例。}
...
\mt{v93：老师最后9张截图 $\to$ 考场入口}
\card{01 势流叠加总览}{触发：$\varphi,\psi$、源/汇/涡/圆柱/镜像/环量。入口：第1页母题4/5；第6页骨架12/13；先写总$\psi,\varphi$，再速度，最后Bernoulli或$L'=\rho U\Gamma$。}
\card{02 激波总览}{触发：正激波、斜激波、脱体激波、超声速压缩。入口：第1页母题7/8；第6页骨架16/17；先标1/2，再查$M_1$或$M_1,\theta$。}
\card{03 PM膨胀波}{触发：超声速外角、膨胀扇、Prandtl-Meyer角。入口：第1页母题9；写$\nu(M_2)=\nu(M_1)+\delta$，再等熵求$p,T$。}
\card{04 Laval喷管状态图}{触发：$p_0,T_0,A^*,A_e,p_b$、阻塞、背压变化。入口：第1页母题6；第6页原题缩影1/2；先判0/*/e/b，再比$p_e,p_b$。}
\card{05 粘性流动总览}{触发：Re、N-S、管内压损、外绕阻力。入口：第1页母题10/11/13；第4页管路扩展；先算$Re$，再决定解析解/经验公式/查Moody。}
\card{06 边界层理论}{触发：平板、边界层厚度、摩擦阻力、Karman。入口：第1页母题12；第6页骨架23和v90补丁；先算$Re_x/Re_L$判层/湍/混合。}
\card{07 守恒律/连续介质}{触发：欧拉/拉格朗日、控制体、守恒律证明。入口：第1页母题15/17；第6页概念证明；写对象、假设、守恒式、化简、限制。}
\card{08 物质模型地图}{触发：静止/不可压无粘/可压无粘/不可压有粘分类。入口：第1页母题索引；先判物质模型，再选静水、势流、可压或粘性。}
\card{09 考试要求}{允许教材+6页A4。执行：先用本表定位模型和过程，数值表/长表从教材查；本表只负责入口、公式链、查表关键词和易错检查。}
\card{最后20秒保底}{不会做也写：题型+假设；截面/CV/波前波后；主方程；查表入口；方向/单位/表压绝压。空白比不完整过程更亏。}
\mt{v94 第6页总索引：按“要求什么”找}
\begin{tabularx}{\linewidth}{|p{.19\linewidth}|p{.26\linewidth}|X|}\hline
\textbf{题目要求} & \textbf{先找入口} & \textbf{立刻写什么}\\\hline
求流量$Q$ & A管路 / 文丘里 / 喷管 & 液体：$Q=AV$进能量；气体：先判阻塞，$\dot m=\rho VA$或临界流量。\\\hline
求压强$p$ & A管路 / B静水 / E可压 & 液体低速多表压；气体状态方程/激波/汽蚀用绝压；先写截面位置。\\\hline
求力$F$ & B静水 / C动量 / F外绕 & 静水按压强积分；动量按$\sum F=\dot m(V_2-V_1)$；外绕按$D=C_D\rho U^2A/2$。\\\hline
求功率$P$ & A泵水轮机 / C叶轮 & 泵轴功率$\rho gQH_p/\eta$；水轮机输出$\eta\rho gQH_T$；叶轮用$P=M\omega$。\\\hline
求速度分布 & F粘性解析 / 湍流壁面 & 先写N-S简化；层流积分；湍流近壁先算$u_*,y^+$。\\\hline
求边界层厚度/阻力 & F边界层 & 先算$Re_x,Re_L$；判层/湍/混合；再选$\delta,C_f,D$。\\\hline
求马赫数/温压 & E可压 & 等熵段用$p_0/p,T_0/T,A/A^*$；激波后换新$p_{02}$。\\\hline
求角度/波系 & E斜激波/PM & 压缩角查$\theta-\beta-M$；膨胀角用$\nu(M_2)=\nu(M_1)+\delta$。\\\hline
求$\psi,\varphi$ & D势流 & 基本流相加；壁面$\psi=C$；速度由导数；压力再Bernoulli。\\\hline
求应力/加速度/相似 & G代数概念 & 应力$\bm t=P\bm n$；加速度$D\bm V/Dt$；相似先列$\pi$群。\\\hline
\end{tabularx}
\mt{v94 查表入口总表：翻教材前先写这行}
\card{Moody图}{入口：$Re=Vd/\nu$ 和相对粗糙度$\epsilon/d$。层流不用查，直接$\lambda=64/Re$。}
\card{等熵/面积-M表}{入口：已知$M$查$p_0/p,T_0/T,A/A^*$；已知$A/A^*>1$有亚声/超声两支，必须由位置和背压选支。}
\card{正激波表}{入口：波前$M_1>1$。输出$M_2,p_2/p_1,T_2/T_1,\rho_2/\rho_1,p_{02}/p_{01}$；跨波后不要继续用原$p_0$。}
\card{斜激波图/表}{入口：$M_1+\theta$求$\beta$，或$M_1+\beta$求$\theta$。若$\theta$过大，附体斜激波不存在，写脱体激波。}
\card{PM表}{入口：$\nu(M)$或给定转角$\delta$。外凸膨胀才用；PM等熵，$p_0,T_0$不变。}
\card{$C_D-Re$图}{入口：外绕物形状和$Re=Ud/\nu$。面积用迎风面积，球/圆柱不要用湿表面积。}
\mt{v94 最终卷面检查顺序}
\card{1 模型}{A-G 是否选对：静水、能量、CV、势流、可压、粘性、代数概念。}
\card{2 截面/对象}{截面是否放在均匀速度或已知压强处；CV 是流体还是固体；波前波后有没有标清。}
\card{3 单位}{Pa/kPa/m水柱统一；摄氏度转K；面积$m^2$；流量$m^3/s$或$kg/s$。}
\card{4 方向}{力题最后写“谁对谁”；支座/管壁/流体互为反向；角度题写相对来流偏转方向。}
\card{5 压力}{液体能量多表压；气体状态方程、汽蚀、激波、喷管用绝压；背压$p_b$不是总压。}
\card{6 假设回检}{用层流公式后回检$Re$；用Stokes后回检$Re<1$；用等熵后确认没有跨激波/损失；用Bernoulli后确认条件。}
\mt{v94 不会做时的最低可写答案}
\card{模板}{“本题属于入口\_\_。取\_\_为研究对象，设截面1为\_\_、截面2为\_\_，取\_\_方向为正。在\_\_假设下列\_\_方程。已知量先算\_\_，查表入口为\_\_，最后目标量为\_\_，单位\_\_，方向/表压绝压为\_\_。”}
\mt{v94 A-G 典型小题模板：看到题干直接套}
\card{A1 水池到出口求$Q$}{图像：水池+管+出口大气。写：1水面、2出口，$p_1=p_2=0$表压，$V_1\approx0$；$z_1-z_2=V^2/2g+h_L$；$V=Q/A$，$h_L=(\lambda L/d+\sum\zeta)V^2/2g$。}
\card{A2 泵入口汽蚀/安装高度}{图像：泵高于水面吸水。写：水面1到泵入口2；$p_a/\rho g=z_2+p_2/\rho g+V^2/2g+h_L$；要求$p_2\ge p_v+\Delta p$，解$z_2-z_1$。}
\card{A3 管路压力表/泵功}{图像：管上压力表、泵或水轮机。写：压力表表压直接进$p/\rho g$；能量式解$H_p/H_T$；泵轴功率除效率，水轮机输出乘效率。}
\card{B1 U管/差压计}{图像：U形管或两点压差。写：沿液柱走，向下加$\rho gh$、向上减$\rho gh$；同一静止连通液体同高压强相等；最后按题问表压或绝压。}
\card{B2 平面/曲面闸门}{平面：$F=\rho gh_cA$，$h_p=h_c+I_G/(h_cA)$；倾斜面深度仍取竖直深度。曲面：$F_H=$竖直投影压力，$F_V=$假想水体重。}
\card{C1 弯管支座力}{图像：管流转弯。写：取管内流体为CV；压力力压入截面，支座力先假设；列$\sum F_x,\sum F_y=\dot m(V_2-V_1)$；题问管受力则取反。}
\card{C2 射流打板/喷嘴反力}{自由射流出口表压0；固定光滑板速度大小近似不变，只改方向；喷嘴先能量求出口速度，再动量求反力。}
\card{D1 源汇涡镜像}{先写总$\psi$或$\varphi$；壁面不穿透就是$\psi=C$；镜像源同号、镜像涡反号常见；驻点令速度分量为0。}
\card{D2 圆柱/环量/升力}{圆柱表面$v_\theta=-2U\sin\theta+\Gamma/(2\pi a)$；$C_p=1-(v_\theta/U)^2$；无环量阻力0；有环量升力$L'=\rho U\Gamma$。}
\card{E1 收缩喷管/最大流量}{大容器入口速度可忽略：$p_0,T_0$已知。若$p_b/p_0>0.528$，未阻塞$p_e=p_b$；若$\le0.528$，喉/出口$M=1$，用临界流量。}
\card{E2 Laval/管内激波}{先由$A/A^*$取支求波前$M$；无激波等熵算$p_e$；若有正激波，波前等熵、过激波、波后用新$p_{02}$继续。}
\card{E3 斜激波/PM}{压缩角用斜激波：查$M_1,\theta\to\beta$；膨胀外角用PM：$\nu(M_2)=\nu(M_1)+\delta$。多折角逐段更新状态。}
\card{F1 两板/圆管层流}{先写充分发展N-S。Couette无压差线性；Poiseuille压差驱动抛物线；圆管层流$u_{max}=2\bar u$，$\lambda=64/Re$。}
\card{F2 湍流壁面/边界层}{湍流近壁先算$u_*=\sqrt{\tau_w/\rho}$和$y^+=yu_*/\nu$。平板先算$Re_x/Re_L$，再判层流/湍流/混合，面积注意单面/双面。}
\card{F3 外绕球柱/沉降}{先算$Re=Ud/\nu$查$C_D$；阻力$D=C_D\rho U^2A/2$；小球Stokes只在$Re<1$用，终速算完必须回检$Re$。}
\card{G1 速度场/应力/量纲}{速度场：不可压看散度，无旋看旋度，加速度用物质导数；应力：先单位法向，再$\bm t=P\bm n$；量纲：列变量、选重复变量、构造$\pi$群。}
\card{G2 概念证明}{固定写法：定义一句；适用条件一句；主方程一句；物理意义一句；限制/反例一句。伯努利、雷诺数、边界层、达朗贝尔佯谬、激波都按这个结构写。}
\mt{v94 版面使用原则}
\card{为什么不保留60条弱化动作}{考试时你没有时间在几十条相似小卡里扫描。v94 保留了源码备份，但打印版只显示 A-G 大入口和目标量索引；先定位，再去对应母题块细算。}
\mt{v94 满分过程句库：写不出来就抄骨架}
\begin{tabularx}{\linewidth}{|p{.16\linewidth}|X|}\hline
A 能量句 & 取1为\_\_、2为\_\_，以\_\_为基准面，列修正伯努利：上游能量+泵头=下游能量+沿程损失+局部损失+水轮机头。\\\hline
B 静水句 & 液体静止，压强只随竖直深度变化，故$p=p_0+\rho gh$；受压面合力通过压力中心，压力中心比形心深。\\\hline
C 动量句 & 取流体为控制体，外力包括压力力、重力和壁面对流体作用力；由动量方程求出外界对流体的力，再按题问对象取反。\\\hline
D 势流句 & 平面不可压无旋，存在$\varphi,\psi$且可线性叠加；固壁是不穿透流线，求得速度后由Bernoulli求压强。\\\hline
E 可压句 & 喷管内无激波段按等熵，跨激波不能等熵；先标滞止、喉部、出口、背压，再比较出口内压与背压。\\\hline
F 粘性句 & 先用$Re$判状态；简单层流由N-S化简积分，工程湍流用经验摩阻或Moody，外绕阻力用$C_D-Re$。\\\hline
G 概念句 & 本结论成立条件为\_\_；若条件不满足，如有粘性损失、激波、非定常或跨流线，则不能直接使用该简式。\\\hline
\end{tabularx}
\mt{v94 模块陷阱清单：最后一分钟扫}
\begin{tabularx}{\linewidth}{|p{.17\linewidth}|X|}\hline
A 管路 & 粗糙度没用、局部损失速度取错、泵/水轮机效率方向错、表压绝压混用。\\\hline
B 静水 & 倾斜长度当深度、合力放在形心、曲面直接用曲面面积、U管没有沿液柱逐段加减。\\\hline
C 动量 & 管对流体和流体对管反了、压力力方向画反、漏掉重量、出口自由射流表压没置0。\\\hline
D 势流 & 不可压和无旋混淆、极坐标漏$1/r$、镜像涡号错、把势流圆柱阻力0当现实阻力。\\\hline
E 可压 & 用0.528乱判所有出口超声、跨激波继续用原$p_0$、把背压当总压、PM和斜激波用反。\\\hline
F 粘性 & 平均速度当最大速度、单面/双面面积错、Stokes没回检$Re<1$、湍流壁面没先算$u_*,y^+$。\\\hline
G 概念 & 只写结论不写条件；证明题没写对象和假设；量纲题漏变量；应力法向没单位化。\\\hline
\end{tabularx}
\mt{v94 数值回检：算完用这个判离谱}
\card{压力数量级}{水头$1m\approx9.81kPa$；若普通水管算出几百MPa，通常单位或水头错。气体状态方程用Pa和K。}
\card{流速/流量}{管流$V=Q/A$，$A=\pi d^2/4$。若直径用mm没转m，速度会错$10^6$量级。}
\card{Re/Ma}{管流层湍公式先看$Re$是否匹配；可压题算出$Ma$后要和选的亚声/超声支一致。}
\card{功率}{泵/水轮机功率数量级用$\rho gQH$估。若$Q$是$m^3/h$，先除3600。}
\card{面积}{管流截面、平板湿表面积、外绕迎风面积三者不同；边界层阻力常用湿表面积，压差阻力常用迎风面积。}
\card{答案文字}{最后一句必须补：单位、方向、表压/绝压、是否乘效率、查表来源。}

\mt{v94 题干关键词字典：先扫这些词}
\begin{tabularx}{\linewidth}{|p{.23\linewidth}|p{.13\linewidth}|X|}\hline
\textbf{看到的词/图} & \textbf{入口} & \textbf{第一动作}\\\hline
水池、管、弯头、阀、泵、压力表 & A & 画1、2截面，写修正Bernoulli，先算$Q,V,Re,\lambda,h_L$。\\\hline
水面、闸门、U管、浮体、静止液体 & B & 写$p=p_0+\rho gh$；受力题先定受压面形心和压力中心。\\\hline
弯管、喷嘴、射流、叶轮、支座力 & C & 取流体CV，先假设支座力方向，列动量方程，最后按题问对象取反。\\\hline
无旋、势函数、流函数、源汇涡、镜像、圆柱 & D & 写$\psi=\sum\psi_i$或$\varphi=\sum\varphi_i$，再求速度和压力。\\\hline
马赫数、喷管、背压、阻塞、激波、膨胀角 & E & 标$p_0,T_0,A^*,A_e,p_b$和波前/波后；先判等熵段还是跨激波。\\\hline
Re、粘度、壁剪、边界层、阻力、粗糙度 & F & 先算$Re$；层流用解析式，湍流/粗糙管查Moody或经验式。\\\hline
证明、物理意义、量纲、应力张量、加速度 & G & 写定义+条件+主方程；应力先单位法向，量纲先列变量。\\\hline
\end{tabularx}

\mt{v94 反向求量链：题目问什么就沿链走}
\begin{tabularx}{\linewidth}{|p{.22\linewidth}|X|}\hline
\textbf{目标量} & \textbf{可执行链条}\\\hline
$Q$或$\dot m$ & 液体：$Q=AV\leftarrow$能量式求$V$；气体：先判阻塞，未阻塞用出口状态$\rho_eV_eA_e$，阻塞用$A^*,p_0,T_0$。\\\hline
某点$p$ & 静水：深度差；管路：能量式移项；喷管：等熵/激波关系；汽蚀必须用绝压。\\\hline
$h_L,\lambda$ & $h_L=(\lambda L/d+\sum\zeta)V^2/(2g)$；$\lambda$由$Re$和$\epsilon/d$，层流直接$64/Re$。\\\hline
力/反力 & 静水用压力积分；流动用CV动量；外绕用$C_D$；最后写“流体受力/物体受力”是否取反。\\\hline
速度分布 & 先判粘性层流解析题；写N-S简化$\to$积分$\to$边界条件$\to$速度/流量/剪应力。\\\hline
边界层厚度/阻力 & $Re_x$定位，层流$\delta\sim x/\sqrt{Re_x}$；混合流先分段，阻力用$C_f\rho U^2A/2$。\\\hline
Ma和$p,T,\rho$ & 等熵：一个Ma连出$p,T,\rho,V$；正激波：先$M_1$再$M_2$和压力比；斜激波先法向分量。\\\hline
角度 & 斜激波压缩：$\theta-\beta-M$；PM膨胀：$\delta=\nu(M_2)-\nu(M_1)$；角度均相对来流。\\\hline
\end{tabularx}
