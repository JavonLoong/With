...
\subq{子题八：一维定常正激波状态计算}
作业代表题：定常正激波。原题：空气（$\gamma=1.4$）以马赫数 $Ma_1 = 2.0$, 静压 $p_1 = 100$ kPa, 静温 $T_1 = 300$ K 流过正激波。求波后马赫数 $Ma_2$, 静压 $p_2$, 静温 $T_2$ 和总压损失 $\Delta p_0$。\\
公式链：
1. 波后马赫数：$Ma_2 = \sqrt{\frac{1 + 0.2Ma_1^2}{1.4Ma_1^2 - 0.2}} = \sqrt{\frac{1 + 0.2 \cdot 4}{1.4 \cdot 4 - 0.2}} = \sqrt{\frac{1.8}{5.4}} \approx 0.577$。
2. 静压比与静压：$\frac{p_2}{p_1} = \frac{7Ma_1^2 - 1}{6} = \frac{7 \cdot 4 - 1}{6} = 4.5$。则 $p_2 = 4.5 \times 100 = 450$ kPa。
3. 密度比与静温：$\frac{\rho_2}{\rho_1} = \frac{6Ma_1^2}{Ma_1^2 + 5} = \frac{6 \cdot 4}{4 + 5} = 2.667$。静温 $T_2 = T_1 \cdot \frac{p_2/p_1}{\rho_2/\rho_1} = 300 \cdot \frac{4.5}{2.667} = 506.25$ K。
4. 总压损失计算：波前总压 $p_{01} = p_1 (1 + 0.2Ma_1^2)^{3.5} = 100 \cdot (1.8)^{3.5} = 782.4$ kPa。
   波后总压 $p_{02} = p_{01} \cdot 0.72 = 563.3$ kPa。总压损失 $\Delta p_0 = p_{01} - p_{02} = 219.1$ kPa。

\subq{子题九：运动正激波与撞墙反射计算}
作业代表题：撞墙反射。原题：一运动正激波在静止空气（$p_1 = 100$ kPa, $T_1 = 300$ K）中以速度 $V_s = 680$ m/s 向前传播。撞击垂直墙壁后发生反射。求入射波后气流速度 $V_2$, 绝对压力 $p_2$ 及反射波后的压力 $p_3$。\\
公式链：
1. 声速及入射波前相对马赫数：空气声速 $a_1 = \sqrt{\gamma R T_1} = 347.2$ m/s。入射波马赫数 $Ma_1 = V_s / a_1 = 680 / 347.2 = 1.959$。
...
\subq{子题一：单道斜激波（斜楔压缩）流动计算}
作业代表题：斜楔激波。原题：超声速来流 $M_1 = 2.0$, 静压 $p_1 = 100$ kPa, 静温 $T_1 = 300$ K 绕倾角 $\theta = 10^\circ$ 的楔体流动。试求斜激波倾角 $\beta$, 波后马赫数 $M_2$ 及波后压力 $p_2$。\\
公式链：
1. 确定激波角：由 $M_1 = 2.0, \theta = 10^\circ$ 查曲线，得到弱斜激波解 $\beta \approx 39.3^\circ$。
2. 法向波前马赫数：$M_{n1} = M_1 \sin\beta = 2.0 \cdot \sin(39.3^\circ) = 1.267$。
3. 法向波后马赫数：$M_{n2} = \sqrt{\frac{1 + 0.2 M_{n1}^2}{1.4 M_{n1}^2 - 0.2}} = 0.804$。
4. 波后真实马赫数：$M_2 = \frac{M_{n2}}{\sin(\beta-\theta)} = \frac{0.804}{\sin(39.3^\circ - 10^\circ)} = 1.64$。
5. 波后压力：$p_2 = p_1 \cdot \frac{7 M_{n1}^2 - 1}{6} = 170.5$ kPa。

\subq{子题二：Prandtl-Meyer膨胀扇（凸角膨胀）流动计算}
作业代表题：凸角膨胀波。原题：超声速来流 $M_1 = 1.5$, 静压 $p_1 = 100$ kPa, 静温 $T_1 = 300$ K 绕折角 $\delta = 14.5^\circ$ 的凸角流动。试求波后马赫数 $M_2$, 压力 $p_2$ 及温度 $T_2$。\\
公式链：
...
\subq{子题三：超声速绕双凸极/接触面交汇计算}
作业代表题：多波交汇。原题：超声速流交汇，已知两侧折角引起的反射偏转或接触面交汇。\\
公式链：
1. 接触面（Slipstream）边界条件：两侧交汇静压相等 $p_{2a}=p_{2b}$，且流动方向平行 $\theta_a = \theta_b$。
2. 规律：利用两侧弱斜激波/PM膨胀关系，各段逐步推进，可得最终压力与流向。
