...
\infotxt{%
1. \key{连续介质与物理性质}：将流体视为由质点连续无间隙分布的介质。温度升高，\warn{液体粘度下降}，\warn{气体粘度上升}。不可压缩流体中声速为无穷大；马赫数 $Ma < 0.3$ 时可忽略压缩性。\\
2. \key{流线与迹线}：迹线是质点轨迹；流线是瞬时切线。在\warn{定常流动中流线与迹线完全重合}，非定常流动中通常不重合。随体加速度由当地加速度（$\partial/\partial t$）与迁移加速度组成。速度旋度是旋转角速度的\warn{两倍}。\\
3. \key{三大基本方程基本概念}：连续性方程源自质量守恒，\warn{粘性流体与理想流体的形式完全相同}。动能/动量修正系数因截面流速不均匀产生，层流修正系数大于紊流（圆管层流 $\alpha=2.0$，紊流 $\alpha\approx 1.05\sim 1.10$）。管嘴出流量大于孔口，因收缩断面处\warn{存在真空区起抽吸作用}。\\
4. \key{理想流体动力学与无旋流}：理想流体无粘性（$\mu=0$）。其控制方程 Euler 方程\warn{同样是非线性偏微分方程}。伯努利方程沿流线成立条件为定常、不可压、无粘理想流体且无摩擦。若全场无旋，则伯努利常数全场相等。
}
...
3. \key{混淆要点}：二维不可压有旋流动有流函数但无势函数；三维无旋流动有势函数但无二维流函数；有粘流动在局部无旋时也可以存在势函数。\\
4. \key{复势与正交性}：若流动既是二维不可压又是无旋的，势函数与流函数均满足 Laplace 方程，且等势线与等流函数线正交。\\
...
\infotxt{%
1. \key{边界层外界线}：由名义厚度 $\delta$ 标定的边界层粘性外界线\warn{不是流线}（因排挤效应沿程增厚，有外部流体跨越进入）。边界层积分求解\warn{必须同时使用}名义厚度 $\delta$、排挤厚度 $\delta^*$ 和动量损失厚度 $\theta$。\\
