...
{\fontspec{SimSun}①} 忽略惯性项，由线性 Stokes 方程控制，\warn{绝不能使用无粘 Euler 方程}。\\
{\fontspec{SimSun}②} 小球总阻力 $D = 3\pi \mu d U$，其中\warn{摩擦阻力占 2/3}，压差阻力占 1/3，无任何流动分离。\\
{\fontspec{SimSun}③} 极低雷诺数下，流线型物体因湿面积大导致摩擦阻力大，总阻力可能大于圆球。\\
3. \key{粘性流动解析解}：\\
{\fontspec{SimSun}①} Couette流（剪切驱动，无压梯度下速度呈线性分布）；Poiseuille流（压力驱动，速度呈抛物线分布）。\\
{\fontspec{SimSun}②} 两层粘性流体交界面上，\warn{剪切应力与速度必须连续}。\\
{\fontspec{SimSun}③} 圆管完全发展段中速度剖面恒定，\warn{沿程压力梯度 $\frac{dp}{dx}$ 为常数}。\\
{\fontspec{SimSun}④} 光滑管替换不一定显著增加总流量（若局部阻力占主导或处于水力光滑区，改变粗糙度无用）。\\
4. \key{当量直径与动力相似}：\\
