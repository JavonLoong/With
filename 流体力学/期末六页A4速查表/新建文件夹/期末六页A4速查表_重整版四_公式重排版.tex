...
4. \key{无粘流体动力学}：\\
{\fontspec{SimSun}①} 理想流体无粘性（$\mu=0$）。\\
{\fontspec{SimSun}②} N-S方程与无粘流体Euler方程\warn{均为非线性}偏微分方程（均含有对流加速度项$(\bm V\cdot\nabla)\bm V$）。\\
{\fontspec{SimSun}③} 伯努利方程沿流线成立条件为定常、不可压、无粘。若全场无旋，则伯努利常数全场相等。\\
{\fontspec{SimSun}④} 三维理想流体中因涡线拉伸弯曲，\warn{涡量沿流线通常不守恒}，仅二维理想不可压中守恒。\\
5. \key{水击、空化与虹吸}：\\
