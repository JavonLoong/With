...
\pvzfig
\conceptq{流体物理性质}表面张力：液滴内外压差$\Delta p=2\sigma/R$；泡$\Delta p=4\sigma/R$；液柱$\Delta p=\sigma/R$。毛细上升高度$h=\frac{2\sigma\cos\theta}{\rho g R}$（水与干净玻璃$\theta\approx 0$，水银$\theta\approx 140^\circ$）。
\idq{题干完整骨架}
...
先由连续/能量求$V,Q$，再取管内流体为CV：\fml{\sum\bm F=\sum_{\rm out}\rho Q\bm V-\sum_{\rm in}\rho Q\bm V}。入口压力推入控制体，出口对大气表压为0；求管壁/法兰受力时，最后对流体受力取反。
...
\columnbreak
\infoh{选择判断秒杀库-1.低雷诺数与粘性主导流动}
\infotxt{%
1. \key{流态与蠕动流}：极低雷诺数（$Re<1$）流动又称\warn{蠕动流}或\warn{斯托克斯流}，粘性力占绝对主导，惯性力小到可忽略。\\
2. \key{控制方程选用}：低Re绕流近似解中忽略了惯性项（质点动量变化），控制方程简化为线性Stokes方程，但\warn{绝不能使用理想无粘Euler方程}（粘性是主导，且流线分布截然不同）。\\
3. \key{斯托克斯阻力}：低Re小球绕流总阻力为$D=3\pi\mu dU$，阻力系数$C_D=24/Re$。其中，\warn{摩擦阻力占2/3}，压差阻力占1/3，流线具有前后完全对称性，无流动分离。\\
4. \key{流线型阻力对比}：在极低雷诺数下，由于摩擦阻力占绝对主导且无分离，流线型物体的总阻力\warn{可能大于圆球}（因流线型湿表面积大，导致摩擦阻力显著增加）。
}

\infoh{选择判断秒杀库-2.高雷诺数与惯性/边界层流动}
\infotxt{%
1. \key{非线性本质}：高雷诺数下即使忽略粘性，理想流体Euler方程由于含有对流项$(\bm V\cdot\nabla)\bm V$，\warn{同样是非线性偏微分方程}（N-S方程同理）。\\
2. \key{边界层外界线}：由名义厚度$\delta$标定的边界层粘性外界线\warn{不是流线}（因排挤效应沿程增厚，有外部流体跨越卷吸进入）。边界层积分求解\warn{必须同时使用}名义厚度$\delta$、排挤厚度$\delta^*$和动量损失厚度$\theta$。\\
3. \key{流动分离判据}：高Re绕流在后部减速段存在\warn{逆压梯度}（$dp/dx>0$）；壁面剪力降为零（$\tau_w=0$即$\partial u/\partial y|_w=0$）是分离的临界条件。分离后压差阻力剧增。\\
4. \key{阻力危机}：球/圆柱在临界$Re$附近边界层转捩为湍流，抗逆压梯度能力增强，分离点后移、尾迹变窄，使\warn{阻力系数$C_D$骤降}。\\
5. \key{湍流时均方程}：流动易失稳转捩为湍流，临界雷诺数非固定常数。不可压湍流时均动量方程（RANS）因脉动引起的\warn{雷诺应力项而未封闭}，须引入半经验湍流模型。
}

\infoh{选择判断秒杀库-3.物理性质与连续介质}
\infotxt{%
...
\infoh{选择判断秒杀库-4.流体静力学与等压面}
\infotxt{%
1. \key{静压强特性}：静止流体中一点的静压强\warn{大小与作用面方位无关}，方向垂直指向受压面（各向同性，只承受压应力）。\\
...
\infoh{选择判断秒杀库-13.期末真题判断与选择专练}
\infotxt{%
1. \key{边界层与流线}：由名义厚度$\delta$标定的边界层粘性外界线\warn{不是流线}（因排挤效应沿程增厚，有外部流体跨越进入）。\\
2. \key{极低Re流线型阻力}：在极低雷诺数下，流线型绕流总阻力\warn{可能大于圆球}（因摩擦阻力占主导且流线型湿表面积大）。\\
3. \key{Laval管与激波流量}：上游滞止状态相同时，管内有正激波与出口存在膨胀波时喉部均壅塞，通过的质量流量\warn{相同}。\\
4. \key{N-S与Euler非线性}：N-S方程与无粘流体Euler方程\warn{均为非线性}偏微分方程（均含有对流加速度项$(\bm V\cdot\nabla)\bm V$）。\\
5. \key{牛顿本构方向性}：牛顿流体本构中应力与变形速率张量为线性关系，因流体各向同性，该比例系数与\warn{应力作用方向无关}。\\
6. \key{动量积分方程求解}：Von Karman动量积分方程求解\warn{必须同时使用}名义厚度$\delta$、排挤厚度$\delta^*$和动量损失厚度$\theta$。\\
7. \key{参考状态与坐标系}：一维等熵气流的滞止参数等参考状态\warn{与坐标系选取有关}（因流速具相对性，不同运动框架下滞止量不同）。\\
8. \key{低Re圆柱与无粘}：低Re圆柱绕流粘性起主导作用，流线与无粘完全不同，\warn{不能使用无粘Euler方程}，须用Stokes方程。\\
9. \key{粘性流体流函数}：二维不可压流动满足连续方程时，粘性流体\warn{也有流函数$\psi$}（其存在性仅取决于质量守恒）。\\
10. \key{流线体逆压区}：高Re流线体绕流在后部收缩段\warn{仍会出现逆压区}，虽能延迟分离，但大攻角等工况下仍可能发生分离。\\
11. \key{正激波前后状态}：通过静止正激波后：静压、密度、温度上升；\warn{滞止温度、滞止焓、临界截面积$A^*$不变}；总压/滞止压强下降。\\
12. \key{连续性方程对比}：粘性流体与理想流体的连续性方程\warn{完全相同}（源于普适质量守恒，与本构方程/粘性无关）。\\
13. \key{低Re小球绕流近似}：Stokes小球绕流近似解中，因$Re \ll 1$而\warn{忽略了流体质点加速度}（惯性力），摩擦阻力占总阻力的 $2/3$。\\
14. \key{非定常流函数与势函数}：非定常不可压流场\warn{可能存在流函数}（二维即存在）；有流函数不一定有势函数（可能有旋）。\\
15. \key{层流湍流与时均方程}：临界转捩雷诺数非固定常数；不可压湍流时均动量方程因雷诺应力项而\warn{不封闭}，须引入湍流模型。
}
