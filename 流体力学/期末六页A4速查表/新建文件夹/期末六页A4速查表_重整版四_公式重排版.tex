...
\infotxt{%
1. \key{连续介质与物理性质}：将流体视为质点连续无间隙介质。温度升高，\warn{液体粘度下降}，\warn{气体粘度上升}。任何流体处于静止（或平衡）状态下\warn{均不能承受切应力}，一旦受剪切力必产生连续变形。重力场下\warn{等温大气的气压随高度呈指数级下降}（$p=p_0 e^{-gz/RT}$），而非线性。因各向同性，牛顿流体本构中应力与变形速率张量的比例系数（粘度）与\warn{应力作用方向无关}。水滴与水中气泡内部压强总是大于外部（$\Delta p = 2\sigma/R$），极细测压管读数须修正毛细高度（$h = \frac{4\sigma}{\rho g d}$）。牙膏、油漆、煤泥水低于屈服应力时不流动，属于宾汉塑性流体。不可压声速为无穷大；$Ma < 0.3$ 可忽略压缩性。\\
...
4. \key{圆柱绕流与升力}：理想流体无环量绕圆柱无阻力也无升力（达朗贝尔佯谬）；若有速度环量 $\Gamma$，则产生与来流垂直的升力 $L=\rho_\infty V_\infty \Gamma$（库塔-儒可夫斯基定理）。
}
...
2. \key{雷诺应力与方程不封闭}：不可压湍流时均动量方程（RANS）中，由于引入了脉动动量交换产生的\warn{雷诺应力项而导致方程不封闭}，必须引入半经验湍流模型。紊流圆管中总切应力沿过流断面呈\warn{线性分布}，管轴处为0，管壁处最大。\\
...
\infotxt{%
1. \key{边界层外界线与求解步骤}：由名义厚度 $\delta$ 标定的边界层外界线\warn{不是流线}（有外部流体跨越进入）。排挤厚度 $\delta^*$ 和动量损失厚度 $\theta$ \warn{均显著小于名义厚度 $\delta$}。积分求解必须同时使用这三个厚度。平板层流阻力求解：设速度分布 $\to$ 求 $\theta,\tau_w$ 与 $\delta$ 关系 $\to$ 代入冯卡门积分方程 $\to$ 积分求出 $\delta(x)$ 及摩擦阻力。\\
2. \key{流动分离与卡门涡街}：绕流钝体（如圆柱）两侧边界层因\warn{逆压梯度与粘性摩擦}脱流，剪切层不稳定，交替脱落形成排布整齐、旋转方向相反的涡列即卡门涡街。其释放频率 $f = St \frac{U}{d}$ 与流速 $U$ 成正比，与直径 $d$ 成反比，阻力危机前 $St \approx 0.2$。高Re流线体绕流在后部收缩段\warn{仍会出现逆压区}，虽能延迟分离，但在大攻角等工况下仍可能发生分离。\\
