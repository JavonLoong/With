...
\definecolor{infobg}{HTML}{D2ECD3}
\definecolor{subbg}{HTML}{B9D7FF}
...
\newcommand{\chap}[1]{\vspace{0.35pt}\colorbox{h1}{\begin{minipage}{\dimexpr\linewidth-2\fboxsep\relax}\fontsize{9.00}{9.30}\selectfont\bfseries\color{white}#1\end{minipage}}\par\vspace{0.15pt}}
...
\newcommand{\tagsecbg}[3]{\vspace{0.10pt}\colorbox{subbg}{\begin{minipage}{\dimexpr\linewidth-2\fboxsep\relax}\fontsize{5.05}{5.24}\selectfont\bfseries\color{#1}\colorbox{#1}{\color{white}\fontsize{4.00}{4.14}\selectfont #2}\,\color{black}#3\end{minipage}}\par}
\newcommand{\realq}[1]{\tagsecbg{realc}{真题}{#1}}
\newcommand{\subq}[1]{\tagsecbg{subc}{子题}{#1}}
\newcommand{\varq}[1]{\tagsecbg{varc}{变式}{#1}}
...
\useasboundingbox (-0.1,-0.35) rectangle (6.3,1.65);
...
\useasboundingbox (-0.3,-1.15) rectangle (6.2,0.9);
...
\useasboundingbox (-0.35,-0.9) rectangle (4.75,1.25);
...
\node[red, right] at (4.9,-0.1) {膨胀波 $\delta$};
\draw[->, blue] (3.3,0.3) -- (3.9,0.3) node[midway, above] {$Ma_1$};
\draw[->, blue] (4.4,-0.2) -- (5.0,-0.45) node[midway, above] {$Ma_2$};
\end{tikzpicture}\end{center}\vspace{-3pt}}
\newcommand{\schemeviscous}{\begin{center}\begin{tikzpicture}[>=Latex,scale=0.58,every node/.style={font=\fontsize{4.8}{5.8}\selectfont}]
...
\node[below] at (0.8, -0.15) {离心泵安装高度/汽蚀};
...
\node[below] at (4.6, -0.15) {Couette/Poiseuille流};
\end{tikzpicture}\end{center}\vspace{-3pt}}
\newcommand{\schemebl}{\begin{center}\begin{tikzpicture}[>=Latex,scale=0.58,every node/.style={font=\fontsize{4.8}{5.8}\selectfont}]
\useasboundingbox (-0.4,-1.05) rectangle (5.2,1.05);

% ==================== LEFT: FLAT PLATE BOUNDARY LAYER ====================
% Flat plate (solid wall with hatching)
\draw[thick] (0,0) -- (2.6,0);
\foreach \x in {0.0,0.2,...,2.6}
  \draw[ultra thin] (\x,0) -- (\x-0.05,-0.05);

% Free stream velocity arrow
\draw[->, blue, thick] (-0.3,0.4) -- (0.2,0.4) node[above, midway] {$U$};

% Boundary layer curve (red, using sqrt(x) to look laminar)
\draw[red, thick] plot[domain=0:2.4, samples=50] (\x, {0.42*sqrt(\x)});

% Velocity profile at x=0.8
\draw[dashed, gray, ultra thin] (0.8,0) -- (0.8,0.375); % 0.42*sqrt(0.8) approx 0.375
\draw[blue!80, thin] (0.8,0) to[out=40, in=-90] (0.95,0.375);
\foreach \y in {0.1,0.2,0.3}
  \draw[->, blue!50, ultra thin] (0.8,\y) -- ({0.8 + 0.15*sin(\y/0.375 * 90)}, \y);
\draw[->, blue!80, thin] (0.8,0.375) -- (0.95,0.375);

% Velocity profile at x=2.0
\draw[dashed, gray, ultra thin] (2.0,0) -- (2.0,0.594); % 0.42*sqrt(2) approx 0.594
\draw[blue!80, thin] (2.0,0) to[out=45, in=-90] (2.25,0.594);
\foreach \y in {0.15,0.3,0.45}
  \draw[->, blue!50, ultra thin] (2.0,\y) -- ({2.0 + 0.25*sin(\y/0.594 * 90)}, \y);
\draw[->, blue!80, thin] (2.0,0.594) -- (2.25,0.594);

% Boundary layer thickness marker at x=2.3
\draw[dashed, gray, ultra thin] (2.3, {0.42*sqrt(2.3)}) -- (2.55, {0.42*sqrt(2.3)});
\draw[<->, >=Stealth, thin, gray] (2.5,0) -- (2.5, 0.637) node[right, pos=0.5, black] {$\delta(x)$};

\node at (1.15,-0.25) {平板边界层};

% ==================== RIGHT: CYLINDER SEPARATION WAKE ====================
% Cylinder centered at (4.2, 0), Radius R=0.32
\draw[thick, fill=gray!25] (4.2,0) circle (0.32);

% Separation shear layers (red)
% Upper separation point at angle 100 deg: (4.2 + 0.32*cos(100), 0.32*sin(100)) = (4.2 - 0.056, 0.315) = (4.144, 0.315)
% Lower separation point: (4.144, -0.315)
\draw[red, thick] (3.3, 0.35) to[out=0, in=170] (4.144, 0.315) to[out=-10, in=175] (5.1, 0.48);
\draw[red, thick] (3.3, -0.35) to[out=0, in=190] (4.144, -0.315) to[out=10, in=185] (5.1, -0.48);

% Wake recirculating zone (clockwise and counter-clockwise vortex arrows)
\draw[->, gray!80, thin] (4.7, 0.1) arc (90:-250:0.08);
\draw[->, gray!80, thin] (4.7, -0.1) arc (-90:250:0.08);

% Separation point indicators
\fill[red] (4.144, 0.315) circle (0.035);
\fill[red] (4.144, -0.315) circle (0.035);
\node[red, font=\fontsize{3.8}{4.8}\selectfont, above] at (4.05, 0.32) {分离点};

\node at (4.2,-0.95) {圆柱分离尾流};

\end{tikzpicture}\end{center}\vspace{-3pt}}
\newcommand{\pvzfig}{\par\vspace{-4pt}\noindent\hfill\begin{tikzpicture}[>=Latex,scale=.50,every node/.style={font=\tiny},baseline=(current bounding box.center)]
...
\node at (1.25,.22){A细};\node at (3.25,.18){B粗};\node at (2.15,.82){突扩};\node at (.45,1.38){C};\node at (4.85,1.02){D};\end{tikzpicture}\hfill\null\par\vspace{-7pt}}
\begin{document}
...
\pvzfig
\conceptq{流体物理性质}表面张力：水滴和气泡内压大于外压，压差 $\Delta p=2\sigma/R$。毛细上升高度 $h=\frac{2\sigma\cos\theta}{\rho g R}$（水与干净玻璃 $\theta\approx 0$，水银毛细下降约 $140^\circ$），极细管测压须修正毛细上升以求真实静压。宾汉塑性流体（如牙膏、油漆、煤泥水）低于屈服应力时不流动，超过后呈线性流动。
\idq{题干完整骨架}
...
\subq{弯管/喷嘴/法兰受力：已知$p,Q,A,\alpha$，求支座或螺栓力}
先由连续/能量求$V,Q$，再取管内流体为CV：\fml{\sum\bm F=\sum_{\rm out}\rho Q\bm V-\sum_{\rm in}\rho Q\bm V}。高压水枪喷嘴流速增大，动量流出大于流入需受向前力，流体对喷嘴反作用推力向后，消防员须用力向后抱紧。反力 $F_{\text{nozzle}} = \rho Q(V_{\rm out} - V_{\rm in}) - (p_{\rm in}A_{\rm in} - p_{\rm out}A_{\rm out})$，求管受力时对流体受力取反。
...
列瞬时出流$Q=C_dA_o\sqrt{2gH}$，再接水池面积方程$A_t\,dH/dt=-Q$。若容器截面积恒定：积分$\int_{H_1}^{H_2}dH/\sqrt{H}=-C_dA_o\sqrt{2g}\,t/A_t$。堰流无上部约束，闸孔出流受上部刚性约束，以 $e/H \le 0.65$ 划分。
\subq{普通池底放水管/虹吸管：求是否汽蚀或最大高度}
先从上游自由液面到危险最高点列能量，求该点绝压$p_{\rm abs}$；再和汽化压$p_v$比较。虹吸动力为重力势能差，但跨越最高点靠大气压支撑，最高点相对高差理论上限为大气压水柱高（10.3m，防空化通常小于7-8m）。空化（气蚀）成因：流速极高使静压降至饱和蒸汽压以下产生汽泡，在压强高处溃灭时微射流会破坏壁面。
\subq{气球受风：已知浮力、风速、$C_D$和拉绳，求绳角或风阻}
画自由体图：竖直方向浮力$F_B=\rho_{\rm air}gV_{\rm ball}$向上，重力$W=(m_{\rm ball}+m_{\rm He})g$向下；水平方向风阻$D=C_D(0.5\rho_{\rm air}U^2)A_f$（$A_f=\pi d^2/4$为迎风面积）。绳张力$T$沿绳向下偏风向。平衡：
\fmlb{\begin{aligned}
T\cos\alpha&=F_B-W,\quad T\sin\alpha=D\\
\tan\alpha&=\frac{D}{F_B-W}=\frac{C_D(0.5\rho U^2)A_f}{(\rho_{\rm air}-\rho_{\rm He})gV-m_{\rm ball}g}
\end{aligned}}
绳角$\alpha$从竖直量起。检查：$\rho_{\rm He}\approx0.164$kg/m$^3$，$\rho_{\rm air}\approx1.225$kg/m$^3$；浮力体积是球体积$\pi d^3/6$。
\idq{母题1图像关键词}
大水面、油箱、水池、管径变化、弯头、阀门、入口/出口、泵、水轮机、压力表、U管、突然扩大/缩小、喷嘴、射流、法兰、支座。出现这些图，优先按“PVZ闭合$Q,p,H$，必要时接CV动量或查Moody”处理。
\subq{真题：离心泵吸水安装高度与防汽蚀计算}
镀锌钢管吸水管 $e=0.15$mm，$L=6$m，$D=100$mm，弯头 $\zeta_{\text{bend}}=0.29$，带滤网底阀 $\zeta_{\text{valve}}=2.5$，流量 $Q=50$m$^3$/h，水温下汽化压 $p_v=19.6$kPa，大气压 $p_a=101.3$kPa。要求入口绝压至少比汽化压大 $20$kPa，求最大安装高度 $h_{\max}$。\\
解：1. 统一单位：$Q = \frac{50}{3600} = 0.01389$m$^3$/s，速度 $V = \frac{4Q}{\pi D^2} = 1.768$m/s。
2. 算 $Re$ 及摩阻：$\nu \approx 1.0\times 10^{-6}$m$^2$/s，则 $Re = \frac{VD}{\nu} = \frac{1.768\times 0.1}{1.0\times 10^{-6}} = 1.768\times 10^5$。
相对粗糙度 $\varepsilon/D = \frac{0.15}{100} = 0.0015$。由 Haaland 式求摩阻系数 $\lambda$：
\fmlb{1/\sqrt{\lambda} \approx -1.8 \log_{10} [ (0.0015/3.7)^{1.11} + 6.9/Re ] \Rightarrow \lambda \approx 0.0228}
3. 算总损失（含沿程与局部）：
\fmlb{h_f = \lambda \frac{L}{D} \frac{V^2}{2g} = 0.218\text{ m},\quad
h_\zeta = (\zeta_{\text{bend}} + \zeta_{\text{valve}}) \frac{V^2}{2g} = 2.79 \times \frac{1.768^2}{19.62} = 0.444\text{ m}}
能量方程：$\frac{p_a}{\rho g} = h_{\max} + \frac{p_B}{\rho g} + \frac{V^2}{2g} + h_f + h_\zeta$。
约束为 $p_B \ge p_v + 20\text{ kPa} = 39.6$kPa。代入极值 $p_B = 39.6$kPa 及 $V^2/2g = 0.159$m：
\fmlb{h_{\max} = \frac{101.3 - 39.6}{9.81} - 0.159 - 0.218 - 0.444 = 5.47\text{ m}}
\subq{真题：水轮机管路与轴功率计算}
上游大水池接斜管 $D_1=20$cm，$L_1=30$m，随后接 $D_2=30$cm，$L_2=15$m 进入水轮机，排水端带压力表读数 $70$kPa，已知流量 $Q$ 下斜管速 $V_1=2.0$m/s，水面高差 $15$m，粗糙度 $e=0.25$mm，水 $\nu=1.3\times10^{-6}$m$^2$/s，水轮机效率 $\eta=0.85$，求水轮机输出轴功率。\\
解：1. 由连续性方程：$V_2 = V_1 (D_1/D_2)^2 = 2.0\times (2/3)^2 = 0.889$m/s。
$Q = \frac{\pi}{4} D_1^2 V_1 = 0.0628$m$^3$/s。
2. 算 $Re$ 与摩阻：$Re_1 = \frac{V_1 D_1}{\nu} = 3.08\times 10^5$，$\varepsilon/D_1 = 0.00125 \Rightarrow \lambda_1 \approx 0.0215$。
$Re_2 = \frac{V_2 D_2}{\nu} = 2.05\times 10^5$，$\varepsilon/D_2 = 0.00083 \Rightarrow \lambda_2 \approx 0.0202$。
3. 算沿程损失：
\fmlb{h_{f1} = \lambda_1 \frac{L_1}{D_1} \frac{V_1^2}{2g} = 0.658\text{ m},\quad
h_{f2} = \lambda_2 \frac{L_2}{D_2} \frac{V_2^2}{2g} = 0.041\text{ m} \Rightarrow h_f = 0.699\text{ m}}
4. 能量方程求水头 $H_T$：$H_T = z_1 - z_2 - \frac{p_2}{\rho g} - \frac{V_2^2}{2g} - h_f$。
代入 $z_1-z_2 = 15$m，$p_2=70$kPa，$V_2^2/2g = 0.040$m：
\fmlb{H_T = 15 - 7.136 - 0.040 - 0.699 = 7.125\text{ m}}
输出轴功率 $P = \eta \rho g Q H_T = 0.85 \times 1000 \times 9.81 \times 0.0628 \times 7.125 = 3.73\text{ kW}$。
\subq{真题：水平喷嘴法兰螺栓受拉力计算}
水平收缩喷嘴 $D_1=200$mm，$D_2=100$mm，进口绝压 $345$kPa，出口排向大气（$p_a=103.4$kPa），出口速度 $V_2=22$m/s。求法兰螺栓受力（忽略摩擦）。\\
解：1. 速度与流量：$V_1 = V_2 (D_2/D_1)^2 = 22 \times (1/2)^2 = 5.5$m/s。
流量 $Q = A_2 V_2 = \frac{\pi}{4} \times 0.1^2 \times 22 = 0.1728$m$^3$/s。
2. 控制体动量方程（取喷嘴内流体，设向右为 $+x$）：
\fmlb{R_x + p_{1g} A_1 - p_{2g} A_2 = \rho Q (V_2 - V_1)}
进口表压 $p_{1g} = 345 - 103.4 = 241.6$kPa，出口表压 $p_{2g}=0$。
\fmlb{R_x = 1000 \times 0.1728 \times (22 - 5.5) - 241600 \times \left(\frac{\pi}{4} \times 0.2^2\right) = 2851 - 7590 = -4739\text{ N}}
3. 螺栓受力：流体对喷嘴壁拉力为 $F_{\text{fluid}} = -R_x = 4739$N。故法兰螺栓受拉力为 $4739$N。
...
\idq{题图识别+查表+公式链}
\idq{题图一眼选模型}
\renewcommand{\arraystretch}{0.67}{\centering\begin{tabular}{|>{\centering\arraybackslash}p{0.30\linewidth}|>{\centering\arraybackslash}p{0.62\linewidth}|}
...
{\centering\begin{tabular}{|>{\centering\arraybackslash}p{0.42\linewidth}|>{\centering\arraybackslash}p{0.52\linewidth}|}\hline
...
\end{tabular}\par}\renewcommand{\arraystretch}{1}
\conceptq{三维基元势流}三维点源：$\phi=-\frac{Q}{4\pi r}$，速度$v_r=\frac{Q}{4\pi r^2}$。三维偶极子：$\phi=\frac{m\cos\theta}{4\pi r^2}$，速度$v_r=-\frac{m\cos\theta}{2\pi r^3},v_\theta=-\frac{m\sin\theta}{4\pi r^3}$。\\
...
原题：$T_0=500$K、$p_0=300$kPa的大容器接Laval喷管，$A_t=0.06{\rm m^2}$，定常等熵，求最大质量流量。写法：大容器$V\approx0$，故给定$p,T$就是$p_0,T_0$；最大流量$\Rightarrow$喉部阻塞$M_t=1,A_t=A^*$，\warn{不用液体Bernoulli/H--P}。
\fmlb{\begin{aligned}
...
\begin{center}\begin{tikzpicture}[>=Latex,scale=0.58,every node/.style={font=\fontsize{4.8}{5.8}\selectfont}]
% Nozzle outline
...
\draw[->,blue,thick] (-0.5,0) -- (0.0,0);

% U-tube Mercury Fill (drawn first so lines are on top)
\fill[gray!30] (1.75,-1.5) -- (1.95,-1.5) -- (1.95,-1.6) -- (3.95,-1.6) -- (3.95,-1.1) -- (4.15,-1.1) -- (4.15,-1.8) -- (1.75,-1.8) -- cycle;

% Mercury Surfaces
\draw[blue, thick] (1.75,-1.5) -- (1.95,-1.5);
\draw[blue, thick] (3.95,-1.1) -- (4.15,-1.1);

% U-tube Outline
\draw[thick] (1.75,-0.24) -- (1.75,-1.8);
\draw[thick] (1.95,-0.24) -- (1.95,-1.6);
\draw[thick] (3.95,-0.78) -- (3.95,-1.6);
\draw[thick] (4.15,-0.78) -- (4.15,-1.8);
\draw[thick] (1.75,-1.8) -- (4.15,-1.8);
\draw[thick] (1.95,-1.6) -- (3.95,-1.6);

% Height dimension
\draw[dashed, ultra thin] (1.95,-1.5) -- (2.95,-1.5);
\draw[dashed, ultra thin] (3.95,-1.1) -- (2.95,-1.1);
\draw[<->] (2.95,-1.5) -- (2.95,-1.1) node[midway,right] {$h=15$cm Hg};

% Labels
\node[left, align=right] at (-0.1,0.4) {$p_0=300$kPa\\$T_0=373$K};
...
{\centering\begin{tabular}{|>{\centering\arraybackslash}p{0.21\linewidth}|>{\centering\arraybackslash}p{0.70\linewidth}|}\hline
...
\renewcommand{\arraystretch}{0.76}{\centering\begin{tabular}{|>{\centering\arraybackslash}p{0.30\linewidth}|>{\centering\arraybackslash}p{0.61\linewidth}|}
...
\subq{可压缩喷管受力：已知$p_0,T_0,A_{\rm in},A_e,p_b$，求喷管壁对流体的力}
取喷管内流体为控制体。入口截面压力力$p_{\rm in}A_{\rm in}$（推入），出口$p_eA_e$（推出），壁面作用力$R_x$：
\fmlb{R_x+p_{\rm in}A_{\rm in}-p_eA_e=\dot m(V_e-V_{\rm in})}
若入口速度可忽略（$V_{\rm in}\approx0,p_{\rm in}\approx p_0$）：
\fmlb{R_x=\dot m V_e-(p_0A_{\rm in}-p_eA_e)}
\warn{所有压力用表压或同时用绝压}。出口若非设计态$p_e\ne p_b$，$p_e$仍由管内等熵/激波定。若出口外大气$p_a$作用在外壁，用表压最方便。
\subq{Laval向有限容器充气：已知$p_0,T_0,V_{\rm tank}$，求充气时间}
容器初压$p_i$、终压$p_f$，喉部面积$A^*$。喷管阻塞时$\dot m=\dot m_{\max}=$常数：
\fmlb{\dot m_{\max}=0.0404\frac{A^*p_0}{\sqrt{T_0}}\quad(\text{空气})}
容器等温充气质量守恒：$dm_{\rm tank}/dt=\dot m_{\max}$。若容器温度$T_{\rm tank}$恒定：
\fmlb{\frac{dp}{dt}=\frac{RT_{\rm tank}}{V_{\rm tank}}\dot m_{\max},\quad t=\frac{V_{\rm tank}(p_f-p_i)}{RT_{\rm tank}\dot m_{\max}}}
阻塞条件：只要$p_{\rm tank}/p_0<0.528$，$\dot m$恒定；当$p_{\rm tank}$升高到$p_0\times0.528$以上时不再阻塞，$\dot m$减小，需分段积分。若绝热模型则需数值迭代。
\stepq{喷管题骨架}
...
\lookq{可压缩题查表入口}
{\centering\begin{tabular}{|>{\centering\arraybackslash}p{0.29\linewidth}|>{\centering\arraybackslash}p{0.29\linewidth}|>{\centering\arraybackslash}p{0.34\linewidth}|}\hline
题眼 & 查书/表 & 查完怎么接\\\hline
...
\lookq{小查表：空气等熵/PM}
\renewcommand{\arraystretch}{0.66}{\centering\begin{tabular}{|c|c|c|c|c|}
...
\subq{真题：斜激波与弱激波角计算}
超声速空气流 $M_1=2.5$，遇到偏转角 $\theta=10^\circ$ 的楔形物。求弱斜激波的激波角 $\beta$、波后的马赫数 $M_2$ 和压强比 $p_2/p_1$。\\
解：1. 由 $\theta-\beta-M$ 方程或图表，在 $M_1=2.5$ 且 $\theta=10^\circ$ 时查得弱解激波角 $\beta \approx 31.8^\circ$。
2. 法向马赫数：$M_{n1} = M_1 \sin\beta = 2.5 \sin 31.8^\circ = 1.317$。
3. 过激波关系（用法向马赫数 $M_{n1} = 1.317$ 代入正激波公式）：
\fmlb{M_{n2} = \sqrt{\frac{1 + 0.2 M_{n1}^2}{1.4 M_{n1}^2 - 0.2}} = \sqrt{\frac{1.347}{2.228}} = 0.777}
\fmlb{\frac{p_2}{p_1} = 1 + \frac{2.8}{2.4}(M_{n1}^2 - 1) = 1 + 1.167\times (1.734 - 1) = 1.856}
4. 波后实际马赫数：
\fmlb{M_2 = \frac{M_{n2}}{\sin(\beta - \theta)} = \frac{0.777}{\sin 21.8^\circ} = \frac{0.777}{0.3714} = 2.09}
\formq{斜激波方程组}
...
对于空气（$\gamma=1.4$），常写为 $\nu(M) = \sqrt{6}\tan^{-1}\sqrt{(M^2-1)/6} - \tan^{-1}\sqrt{M^2-1}$。
\subq{真题：超声速气流 Prandtl-Meyer 凸角膨胀计算}
超声速空气流 $M_1=1.5$，遇到偏转角（凸角） $\delta = 12^\circ$。求膨胀后的马赫数 $M_2$ 及静压比 $p_2/p_1$。\\
解：1. 由 $M_1=1.5$ 算得 $\nu_1 = \nu(M_1)$：
\fmlb{\nu_1 = \sqrt{6}\tan^{-1}\sqrt{(1.5^2-1)/6} - \tan^{-1}\sqrt{1.5^2-1} = 0.2075\text{ rad} \approx 11.89^\circ}
2. 膨胀后的 PM 函数值：$\nu_2 = \nu_1 + \delta = 11.89^\circ + 12^\circ = 23.89^\circ$（即 $0.4170$ rad）。
3. 反查 PM 表或迭代求解：对于 $\nu_2 = 23.89^\circ$，算得波后马赫数 $M_2 \approx 1.91$。
4. 算静压比（由于膨胀波为等熵流，总压 $p_0$ 保持不变）：
\fmlb{\frac{p_2}{p_1} = \left( \frac{1+0.2M_1^2}{1+0.2M_2^2} \right)^{3.5} = \left( \frac{1+0.2\times 1.5^2}{1+0.2\times 1.91^2} \right)^{3.5} = (0.838)^{3.5} = 0.54}
\stepq{流程E：给激波图}
...
\subq{纹影图波角识别：已知图中线角度，求$M$或偏转角}
纹影图三类线：(1)\textbf{马赫线/波}：极弱扰动，角$\mu=\sin^{-1}(1/M)$。(2)\textbf{斜激波}：角$\beta>\mu$，由$M_1,\theta$关系决定，是激波面与来流方向的夹角。(3)\textbf{膨胀扇}：由首波（$\mu_1$）和尾波（$\mu_2$）张开的扇形区域。\\
\warn{图题读角步骤}：先从图中量出波与来流方向夹角$\alpha_{\rm img}$。若是马赫波，$M=1/\sin\alpha_{\rm img}$；若是斜激波，$\beta=\alpha_{\rm img}$，结合楔角$\theta$（壁面偏转角）查表得$M_1$。
\subq{菱形翼/双楔翼：已知攻角$\alpha$、半角$\varepsilon$、弦长$c$，求升力与阻力}
上下表面各经历不同偏转角：前缘上表面膨胀$\theta_1=\varepsilon-\alpha$（若$\varepsilon>\alpha$），下表面压缩$\theta_2=\varepsilon+\alpha$；用斜激波或PM求出四个面板的压力$p_1$（前上）、$p_2$（前下）、$p_3$（后上）、$p_4$（后下）。\\
积分力（弦长为$c$，前/后半弦长各为$c/2$）：
\fmlb{\begin{aligned}
\text{法向力 } N'&=(p_2+p_4-p_1-p_3)\frac{c}{2}\\
\text{轴向力 } A'&=(p_1+p_2-p_3-p_4)\frac{c}{2}\tan\varepsilon\\
\text{升阻力 } L'&=N'\cos\alpha-A'\sin\alpha,\quad D'=N'\sin\alpha+A'\cos\alpha
\end{aligned}}
注：若无摩擦阻力，此$D'$即为波阻力。对称零攻角时$L'=0, D'=2(p_{\rm front}-p_{\rm rear})\frac{c}{2}\tan\varepsilon$。
...
\mview{\key{母题总览}：看见黏度、充分发展、圆管、两平板、油膜、粗糙度，先判层/湍和边界条件。首写简化N--S或机械能损失。顺序：$Re\to$层流解析/H--P或Moody$\lambda\to h_L,\Delta p,\tau,Q$。检查：充分发展、层流才用H--P、速度剖面与平均速度别混。\warn{【别找错：平板边界层/位移/动量厚度/流动分离去第5页母题6红色区！】}}
\schemeviscous
...
\textbf{Couette与Poiseuille本质区分}：Couette为剪切驱动（运动壁带动，无压差时 $\tau=\text{const}$，速度线性）；Poiseuille为压力驱动（由压差 $dp/dx$ 驱动，壁静止，$\tau$ 沿截面线性分布，速度为抛物线，中心 $\tau=0$ 且速度最大）。广义Couette是二者的叠加。两层流体交界面速度与剪应力双连续。\\
...
\infotxt{%
层流圆管$u_{\max}=2\bar u$；平板Poiseuille$\bar u=2u_{\max}/3$；圆管壁剪$\tau_w=R\Delta p/(2L)=8\mu\bar u/d$。圆管流态：$Re<2300$层流，$2300\sim4000$过渡，$>4000$通常湍流。近壁分区：$y^+<5$粘性底层，$5<y^+<30$缓冲层，$y^+>30$对数/湍流区。数值模拟：DNS无近似计算量大仅低Re；LES解大涡/SGS模型近小涡（中）；RANS时均化/模型闭合（小最实用）。粗糙度反推：先由能量方程求$\lambda$，再由$Re,\lambda$反查$\varepsilon/d$。\\
流速测量：HWFA（热线，King1914，热散失）；LDA（激光多普勒，Yeh1964，多普勒频移，无接触）；PDPA（相位多普勒，1989，粒径与速度）；PIV（粒子图像，1993，全场测速）。
}
\infoh{粘性管流信息库D 易错检查}
\infotxt{%
\warn{【防错导航】若是平板、边界层、位移/动量厚度、流动分离，立刻前往第5页母题六（红色背景区）！}\\
...
\end{aligned}}
\stepq{管路题统一流程}
1 算$Q$得$V=4Q/(\pi d^2)$；2 算$Re=Vd/\nu$判层/湍；3 算$\lambda=64/Re$、Moody/公式；4 算$h_f$、弯头$\sum h_\zeta$；5 列能量求$p,H_p,H_T$。
\subq{真题：水池粗糙圆管放水与摩阻迭代计算}
水池接相对粗糙度 $\varepsilon/D=0.0012$ 的粗糙圆管向大气放水，水温 $20^\circ$C（$\nu=1.0\times10^{-6}$m$^2$/s），液面到管出口高 $H=2$m，管径 $D=50$mm，长 $L=10$m，入口 $\zeta_{\text{in}}=0.5$。求出口流量 $Q$。若换为 $20^\circ$C SAE 10W30 油（$\nu = 1.1\times 10^{-4}$m$^2$/s），求油流量并判定流态。\\
解：1. 对大水面（1）和管出口（2）列能量方程（其中 $1.0$ 为出口动能项）：
\fmlb{H = \left(1.5 + \lambda \frac{L}{D}\right) \frac{V^2}{2g} \Rightarrow 2 = (1.5 + 200\lambda) \frac{V^2}{2g}}
2. 迭代求解水流量：初猜 $\lambda = 0.02$，得 $V = \sqrt{\frac{2\times 9.81\times 2}{1.5 + 200\times 0.02}} = 2.67$m/s。
$Re = \frac{VD}{\nu} = 1.33\times 10^5$。由 Haaland 式求摩阻系数：
\fmlb{1/\sqrt{\lambda} \approx -1.8 \log_{10} [ (0.0012/3.7)^{1.11} + 6.9/Re ] \Rightarrow \lambda \approx 0.0218}
将新 $\lambda$ 代回：$V = \sqrt{\frac{39.24}{1.5 + 200\times 0.0218}} = 2.59$m/s，更新 $Re = 1.29\times 10^5 \Rightarrow \lambda \approx 0.0219$。
再代回得 $V = 2.58$m/s。收敛。流量 $Q = \frac{\pi}{4} D^2 V = 5.07 \times 10^{-3}$m$^3$/s $= 18.2$m$^3$/h。
3. 求解油流量：假设流态为层流，阻力系数 $\lambda = 64/Re = \frac{64\nu}{VD}$。代入能量方程：
\fmlb{H = 1.5 \frac{V^2}{2g} + \frac{64\nu L}{D^2} \frac{V}{2g} \Rightarrow 1.5 V^2 + \frac{64\times 1.1\times 10^{-4} \times 10}{0.05} V - 4g = 0}
化简为二次方程 $1.5 V^2 + 28.16 V - 39.24 = 0 \Rightarrow V = 1.30$m/s。
验证雷诺数：$Re = \frac{VD}{\nu} = \frac{1.30 \times 0.05}{1.1\times 10^{-4}} = 591 < 2300$。确实为层流流态。
流量 $Q_{\text{oil}} = \frac{\pi}{4} D^2 V = 2.55 \times 10^{-3}$m$^3$/s $= 9.19$m$^3$/h。
...
\lookq{粘性/阻力查表入口}
{\centering\begin{tabular}{|>{\centering\arraybackslash}p{0.31\linewidth}|>{\centering\arraybackslash}p{0.28\linewidth}|>{\centering\arraybackslash}p{0.33\linewidth}|}\hline
题眼 & 查书/图 & 查完怎么接\\\hline
...
\textbf{动量积分链}：给速度剖面$f(\eta)$，先算$\delta^*=\delta\int(1-f)d\eta$、$\theta=\delta\int f(1-f)d\eta$；再用$\tau_w/(\rho U^2)=d\theta/dx$。平板层流阻力求解：由剖面求 $\theta,\tau_w$ 与 $\delta$ 关系，代入卡门方程积分求出 $\delta(x)$，进而求摩擦力。\\
\textbf{外绕阻力链}：球/圆柱/钝体先算$Re$，查$C_D$，用迎风面积；平板摩擦用$C_f$和湿面积。\\
\textbf{相似链}：列变量$\to$选重复变量$\rho,V,L\to$写$\pi$数；重力主导保$Fr$，粘性主导保$Re$，高速气体保$Ma$。\\
\textbf{边界层存在}：高$Re$外流粘性只在近壁薄层内重要；壁面无滑移使速度从0过渡到外流速度。\\
\textbf{逆压梯度$\to$分离}：流向压力升高使近壁低能流体减速，壁面速度梯度降至0并反向，流线脱离壁面形成回流区。\\
\textbf{湍流阻力大}：脉动增强动量交换，等效剪应力增大；边界层更厚但抗分离能力更强。\\
\textbf{扩压器易分离}：亚声速减速升压，存在逆压梯度；扩张角过大时边界层不能克服逆压梯度，产生分离。\\
\textbf{位移厚度$\delta^*$}：边界层内速度亏损使外流被壁面向外排挤的等效距离。\\
\textbf{动量厚度$\theta$}：边界层造成的动量亏损厚度，动量积分方程把壁剪和$\theta$的增长联系起来。
}
\infoh{边界层/外绕信息库C 常见结论库}
\infotxt{%
名义厚度$\delta$是$u=0.99U$处；排挤厚度$\delta^*$和动量损失厚度$\theta$均显著小于 $\delta$。逆压梯度$dp/dx>0$使边界层增厚并可能分离，$\tau_w=0$是分离临界。卡门涡街脱落频率 $f = St \frac{U}{d}$。流线型小迎角常摩擦阻力主导；钝体分离后压差阻力主导。平板若未说明双面，先按题图/文字判断，不能自动乘2。
}
...
\infoh{位移/动量厚度/Von Karman}
\redinfotxt{%
\fmlbred{\begin{aligned}
...
\end{aligned}}
\subq{真题：圆球高雷诺数绕流阻力计算}
直径 $d=10$cm 的球体在 $20^\circ$C 水（$\rho=998$kg/m$^3$, $\mu=1.0\times 10^{-3}$Pa·s）中以 $U=0.5$m/s 速度运动。求其阻力（已知高 $Re$ 时球体阻力系数 $C_D \approx 0.44$）。\\
解：1. 算雷诺数判流态：$Re = \frac{\rho U d}{\mu} = \frac{998 \times 0.5 \times 0.1}{1.0\times 10^{-3}} = 4.99 \times 10^4$。
2. 查图：在 $10^3 < Re < 2\times 10^5$ 区间阻力系数恒定，查得 $C_D \approx 0.44$。
3. 算投影面积：$A = \frac{\pi}{4} d^2 = \frac{\pi}{4} \times (0.1)^2 = 7.85 \times 10^{-3}$m$^2$。
4. 阻力：$D = C_D \frac{1}{2}\rho U^2 A = 0.44 \times \frac{1}{2} \times 998 \times 0.5^2 \times 7.85 \times 10^{-3} = 0.431$N。
\subq{真题：压降量的 Buckingham $\Pi$ 定理无量纲分析}
已知圆管压降 $\Delta p$ 与管径 $d$、管长 $l$、壁面粗糙度 $\varepsilon$、流速 $U$、流体密度 $\rho$、动力粘度 $\mu$ 有关，试用 Buckingham $\Pi$ 定理导出无量纲关系式。\\
解：1. 变量数 $n=7$，基本量纲 $M, L, T$（数 $r=3$）。
2. 量纲式：$[\Delta p]=ML^{-1}T^{-2}$, $[d]=L$, $[l]=L$, $[\varepsilon]=L$, $[U]=LT^{-1}$, $[\rho]=ML^{-3}$, $[\mu]=ML^{-1}T^{-1}$。
3. 重复变量：选 $\rho, U, d$。量纲式独立。
4. $\Pi$ 组数：$n-r = 4$ 个无量纲组。
- $\Pi_1 = \Delta p \, \rho^a U^b d^c \Rightarrow ML^{-1}T^{-2}(ML^{-3})^a(LT^{-1})^b L^c = M^0 L^0 T^0 \Rightarrow a=-1,b=-2,c=0 \Rightarrow \Pi_1 = \frac{\Delta p}{\rho U^2}$
- $\Pi_2 = l/d$，$\Pi_3 = \varepsilon/d$（量纲相同直接相比）
- $\Pi_4 = \mu \, \rho^a U^b d^c \Rightarrow ML^{-1}T^{-1}(ML^{-3})^a(LT^{-1})^b L^c = M^0 L^0 T^0 \Rightarrow a=-1,b=-1,c=-1 \Rightarrow \Pi_4 = \frac{\mu}{\rho U d} = \frac{1}{Re}$
5. 最终无量纲关系（取 $\Pi_4$ 倒数）：$\frac{\Delta p}{\rho U^2} = \Phi\left(\frac{l}{d}, \frac{\varepsilon}{d}, Re\right)$。
\stepq{平板题统一流程}
1 算$Re_L$判全层流/转捩/湍流。2 选$\bar C_f,\delta(L)$。3 阻力面积分清单面$Lb$、双面$2Lb$、湿表面积。4 给速度剖面先积分$\delta^*,\theta$，再用Karman动量积分。

\redinfotxt{%
\stepq{分离与阻力}
\warn{必要条件：逆压梯度$dp/dx>0$（或速度减小）；充分条件（分离判据）：壁面剪力$\tau_w=0$（即$\partial u/\partial y|_w=0$）。}摩擦阻力来自$\tau_w$；压差阻力来分尾流。
}
\idq{外绕流阻力判断}
...
\subq{边界层位移与动量厚度：已知速度剖面，求$\delta^*$和$\theta$}
\fmlb{\begin{array}{c|c|c}
u/U & \delta^* & \theta\\ \hline
\eta & \delta/2 & \delta/6\\
2\eta-\eta^2 & \delta/3 & 2\delta/15\\
\eta^{1/7} & \delta/8 & 7\delta/72
\end{array}}
...
{\centering\begin{tabular}{|>{\centering\arraybackslash}p{0.27\linewidth}|>{\centering\arraybackslash}p{0.66\linewidth}|}\hline
...
\subq{常用几何：静水压和量纲题会用}
圆$A=\pi d^2/4,\ I_C=\pi d^4/64$；矩形$A=bh,\ I_C=bh^3/12$；三角形$A=bh/2,\ I_C=bh^3/36$；半圆$A=\pi R^2/2,\ y_C=4R/(3\pi)$。量纲法：变量$n$个，基本量纲$r$个，$\pi$群$n-r$个；常选 $\rho,V,L$。模型试验中通常无法在普通风洞/水槽中同时满足 $Re$ 和 $Fr$ 相似（因两者的速度比例要求冲突）。
...
\columnbreak
\infoh{选择判断秒杀库-1.前半学期期中综合秒杀库（第1-4章）}
\infotxt{%
1. \key{基本性质与压力定义}：\\
{\fontspec{SimSun}①} 任何流体处于静止或平衡状态下\warn{都不能承受切应力}，一旦受剪切力必产生连续变形。\\
{\fontspec{SimSun}②} 液体的粘度随温度升高而\warn{降低}，气体的粘度随温度升高而\warn{增加}。\\
{\fontspec{SimSun}③} \warn{理想流体}是为方便而人为引入的\warn{理想模型}（真实流体都有粘），动力粘度/速度变化率极小时剪应力可忽略。\\
{\fontspec{SimSun}④} 理想流体压力是\warn{唯一的表面力}，大小 with 与方向无关，垂直于作用面并指向内法线方向。\\
{\fontspec{SimSun}⑤} 粘性流体压力是\warn{三个主应力的算术平均值的负值}（即 $p = -(\sigma_x+\sigma_y+\sigma_z)/3$），忽略流体粘性时粘性流体的压力就蜕化为理想流体的压力。\\
{\fontspec{SimSun}⑥} 水滴与水中气泡内部压强总是大于外部（$\Delta p = 2\sigma/R$），极细测压管读数（液面高度）不直接是真实静压，须修正毛细上升（$h = \frac{4\sigma}{\rho g d}$）。\\
{\fontspec{SimSun}⑦} 牙膏、油漆、煤泥水低于屈服应力时不流动，属于宾汉塑性流体。\\
2. \key{流场描述与运动学}：\\
{\fontspec{SimSun}①} \key{定常流动定义}：流场中任意空间点上的物理量不随时间变化，即时变偏导数 $\partial/\partial t = 0$。在定常流动中\warn{流线、迹线、脉线完全重合}。\\
{\fontspec{SimSun}②} 定常与非定常流动\warn{与坐标系选取有关}（例如绕物匀速流动在静止坐标系中是非定常，但若将坐标系固定在物体上则周围流场为定常流动）。\\
{\fontspec{SimSun}③} 在非定常流场中，从空间固定点连续释放染料所形成的线是\warn{脉线}（而非流线或迹线）。\\
{\fontspec{SimSun}④} \key{流体线}：由流体中同一时刻的一线流体质点所构成的线（染色质点随时间增长变形形成的抛物线代表流体线，而非流线或速度分布）。\\
{\fontspec{SimSun}⑤} 在定常流动中时变加速度为零，但迁移（位变）加速度 $(\bm V\cdot\nabla)\bm V$ 不一定为零，故质点总加速度 $\bm a = \frac{d\bm V}{dt} = \frac{\partial \bm V}{\partial t} + (\bm V\cdot\nabla)\bm V$ 不恒为零。\\
{\fontspec{SimSun}⑥} 刚体旋转虽然存在速度梯度 $\nabla\bm V \neq 0$，但相邻质点间无相对运动，故变形速率（应变率）为 0，不产生粘性切应力（自转角速度为涡量的一半，即 $\bm\omega = \frac{1}{2}\nabla\times\bm V$）。
}
\infotxt{%
3. \key{三大基本方程基本概念}：\\
{\fontspec{SimSun}①} 连续性方程 $\nabla\cdot\bm V=0$ \warn{适合不可压缩流体的任何流动}（有粘/无粘、定常/非定常均成立）。\\
{\fontspec{SimSun}②} 动量方程中的外力包含粘性切应力，\warn{既适用于理想流体，也适用于粘性流体}。\\
{\fontspec{SimSun}③} 等压面必垂直于质量力的合力方向。静止流体等压面是水平面不代表只受重力，若容器有垂直方向的匀加速度，合外力（重力与惯性力的合力）仍垂直向下，此时\warn{等压面依然是水平面}。\\
{\fontspec{SimSun}④} \key{渐变流}（流线近乎平行直线的流动）中流线弯曲极小，法向加速度可忽略，因此在垂直于流线的过流断面上，静压分布与静力学规律相同（即满足 $z+p/\rho g = \text{const}$）。\\
{\fontspec{SimSun}⑤} \key{管嘴出流量大于孔口}：水流进入短管嘴先收缩后扩大，在收缩断面产生局部真空（负压吸力），这个“吸力”增大了流速，使流量系数（流速）增大。\\
{\fontspec{SimSun}⑥} 以闸门开度与上游水头的比值是否满足 $e/H \le 0.65$ 来划分堰流（$e/H > 0.65$）与闸孔出流（$e/H \le 0.65$）。\\
4. \key{无粘流体动力学}：\\
{\fontspec{SimSun}①} 理想流体无粘性，即动力粘度 $\mu=0$（单位 $\text{Pa}\cdot\text{s}$ 或 $\text{kg/(m}\cdot\text{s)}$），运动粘度 $\nu=\mu/\rho=0$（单位 $\text{m}^2/\text{s}$）。\\
{\fontspec{SimSun}②} N-S方程与无粘流体Euler方程\warn{均为非线性}偏微分方程（均含有对流/迁移加速度项$(\bm V\cdot\nabla)\bm V$）。\\
{\fontspec{SimSun}③} 伯努利方程（$z + \frac{p}{\rho g} + \frac{V^2}{2g} = C$ 中的 $C$ 即伯努利常数，代表单位重量流体的总机械能）沿流线成立条件为：定常、不可压、无粘、仅受重力。若全场无旋，则该常数 $C$ 全场相等（即各条流线上常数相同）。\\
{\fontspec{SimSun}④} 三维理想流体中因涡线拉伸弯曲，\warn{涡量沿流线通常不守恒}，仅二维理想不可压流中守恒。\\
5. \key{水击、空化与虹吸}：\\
{\fontspec{SimSun}①} 水击是流速突变由\warn{流体可压缩性与管壁弹性}引起的高频压力波，防护包括延长关阀时间、加装安全阀/调压塔。\\
{\fontspec{SimSun}②} 空化是局部流速高导致静压降至\warn{饱和蒸汽压以下}产生汽泡，溃灭破坏金属表面。\\
{\fontspec{SimSun}③} 虹吸管动力是重力差，但靠大气压支撑越过高点，理论最大高度约 10.3 米（防空化通常小于 7-8 米）。\\
6. \key{潜水艇稳定性与附加质量}：\\
{\fontspec{SimSun}①} 潜水艇完全潜没重心低于浮心即绝对稳定；漂浮物轮船重心可高于浮心，但定倾中心必须高于重心。\\
{\fontspec{SimSun}②} 非定常流中存在\warn{附加质量}，固体加速运动须带动周围流体而表现为等效质量增加（启动/骤闭时显著）。
}

\infoh{选择判断秒杀库-2. (第5章) 理想不可压缩流体的二维无旋和有旋流动}
\infotxt{%
1. \key{流函数存在条件}：\\
{\fontspec{SimSun}①} 仅限\warn{二维}（平面或轴对称）且\warn{不可压缩}流动。三维流动由于速度分量有三个，单标量流函数无法同时满足连续性方程，故不存在单个流函数。\\
{\fontspec{SimSun}②} 源于连续性方程（质量守恒），只要满足上述条件，粘性流动\warn{同样存在流函数 $\psi$}。\\
{\fontspec{SimSun}③} 流函数存在与否与有无粘性、有旋无旋、定常/非定常均无关。\\
2. \key{势函数存在条件}：\\
{\fontspec{SimSun}①} 仅当流动\warn{无旋}（旋度 $\nabla\times\bm V=0$）时存在势函数 $\phi$。\\
{\fontspec{SimSun}②} 源于无旋的数学定义，其存在性\warn{与维数、压缩性、粘性、定常/非定常均无关}。
}
\infotxt{%
3. \key{混淆要点对比表}：\\
...
\end{tabular} \\
{\fontspec{SimSun}①} 二维无旋流动中，\warn{流线（等 $\psi$ 线）与等势线（等 $\phi$ 线）总是正交的}。\\
4. \key{圆柱绕流与升力}：\\
{\fontspec{SimSun}①} 有速度环量 $\Gamma$ 产生升力 $L=\rho V \Gamma$（库塔-儒可夫斯基定理）。\\
{\fontspec{SimSun}②} \key{K-J假设（后缘条件）}：在理想流体绕流机翼/平板时，后缘流速绝对值为无穷大，与实际不符。库塔-儒克夫斯基提出：一定存在恰当的环量 $\Gamma$，使背面的驻点移至后缘，保证后缘流速有限。\\
{\fontspec{SimSun}③} \key{数学表述}：非零倾角后缘处为驻点，即 $V_{\text{后缘}} = 0$；在尖锐（零倾角）后缘处上下表面流速相等且有限，即 $V_{\text{上}} = V_{\text{下}} < \infty$（从而确定唯一环量 $\Gamma$，如平板绕流 $\Gamma = \pi c V_\infty \sin\alpha$）。
}

\infoh{选择判断秒杀库-3. (第7章) 气体动力学基础}
\infotxt{%
1. \key{声速与马赫数}：\\
{\fontspec{SimSun}①} 声速 $a=\sqrt{\gamma RT}$ 仅是温度的函数。\\
{\fontspec{SimSun}②} 马赫数 $Ma=V/a$ 反映可压缩程度，当 $Ma < 0.3$ 时可近似为不可压缩流动。\\
{\fontspec{SimSun}③} 微弱扰动的声波传播是\warn{等熵过程}（极快、变化微小，来不及热交换的无摩擦绝热过程）；而强扰动的激波（如正激波）过程是\warn{非等熵过程}，激波后熵增加、滞止压力下降（即 $s_2 > s_1, p_{02} < p_{01}$）。
}
\infotxt{%
2. \key{气体动力流动与波}：\\
{\fontspec{SimSun}①} 微弱扰动线在超声速流（$Ma>1$）中形成的线为\warn{马赫线}（其半顶角为马赫角 $\alpha = \arcsin(1/Ma)$）。\\
{\fontspec{SimSun}②} 扰动只能传向马赫锥内部，外部为未扰动区。亚声速中扰动传向全场。\\
{\fontspec{SimSun}③} 激波后压力、温度、密度骤升，\warn{流速由超声速骤降为亚声速}。激波仅在超声速流动中可能产生。\\
3. \key{喷管核心结论}：\\
{\fontspec{SimSun}①} \key{加速原理}：由面速关系 $\frac{dA}{A} = (Ma^2-1)\frac{dV}{V}$ 决定。亚声速（$Ma<1$）在渐缩段加速；若想进一步加速至超声速（$Ma>1$），必须先在喉部达到声速 $Ma=1$（临界），再在渐扩段继续加速。\\
{\fontspec{SimSun}②} \key{临界流量与设计}：空气最大质量流量公式 $\dot{m}_{\text{max}} \approx 0.0404 \frac{p_0 A^*}{\sqrt{T_0}}$（$R=287$），唯一由喉部面积 $A^*$ 和上游滞止状态（$p_0, T_0$）决定。设计背压 $p_{b3} = \frac{p_0}{(1+0.2 Ma_e^2)^{3.5}}$；出口截面存在正激波时背压为 $p_{b2} = p_{b3} \left[ 1 + \frac{2\gamma}{\gamma+1}(Ma^2 - 1) \right]$。
}

\infoh{选择判断秒杀库-4. (第8章) 粘性流体力学基础}
\infotxt{%
1. \key{雷诺数物理本质}：\\
{\fontspec{SimSun}①} 雷诺数 $Re = \rho VL / \mu$ 是惯性力与粘性力之比。\\
{\fontspec{SimSun}②} \warn{惯性力尺度}：随体/对流加速度项引起的惯性力 $\rho (\bm V\cdot\nabla)\bm V \sim \rho V^2 / L$。\\
{\fontspec{SimSun}③} \warn{粘性力尺度}：由于剪切变形引起的粘性摩擦力 $\mu \nabla^2\bm V \sim \mu V / L^2$。\\
2. \key{低雷诺数流动（$Re < 1$ 斯托克斯流）}：\\
{\fontspec{SimSun}①} 忽略惯性项，由线性 Stokes 方程控制（即 $-\nabla p + \mu \nabla^2 \bm V + \bm f = 0$）；\warn{绝不能使用无粘 Euler 方程}（其形式为 $\rho \frac{d\bm V}{dt} = -\nabla p + \bm f$，忽略了粘性项）。\\
{\fontspec{SimSun}②} 小球总阻力 $D = 3\pi \mu d U$（斯托克斯阻力公式），其中\warn{摩擦阻力占 2/3} ($2\pi\mu d U$)，压差阻力占 1/3 ($\pi\mu d U$)，无任何流动分离。\\
{\fontspec{SimSun}③} 极低雷诺数下，流线型物体因湿面积大导致摩擦阻力大，总阻力可能大于圆球。\\
3. \key{粘性流动解析解}：\\
{\fontspec{SimSun}①} Couette流（剪切驱动：靠板移动带动，无压梯度，速度呈线性）；Poiseuille流（压力驱动：靠压强差推动，板静止，速度呈抛物线）。\\
{\fontspec{SimSun}②} 两层粘性流体交界面上，流速连续（$u_1=u_2$，无滑移）且剪应力连续（$\tau_1=\tau_2 = \mu_1 \frac{\partial u_1}{\partial y} = \mu_2 \frac{\partial u_2}{\partial y}$，应力平衡）。\\
{\fontspec{SimSun}③} 圆管完全发展段中速度剖面恒定，\warn{沿程压力梯度 $\frac{dp}{dx}$ 为常数}。\\
{\fontspec{SimSun}④} \key{倾斜板间流动}：重力与滑动壁面（以速度 $U_0$ 滑动）混合驱动，沿斜面向下流动，N-S方程为 $\mu \frac{d^2u}{dy^2} + \rho g \sin\theta = 0$，其零剪切力点 $\tau = 0$ 必落在下半区域 $y \in (0, H/2)$；\\
{\fontspec{SimSun}⑤} \key{光滑管替换与阻力控制}：总阻力损失等于沿程阻力 $h_f = \lambda \frac{l}{d}\frac{v^2}{2g}$ 与局部阻力 $h_j = \zeta \frac{v^2}{2g}$ 之和。若阀门、弯头等局部阻力占主导（$h_j \gg h_f$），或流动本身处于水力光滑区（摩擦系数 $\lambda$ 仅与 $Re$ 有关，与管壁粗糙度无关），则更换光滑管对减小阻力、增加流量几乎无用。
}
\infotxt{%
4. \key{当量直径与动力相似}：\\
{\fontspec{SimSun}①} \key{当量直径} $d_e = 4 R_h = 4 A / \chi$（其中 $R_h = A / \chi$ 是水力半径，$A$ 为过流断面积，希腊字母 $\chi$ 为湿周 [即流体与壁面接触的周长]），作用是将非圆截面折算为圆管进行阻力计算。\\
{\fontspec{SimSun}②} \key{相似准则}：重力起主导作用时选取无量纲数弗劳德数相似（$Fr = V/\sqrt{gl}$，其物理意义为惯性力与重力之比）；粘性力起主导作用时选取无量纲数雷诺数相似（$Re = \rho V l/\mu$，惯性力与粘性力之比）。两者均为无量纲数（无单位）。\\
{\fontspec{SimSun}③} 模型试验中通常无法在普通风洞/水槽中\warn{同时满足雷诺数 $Re$ 与弗劳德数 $Fr$ 相似}（因两者的速度缩放比例要求冲突）。\\
{\fontspec{SimSun}④} 量纲分析中只要选取的重复变量在量纲上相互独立即可（如可取 $\rho,V,L$ 或 $p,V,L$）。\\
{\fontspec{SimSun}⑤} 阻力平方区自模化时阻力系数与 Re 无关。\\
{\fontspec{SimSun}⑥} 欧拉数 $Eu = \Delta p / (\rho V^2)$ 是压力与惯性力之比，$Eu \gg 1$ 代表压力起主导作用。
}

\infoh{选择判断秒杀库-5. (第9章) 湍流}
\infotxt{%
1. \key{转捩与稳定性}：\\
{\fontspec{SimSun}①} 流态由层流向湍流转捩的临界雷诺数受壁面粗糙度、外界扰动、噪声等影响，非确定常数。管内流动存在下临界雷诺数 $Re_{cr} \approx 2300$，低于此值时任何湍流脉动都会衰减并恢复为层流（流动绝对稳定）；上临界雷诺数受实验扰动控制极不确定。\\
2. \key{雷诺应力与混合长度}：\\
...
\infoh{选择判断秒杀库-6. (第10章) 边界层理论基础}
\infotxt{%
1. \key{边界层外界线与摩擦阻力}：\\
{\fontspec{SimSun}①} 边界层名义外界线（$u/U = 0.99$ 处）不是流线，因为外部流体不断卷入边界层内。排挤厚度 $\delta^* = \int_0^\delta (1 - \frac{u}{U}) dy$ 表示因粘性减速导致外部势流被向外排挤的位移；动量损失厚度 $\theta = \int_0^\delta \frac{u}{U}(1 - \frac{u}{U}) dy$ 表示动量流损失的等效厚度。对于平板层流， $\delta^* \approx 0.34\delta$，$\theta \approx 0.13\delta$，均显著小于名义厚度 $\delta$。\\
