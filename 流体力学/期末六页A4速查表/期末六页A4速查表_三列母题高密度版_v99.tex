...
\setlength{\fboxsep}{0.55pt}
\renewcommand{\baselinestretch}{0.63}
...
\newcommand{\FB}[1]{\par\noindent\colorbox{bg}{\begin{minipage}{\dimexpr\linewidth-2\fboxsep\relax}\centering\fontsize{4.72}{4.92}\selectfont\ensuremath{\displaystyle #1}\end{minipage}}\par}
\newcommand{\W}[1]{\colorbox{warnbg}{\fitmath{#1}}}
\newcommand{\mt}[1]{\vspace{0.32pt}{\fontsize{5.72}{5.95}\selectfont\bfseries\color{white}\colorbox{h}{\strut\ #1\ }}\par\vspace{0.15pt}}
\newcommand{\s}[1]{{\fontsize{4.92}{5.12}\selectfont\bfseries\color{b}$\blacktriangleright$ #1}\par}
...
\newcommand{\chain}[1]{\par\noindent\colorbox{bg}{\begin{minipage}{\dimexpr\linewidth-2\fboxsep\relax}\fontsize{4.55}{4.75}\selectfont #1\end{minipage}}\par}
\newcommand{\warnline}[1]{\par\noindent\colorbox{warnbg}{\begin{minipage}{\dimexpr\linewidth-2\fboxsep\relax}\fontsize{4.55}{4.75}\selectfont\bad{反制}：#1\end{minipage}}\par}
...
\begin{document}
\fontsize{4.68}{4.88}\selectfont

% ================= PAGE 1 =================
\begin{multicols}{3}
\mt{v99 三列母题主轴：先入口，再模板，再公式}
\chain{\textbf{0 先看图/词，不按章节翻。}\quad
\textbf{1 选入口A--G。}\quad
\textbf{2 抄对应第一行。}\quad
\textbf{3 按未知量链推进。}\quad
\textbf{4 最后写单位、方向、表压/绝压、查表入口。}}
\begin{tabularx}{\linewidth}{|p{.08\linewidth}|p{.24\linewidth}|p{.29\linewidth}|X|}\hline
\textbf{入口} & \textbf{题干长相} & \textbf{第一行} & \textbf{优先翻页}\\\hline
A & 水池/管/泵/阀/压力表 & 取1--2--3列修正Bernoulli & P1母1；P4入口；P6 A\\\hline
B & 静水/U管/闸门/浮体 & 写$p=p_0+\rho gh$ & P1母2；P4入口；P6 B\\\hline
C & 弯管/喷流/叶片/支座 & 取流体CV列$\sum F=\dot m\Delta V$ & P1母3；P6 C\\\hline
D & 源汇涡/镜像/圆柱/环量 & 写$\psi,\varphi$线性叠加 & P1母4/5；P4 D\\\hline
E & $Ma,p_0,T_0,A^*,p_b$/激波 & 标$0,* ,e,b$或波前1波后2 & P1母6；P2母7--9；P6 E\\\hline
F & 粘度/Re/壁剪/边界层/阻力 & 先算$Re$再选解析/查表 & P2母10--13；P6 F\\\hline
G & 速度场/应力/量纲/证明 & 写对象+假设+主方程 & P2/3母14--17；P6 G\\\hline
\end{tabularx}
\s{统一变量与第一行}
截面用$1,2,3$；CV外法向$\bm n$指出控制体；$x$水平、$y$向上；液体能量题多用\tagg{表压}，气体/汽蚀/激波用\tagg{绝压}。\quad
\F{Q=AV,\ \dot m=\rho Q,\ Re=VL/\nu}\quad \F{Ma=V/\sqrt{\gamma RT}}
\chain{固定首句：“取\_\_为研究对象，设1为\_\_、2为\_\_，取\_\_为正，在\_\_假设下列\_\_方程。”}
\s{方向反制}
压力力压入受压面；CV右边是流出动量-流入动量；题问“管对流体”和“流体对管”相反；水头式单位m，压强式单位Pa。
...
\mt{v95 版面使用原则}
\card{为什么不保留60条弱化动作}{考试时你没有时间在几十条相似小卡里扫描。v95 保留了源码备份，但打印版只显示 A-G 大入口和目标量索引；先定位，再去对应母题块细算。}
\mt{v95 满分过程句库：写不出来就抄骨架}
...
\mt{v95 实战定位示例：把题干翻译成步骤}
\begin{tabularx}{\linewidth}{|p{.28\linewidth}|p{.08\linewidth}|X|}\hline
\textbf{题干片段} & \textbf{入口} & \textbf{落笔顺序}\\\hline
“水泵/水轮机、粗糙管、弯头、压力表” & A & 1画截面；2由$Q$算各段$V,Re$；3查$\lambda$；4能量式求$H_p/H_T/p$；5功率看效率。\\\hline
“离心泵吸水、汽化压、最大安装高度” & A & 1水面到泵入口；2吸入段损失；3令$p_{in}\ge p_v+\Delta p$；4解$h_{max}$，全用绝压。\\\hline
“U形管、差压计、闸门铰链、曲面挡水” & B & 1静水压强；2平面求$F,h_p$或曲面拆$F_H,F_V$；3取矩/合成；4方向写水对壁。\\\hline
“弯管/喷嘴/射流撞板要求支座力” & C & 1取流体CV；2压力力压入；3列$x,y$动量；4求外界对流体；5题问管/板受力则取反。\\\hline
“源汇涡、镜像壁、圆柱加环量、升力” & D & 1写总$\psi$或$\varphi$；2壁面$\psi=C$或$v_n=0$；3求速度；4Bernoulli或$L'=\rho U\Gamma$。\\\hline
“大容器喷管、背压、阻塞、最大流量” & E & 1标$0,* ,e,b$；2比$p_b/p_0$或用面积-M；3无激波等熵；4有激波分段并换$p_{02}$。\\\hline
“楔角、外凸角、斜激波、膨胀扇” & E & 压缩查$\theta-\beta-M$；膨胀用PM；每段更新$M,p,T,p_0$，激波降总压、PM不降总压。\\\hline
“壁剪、速度分布、湍流近壁、平板阻力” & F & 1算$Re$或$u_*$；2层流写N-S简化，湍流查壁律/Moody；3边界层先判转捩；4面积别取错。\\\hline
\end{tabularx}
