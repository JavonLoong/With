...
\idq{题图识别+查表+公式链}
\idq{题图一眼选模型}
\renewcommand{\arraystretch}{0.67}{\centering\begin{tabular}{|p{0.30\linewidth}|p{0.62\linewidth}|}
...
{\centering\begin{tabular}{|p{0.42\linewidth}|p{0.52\linewidth}|}\hline
...
\infotxt{%
\fmlb{\begin{gathered}
\nabla\cdot\bm V=0,\quad \nabla\times\bm V=0,\quad
\bm V=\nabla\phi,\quad \nabla^2\phi=0,\quad \nabla^2\psi=-\Omega_z\\
u=\phi_x=\psi_y,\quad v=\phi_y=-\psi_x,\quad
v_r=\phi_r=\psi_\theta/r,\quad v_\theta=\phi_\theta/r=-\psi_r\\
p=p_\infty+\tfrac12\rho(U_\infty^2-V^2),\quad C_p=1-(V/U_\infty)^2,\quad q'=\psi_2-\psi_1
\end{gathered}}
\renewcommand{\arraystretch}{0.58}{\centering\begin{tabular}{|p{0.16\linewidth}|p{0.36\linewidth}|p{0.36\linewidth}|}
...
\infoh{势流信息库B 证明链和判定链}
\infotxt{%
\textbf{证明有势函数}：无旋$\nabla\times\bm V=0$且单连通$\Rightarrow\oint\bm V\cdot d\bm l=0$，令$\phi=\int \bm V\cdot d\bm r$，故$\bm V=\nabla\phi$；再由不可压$\nabla\cdot\bm V=0$得$\nabla^2\phi=0$。\\
\textbf{证明有流函数}：二维不可压$u_x+v_y=0$，可令$u=\psi_y,v=-\psi_x$自动满足连续；若再无旋，则$\nabla^2\psi=0$。\\
\textbf{判定链}：给速度场$\to$算散度/旋度；给壁面$\to$检查$\psi=C$或$\bm V\cdot\bm n=0$；给压力$\to$先求$V^2$再Bernoulli；给升力$\to$先判有无环量。
}
\infoh{势流信息库C 常见结论库}
\infotxt{%
\renewcommand{\arraystretch}{0.58}{\centering\begin{tabular}{|p{0.31\linewidth}|p{0.60\linewidth}|}
\hline 圆柱绕流 & 均匀流+偶极，壁面$V=2U|\sin\theta|,\ C_p=1-4\sin^2\theta$；无环量$D=L=0$。\\ \hline
有环量圆柱 & $v_\theta=-2U\sin\theta+\Gamma/(2\pi a)$，$L'=\rho U\Gamma$。\\ \hline
镜像法 & 固壁要法向速度0；源/汇同号镜像，点涡反号镜像。\\ \hline
边角流 & $\phi=Ar^n\cos n\theta,\psi=Ar^n\sin n\theta,\ n=\pi/\alpha$，$V\sim r^{n-1}$。\\ \hline
Rankine半体 & 均匀流+点源，驻点$r=Q/(2\pi U)$，分界流线常取$\psi=Q/2$。\\ \hline
\end{tabular}\par}\renewcommand{\arraystretch}{1}
}
\infoh{势流信息库D 易错检查}
\infotxt{%
极坐标速度必须带$1/r$；流线是$\psi=C$不是$\phi=C$；点涡除原点外无旋但环量非零；$\psi$差是单位宽流量；半平面源汇常把$2\pi$改成$\pi$；势流只能给理想压力阻力/升力，真实阻力要看粘性分离或$C_D$；全场Bernoulli只在无旋势流成立。
...
\infotxt{%
\fmlb{\begin{gathered}
p=\rho RT,\quad a=\sqrt{\gamma RT},\quad M=V/a,\quad T_0/T=1+0.2M^2\\
p_0/p=(1+0.2M^2)^{3.5},\quad \rho_0/\rho=(1+0.2M^2)^{2.5}
\end{gathered}}
\fmlb{\begin{gathered}
\frac{T^*}{T_0}=0.833,\quad \frac{p^*}{p_0}=0.528,\quad \frac{\rho^*}{\rho_0}=0.634,\quad M=1,\ A=A^*\\
\frac{A}{A^*}=\frac1M\left(\frac{5+M^2}{6}\right)^3,\quad A/A^*>1\Rightarrow M<1\text{或}M>1\\
\dot m=\rho VA=ApM\sqrt{\gamma/(RT)}
=A p_0\sqrt{\gamma/(RT_0)}M(1+0.2M^2)^{-3}\\
\dot m_{\max}=0.0404A^*p_0/\sqrt{T_0}\quad(\text{空气，}p_0{\rm Pa},T_0{\rm K})
\end{gathered}}
}
\infoh{可压/Laval信息库B 证明链和判定链}
\infotxt{%
\textbf{等熵关系证明链}：能量给$T_0=T+V^2/(2c_p)$，代$V=Ma,\ a^2=\gamma RT,\ c_p=\gamma R/(\gamma-1)$得$T_0/T=1+(\gamma-1)M^2/2$；再用等熵$p/\rho^\gamma=C,\ p=\rho RT$得$p_0/p=(T_0/T)^{\gamma/(\gamma-1)}$。\\
\textbf{阻塞判定链}：质量流量$\dot m(M)$对$M$求极值，最大点$M=1$，故最小面积截面若达到$M=1$则阻塞；继续降背压只改变下游波系，不增大$\dot m$。\\
\textbf{背压判定链}：收缩管比$p_b/p_0$和0.528；Laval先判$A_t=A^*$，再由$A_e/A_t$求两支$M_e$，得$p_{b1},p_{b3}$，再用出口正激波得$p_{b2}$，把实际$p_b$放入区间。
}
\infoh{可压/Laval信息库C 常见结论库}
\infotxt{%
\renewcommand{\arraystretch}{0.58}{\centering\begin{tabular}{|p{0.28\linewidth}|p{0.63\linewidth}|}
\hline 亚声速面积效应 & 收缩加速、扩张减速升压。\\ \hline
超声速面积效应 & 扩张加速降压、收缩减速升压；Laval靠扩张段得到超声速。\\ \hline
收缩喷管 & 未阻塞$p_e=p_b$；阻塞时出口$M=1,p_e=p^*$，$\dot m$最大。\\ \hline
...
\infotxt{%
$p_0,T_0$是滞止量，$p,T$是静量；$p_b$是外界背压，$p_e$是出口内压，二者不总相等。$A_t$是几何喉部，阻塞时才等于$A^*$。$0.528$只表示空气$M=1$临界静压比，不是所有喉部/出口/背压比。面积--马赫数有亚/超两支，必须写明支路。压强用绝压，温度用K；液体管路Bernoulli/H--P不能直接套到可压喷管。
}
...
{\centering\begin{tabular}{|p{0.28\linewidth}|p{0.63\linewidth}|}\hline
...
{\centering\begin{tabular}{|p{0.21\linewidth}|p{0.70\linewidth}|}\hline
...
\renewcommand{\arraystretch}{0.76}{\centering\begin{tabular}{|p{0.30\linewidth}|p{0.61\linewidth}|}
...
\lookq{可压缩题查表入口}
{\centering\begin{tabular}{|p{0.29\linewidth}|p{0.29\linewidth}|p{0.34\linewidth}|}\hline
题眼 & 查书/表 & 查完怎么接\\\hline
...
\infotxt{%
\fmlb{\begin{gathered}
M_2^2=\frac{1+(\gamma-1)M_1^2/2}{\gamma M_1^2-(\gamma-1)/2},\quad
...
\mu=\sin^{-1}(1/M)\\
\delta=\nu(M_2)-\nu(M_1),\quad
\nu_{\rm air}(M)=\sqrt6\tan^{-1}\sqrt{(M^2-1)/6}\\
-\tan^{-1}\sqrt{M^2-1}
\end{gathered}}
}
\infoh{激波/PM信息库B 证明链和判定链}
\infotxt{%
\textbf{正激波证明链}：在固结于激波的坐标系中，用一维质量、法向动量、能量守恒，加理想气体状态方程，消去速度和密度，得到$M_2,p_2/p_1,\rho_2/\rho_1,T_2/T_1$；过程绝热但不可逆，所以$T_0$不变、$p_0$下降。\\
\textbf{斜激波判定链}：只让法向速度分量经历正激波，切向分量不变；所以先算$M_{n1}=M_1\sin\beta$，再用正激波表，最后用几何关系还原$M_2$。\\
\textbf{PM判定链}：超声速流遇凸角/扩张，连续弱扰动等熵展开，$\nu(M)$随$M$增大；压缩角不用PM，转斜激波。
}
...
Laval内激波 & 波前等熵到$M_1$，过波得$M_2,p_{02}$，波后用新总压等熵到出口。\\ \hline
管外波 & $p_e>p_b$欠膨胀$\to$管外膨胀；$p_e<p_b$过膨胀$\to$管外压缩/斜激波。\\ \hline
\end{tabular}\par}\renewcommand{\arraystretch}{1}
}
\infoh{激波/PM信息库D 易错检查}
\infotxt{%
正激波后一定$M_2<1$，斜激波后不一定；斜激波压力比只用$M_{n1}$，不能把$M_1$直接代正激波式；PM只用于膨胀，压缩不能用PM函数；激波后不能沿用波前$p_{01}$；表格角度多为度，计算器确认DEG；给$M_2$也不能直接算全部，静量仍要接波前$p_1,T_1,\rho_1$。
...
\lookq{粘性/阻力查表入口}
{\centering\begin{tabular}{|p{0.31\linewidth}|p{0.28\linewidth}|p{0.33\linewidth}|}\hline
题眼 & 查书/图 & 查完怎么接\\\hline
...
{\centering\begin{tabular}{|p{0.27\linewidth}|p{0.66\linewidth}|}\hline
...
\subq{真题：应力张量与指定平面应力矢量计算}
流体某点应力张量为 $P = \begin{pmatrix} -7 & 0 & 2 \\ 0 & -5 & 0 \\ 2 & 0 & -4 \end{pmatrix}$（单位 kPa），某受压面外法向为 $\bm n = (\frac{2}{3}, -\frac{2}{3}, \frac{1}{3})^T$。求该面上的应力矢量 $\bm t_n$、法向应力标量 $\sigma_n$、法向与切向应力分量 $\bm t_N,\bm t_T$ 及大小 $\tau$，以及夹角 $\alpha$。\\
解：1. 应力矢量：$\bm t_n = P \bm n = (-4, 10/3, 0)^T$ kPa。
2. 法向应力：$\sigma_n = \bm t_n \cdot \bm n = -44/9 \approx -4.89$ kPa (压应力)。法向分量 $\bm t_N = \sigma_n \bm n = (-3.26, 3.26, -1.63)^T$ kPa。
3. 切向分量：$\bm t_T = \bm t_n - \sigma_n \bm n = (-0.74, 0.07, 1.63)^T$ kPa。大小 $\tau = |\bm t_T| = 1.79$ kPa。
4. 夹角：$\cos\alpha = \sigma_n/|\bm t_n| = \frac{-4.89}{\sqrt{16+100/9}} = -0.939 \Rightarrow \alpha \approx 159.9^\circ$。
...
\subq{真题：倾斜平面闸门静水总压力与铰链拉力计算}
水池侧壁有倾斜矩形平板闸门，宽 $b=2$m，长 $L=3$m，倾角 $\theta=60^\circ$。铰链设于顶端（水深 $h_1=1$m处），底端由水平拉线拉住，求静水压力 $F$、压力中心沿板长坐标 $y_p$ 及拉线张力 $T$。\\
解：1. 顶端沿板面距水面交线为 $y_1 = h_1/\sin 60^\circ = 1.155$m。
形心沿板面坐标 $y_C = y_1 + L/2 = 2.655$m。形心深度 $h_C = y_C \sin 60^\circ = 2.30$m。
2. 静水总压力（闸门面积 $A = b \times L = 6$m$^2$）：
\fmlb{F = \rho g h_C A = 1000 \times 9.81 \times 2.30 \times 6 = 135.4\text{ kN}}
3. 压力中心（矩形 $I_C = \frac{b L^3}{12} = 4.5$m$^4$）：
\fmlb{y_p = y_C + \frac{I_C}{y_C A} = 2.655 + \frac{4.5}{2.655 \times 6} = 2.938\text{ m}}
压力中心至铰链距离 $L_p = y_p - y_1 = 1.783$m。
4. 铰链力矩平衡（拉力 $T$ 的垂直力臂为 $L \sin 60^\circ = 2.60$m）：
\fmlb{\sum M_{\text{hinge}} = F \times L_p - T \times (L \sin 60^\circ) = 0 \Rightarrow T = \frac{135.4 \times 1.783}{3 \sin 60^\circ} = 92.9\text{ kN}}
\subq{常用几何：静水压和量纲题会用}
...
{\centering\begin{tabular}{|p{0.28\linewidth}|p{0.66\linewidth}|}\hline
