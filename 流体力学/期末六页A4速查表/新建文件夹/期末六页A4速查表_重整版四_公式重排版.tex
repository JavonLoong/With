...
\infotxt{%
1. \key{连续介质与物理性质}：将流体视为质点连续无间隙介质。温度升高，\warn{液体粘度下降}，\warn{气体粘度上升}。任何流体处于静止（或平衡）状态下\warn{均不能承受切应力}，一旦受剪切力必产生连续变形。理想流体压强垂直于作用面并指向内法线，大小与方位无关；粘性流体压强为三个主应力算术平均值的负值（即 $p = -(\sigma_x+\sigma_y+\sigma_z)/3$），在静止或无粘（理想）时二者合一。水滴与水中气泡内部压强总是大于外部（$\Delta p = 2\sigma/R$），极细测压管读数须修正毛细高度（$h = \frac{4\sigma}{\rho g d}$）。牙膏、油漆、煤泥水低于屈服应力时不流动，属于宾汉塑性流体。不可压声速为无穷大；$Ma < 0.3$ 可忽略压缩性。\\
2. \key{流线迹线与流动描述}：在\warn{定常流动中流线、迹线、脉线完全重合}，且当地（时变）加速度为 0。定常与非定常流动\warn{与坐标系选取有关}（如物体在静止流体中匀速运动，静止系中为非定常，动系中为定常）。非定常中从固定点连续施放颜色液形成的线为\warn{脉线}（而非迹线）。染色流体质点随流动变形形成的线代表\warn{流体线}（而非流线）。自转角速度为涡量的\warn{一半}，速度梯度不为零时不一定会变形，如刚体旋转有速度梯度但应变率为零。\\
3. \key{三大基本方程基本概念}：连续性方程 $\nabla\cdot\bm V=0$ 虽与时间无关，但适用于\warn{不可压流动}（无论定常或非定常）。动量方程与连续方程源自守恒律，\warn{既适用于理想流体，也适用于粘性流体}（外力可包含粘性切应力）。等压面是水平面\warn{不代表只受重力}（如仅有垂直方向加速度的容器中也是水平面）。等压面必垂直于合外力。管嘴出流量大于孔口，因收缩断面有真空起抽吸作用。渐变流\warn{静压仍满足静力学规律}。以 \warn{$e/H \le 0.65$} 划分堰流与闸孔出流。\\
4. \key{无粘流体动力学}：理想流体无粘性（$\mu=0$），N-S方程与无粘流体Euler方程\warn{均为非线性}偏微分方程（均含有对流加速度项$(\bm V\cdot\nabla)\bm V$）。伯努利方程沿流线成立条件为定常、不可压、无粘。若全场无旋，则伯努利常数全场相等。三维理想流体中因涡线拉伸弯曲，\warn{涡量沿流线通常不守恒}，仅二维理想不可压中守恒。\\
5. \key{水击、空化与虹吸}：水击是流速突变由\warn{流体可压缩性与管壁弹性}引起的高频压力波。空化是局部流速高导致静压降至\warn{饱和蒸汽压以下}产生汽泡，溃灭破坏金属表面。虹吸管动力是重力差，但靠大气压支撑越过高点，理论最大高度约 10.3 米（防空化通常小于 7-8 米）。防护包括延长关阀时间、加装安全阀/调压塔。\\6. \key{潜水艇稳定性与附加质量}：潜水艇完全潜没重心低于浮心即绝对稳定；漂浮物轮船重心可高于浮心，但定倾中心必须高于重心。非定常流中存在\warn{附加质量}，固体加速运动须带动周围流体而表现为等效质量增加（启动/骤闭时显著）。
}

\infoh{选择判断秒杀库-2. (第5章) 理想不可压缩流体的二维无旋和有旋流动}
\infotxt{%
1. \key{流函数存在条件}：\warn{二维}（平面或轴对称）且\warn{不可压缩}流动。源于连续性方程（质量守恒），粘性流体只要满足该条件\warn{也有流函数$\psi$}，其存在与有无粘性、有旋无旋、定常与否无关。\\
2. \key{势函数存在条件}：流动\warn{无旋}（旋度 $\nabla\times\bm V=0$）。源于无旋定义，\warn{与维数、压缩性、有无粘性、定常与否无关}。\\
3. \key{混淆要点}：非定常不可压流场可能存在流函数（二维即存在）；有流函数不一定有势函数（可能有旋）。二维不可压有旋流动有流函数但无势函数；三维无旋流动有势函数但无二维流函数；有粘流动在局部无旋时也可以存在势函数。二维无旋流动中，\warn{流线与等势线总是正交的}。\\
4. \key{圆柱绕流与升力}：无环量无阻力无升力（达朗贝尔佯谬）；有速度环量 $\Gamma$ 产生升力 $L=\rho V \Gamma$（库塔-儒可夫斯基定理）。\key{K-J假设（后缘条件）}：在有攻角绕流机翼/平板时，理想流体解在后缘处速度为无穷大，实际流体因粘性后缘速度有限。假设存在一个恰当的环量 $\Gamma$，使背面驻点移至后缘，保证后缘流速有限。
}

\infoh{选择判断秒杀库-3. (第7章) 气体动力学基础}
\infotxt{%
1. \key{声速与马赫数}：声速 $c=\sqrt{\gamma RT}$ 仅是温度的函数。马赫数 $Ma=V/c$ 反映可压缩程度，当 $Ma < 0.3$ 时可近似为不可压缩流动。声波传播极快，\warn{为等熵过程，而非等温过程}。\\
2. \key{等熵滞止参考状态}：一维等熵气流的滞止参数（$p_0, T_0$ 等）\warn{与坐标系选取有关}（因流速具相对性）。\\
3. \key{喉部阻塞与最大流量}：喷管喉部达到声速（$Ma=1$）时发生喉部壅塞。一旦发生阻塞，最大质量流量 $\dot{m}_{\max}$ 仅由喉部面积和上游滞止状态（$p_0, T_0$）唯一决定，\warn{完全不受下游背压变化影响}。管内有正激波或出口有膨胀波时不改变此最大流量。\\
4. \key{正激波与膨胀波状态变化}：气体通过正激波法向减速。其\warn{静压、密度、静温、静焓、熵均上升}，总温、总焓不变，但由于熵增导致\warn{总压（滞止压强）下降}。气体通过 Prandtl-Meyer 膨胀波为等熵过程，总压、总温不变，但静压、静温、密度下降，马赫数增大。
}

\infoh{选择判断秒杀库-4. (第8章) 粘性流体力学基础}
\infotxt{%
1. \key{雷诺数物理本质}：雷诺数 $Re = \rho VL / \mu$ 是惯性力与粘性力之比。\\
  (1) \warn{惯性力尺度}：随体/对流加速度项引起的惯性力 $\rho (\bm V\cdot\nabla)\bm V \sim \rho V^2 / L$。\\
  (2) \warn{粘性力尺度}：由于剪切变形引起的粘性摩擦力 $\mu \nabla^2\bm V \sim \mu V / L^2$。\\
2. \key{低雷诺数流动（$Re < 1$ 斯托克斯流）}：忽略惯性项，由线性 Stokes 方程控制，\warn{绝不能使用无粘 Euler 方程}。小球总阻力 $D = 3\pi \mu d U$，其中\warn{摩擦阻力占 2/3}，压差阻力占 1/3，无任何流动分离。在极低雷诺数下，流线型物体因湿面积大导致摩擦阻力大，总阻力可能大于圆球。\\
3. \key{粘性流动解析解}：Couette流（剪切驱动，无压梯度下速度呈线性分布）；Poiseuille流（压力驱动，速度呈抛物线分布）。两层粘性流体交界面上，\warn{剪切应力与速度必须连续}。圆管完全发展段中速度剖面恒定，\warn{沿程压力梯度 $\frac{dp}{dx}$ 为常数}。光滑管替换不一定显著增加总流量（若局部阻力占主导或处于水力光滑区，改变粗糙度无用）。\\
4. \key{当量直径与动力相似}：当量直径 $d_e = 4 R_h = 4 A / \chi$（水力半径 $R_h = A / \chi$，即过流面积比湿周），作用是将非圆截面折算为圆管阻力计算。重力主导选 Fr 相似，粘性主导选 Re 相似。模型试验中通常无法在普通风洞/水槽中\warn{同时满足雷诺数 $Re$ 与弗劳德数 $Fr$ 相似}（因两者的速度缩放比例要求冲突）。量纲分析中只要选取的重复变量在量纲上相互独立即可（如可取 $\rho,V,L$ 或 $p,V,L$）。阻力平方区自模化时阻力系数与 Re 无关。欧拉数 $Eu = \Delta p / (\rho V^2)$ 是压力与惯性力之比，$Eu \gg 1$ 代表压力起主导作用。
}

\infoh{选择判断秒杀库-5. (第9章) 湍流}
\infotxt{%
1. \key{转捩与稳定性}：流态由层流向湍流转捩的临界雷诺数受壁面粗糙度和外界干扰影响，\warn{不是确定的常数}。\\
2. \key{雷诺应力与混合长度}：RANS时均方程由于脉动引入的\warn{雷诺应力项而导致方程不封闭}，须引入半经验模型。紊流圆管中总切应力沿断面呈\warn{线性分布}。Prandtl\key{混合长度理论}物理意义：将湍流微团脉动与气体分子运动相类比，假设微团横向脉动移动距离 $l$（混合长度）后与周围流体混合并传递时均物理量。\\
3. \key{数值模拟方法对比}：\warn{DNS（直接模拟）}不加近似，网格极密，计算量最大仅低Re；\warn{LES（大涡模拟）}大涡求解，小涡SGS模型近似，计算量中等；\warn{RANS（时均方程）}只求时均，脉动用湍流模型闭合，计算量最小最实用。
}

\infoh{选择判断秒杀库-6. (第10章) 边界层理论基础}
\infotxt{%
1. \key{边界层外界线与求解步骤}：由名义厚度 $\delta$ 标定的边界层外界线\warn{不是流线}（有外部流体跨越进入）。排挤厚度 $\delta^*$ 和动量损失厚度 $\theta$ \warn{均显著小于名义厚度 $\delta$}。平板层流阻力求解：设速度分布 $\to$ 求 $\theta,\tau_w$ 与 $\delta$ 关系 $\to$ 代入冯卡门积分方程 $\to$ 积分求出 $\delta(x)$ 及摩擦阻力。摩擦阻力是流体与物体相对运动时，流体粘性在接触面产生的剪切应力合力。\\
2. \key{流动分离与卡门涡街}：绕流钝体（如圆柱）两侧边界层因\warn{逆压梯度与粘性摩擦}脱流，剪切层不稳定，交替脱落形成卡门涡街。流动分离的两个必要条件：\warn{存在逆压梯度区}及\warn{壁面与粘性对流体的阻滞作用}。释放频率 $f = St \frac{U}{d}$，阻力危机前 $St \approx 0.2$。高Re流线体绕流在后部收缩段\warn{仍会出现逆压区}，虽能延迟分离，但在大攻角等工况下仍可能发生分离。\\
3. \key{阻力危机}：球/圆柱在临界 $Re$ 附近边界层由层流转捩为湍流，抗逆压梯度能力增强，分离点后移、尾迹变窄，使\warn{阻力系数 $C_D$ 骤降}。
}
