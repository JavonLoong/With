...
2. \key{流场描述与运动学}：\\
{\fontspec{SimSun}①} 在定常流动中\warn{流线、迹线、脉线完全重合}。\\
{\fontspec{SimSun}②} 定常与非定常流动\warn{与坐标系选取有关}（只有当坐标系固定在物体上时周围流场才是定常流场）。\\
{\fontspec{SimSun}③} 在非定常流场中，从固定点连续施放颜色液形成的线是\warn{脉线}（而非流线或迹线）。\\
{\fontspec{SimSun}④} 染色流体质点随时间增长变形形成的抛物线代表\warn{流体线}（而非流线或速度分布）。\\
{\fontspec{SimSun}⑤} 在定常流场中速度不随时间变化，但\warn{还可能有迁移（位变）加速度}，故质点加速度不恒为零。\\
{\fontspec{SimSun}⑥} 自转角速度为涡量的一半。刚体旋转有速度梯度但应变率为零。\\
3. \key{三大基本方程基本概念}：\\
{\fontspec{SimSun}①} 连续性方程 $\nabla\cdot\bm V=0$ \warn{适合不可压缩流体的任何流动}（定常/非定常均成立）。\\
{\fontspec{SimSun}②} 动量方程中的外力包含粘性切应力，\warn{既适用于理想流体，也适用于粘性流体}。\\
{\fontspec{SimSun}③} 静止流体等压面是水平面不代表只受重力，\warn{除重力外还可以有垂直方向的惯性力}（等压面必垂直于合质量力方向）。\\
{\fontspec{SimSun}④} 渐变流过流断面上静压分布仍满足静力学规律 $z+p/\rho g = \text{const}$。\\
{\fontspec{SimSun}⑤} 管嘴出流量大于孔口，因收缩断面有真空起抽吸作用。\\
{\fontspec{SimSun}⑥} 以 $e/H \le 0.65$ 划分堰流与闸孔出流。\\
4. \key{无粘流体动力学}：\\
