...
\chap{母题4 二维超声速波系：斜激波与膨胀波}
\mview{\key{母题总览}：研究超声速流绕外角、凸角或绕流的波系结构。看见偏转角/凹角/凸角/绕折线壁面流动，用本母题。首写：偏转角为压缩（凹角）$\Rightarrow$斜激波；为膨胀（凸角）$\Rightarrow$膨胀扇。检查：斜激波后 $p_0$ 降，膨胀波内 $p_0$ 不变；斜激波后不一定为亚声速。}
\schemeshock
\infoh{激波/PM信息库A 基础公式和数学运算}
\subq{正激波：已知波前M1，求波后M2、压力、温度、总压损失}
\fmlb{\begin{aligned}
Ma_2^2 &= \frac{1+\frac{\gamma-1}{2}Ma_1^2}{\gamma Ma_1^2-\frac{\gamma-1}{2}} = \frac{1+0.2Ma_1^2}{1.4Ma_1^2-0.2}\ \text{(空气)}\\
\frac{p_2}{p_1} &= 1+\frac{2\gamma}{\gamma+1}(Ma_1^2-1) = \frac{7Ma_1^2-1}{6}\ \text{(空气)}\\
\frac{\rho_2}{\rho_1} &= \frac{(\gamma+1)Ma_1^2}{(\gamma-1)Ma_1^2+2} = \frac{6Ma_1^2}{Ma_1^2+5}\ \text{(空气)}\\
\frac{T_2}{T_1} &= (p_2/p_1)/(\rho_2/\rho_1),\quad \frac{p_{02}}{p_{01}}=\frac{p_2}{p_1}\frac{p_{02}/p_2}{p_{01}/p_1}
\end{aligned}}
...
&= \sqrt{6}\tan^{-1}\sqrt{(Ma^2-1)/6} - \tan^{-1}\sqrt{Ma^2-1}\ \text{(空气)}
\end{aligned}}

\infoh{激波/PM信息库B 判定链与计算流程}
\stepq{斜激波计算流程}
1. \key{求波角 $\beta$}：由已知 $Ma_1$ 和壁面偏转角 $\theta$，查 $\theta-\beta-Ma$ 曲线（或利用公式试算）得到弱激波解 $\beta$。\\
2. \key{算等效法向马赫数}：$Ma_{n1} = Ma_1\sin\beta$。\\
3. \key{求法向波后状态}：将 $Ma_{n1}$ 视作正激波前马赫数，代入正激波公式或查正激波表，得到 $Ma_{n2}$、$p_2/p_1$、$T_2/T_1$ 及 $p_{02}/p_{01}$。\\
4. \key{还原斜激波后真实马赫数}：$Ma_2 = Ma_{n2}/\sin(\beta-\theta)$。压力比、温度比直接等于法向对应比值，总压损失为 $p_{02} = p_{01} (p_{02}/p_{01})$。

\stepq{Prandtl-Meyer 膨胀波计算流程}
1. \key{求膨胀角 $\delta$}：气流转向流体外侧的壁面偏转角 $\delta$。\\
2. \key{计算波后 PM 函数值}：查表或计算波前马赫数对应的 $\nu(Ma_1)$，则波后为：$\nu(Ma_2) = \nu(Ma_1) + \delta$。\\
3. \key{求波后状态}：由 $\nu(Ma_2)$ 反查/计算波后马赫数 $Ma_2$。因膨胀波为等熵过程，总压不变 $p_{02} = p_{01}$，总温不变 $T_{02}=T_{01}$。\\
4. \key{求静压静温}：$\frac{p_2}{p_1} = \left[\frac{1+0.2Ma_1^2}{1+0.2Ma_2^2}\right]^{3.5}$，$\frac{T_2}{T_1} = \frac{1+0.2Ma_1^2}{1+0.2Ma_2^2}$。

\infoh{激波/PM信息库C 难点大综合与常考物理问答}
\subq{激波反射与交汇波系分析（多波计算）}
* \key{反射波/交汇流面规律}：超声速流沿折线壁面流动时，每个压缩角生成一道斜激波，每个扩张角生成一簇膨胀扇。逐段更新状态，前段波后状态即为后段波前状态。
* \key{接触面边界条件}：两股不同流向的超声速气流交汇时，中间会形成接触面（Slipstream）。两侧的静压必须相等（$p_{2a}=p_{2b}$），且流动方向一致（$\theta_a = \theta_b$）。
\subq{运动正激波与撞墙反射}
* 坐标系固结在激波上：来流以相对速度 $V_{r1} = V_{\text{波}} + V_1$ 迎面吹来，过入射激波求得相对波后速度 $V_{r2}$，换算回绝对坐标系。
* 撞墙反射：反射激波需使波后气流在墙面处的绝对速度为 $0$（即相对反射波的速度等于反射波传播速度）。
\subq{脱体激波判定}
若壁面偏转角 $\theta > \theta_{\max}(Ma_1)$，则斜激波无法附着在楔尖，会在前方形成脱体弓形激波，前缘附近流速变为亚声速。
\subq{概念简答：激波与膨胀波物理特征}
* 激波：强间断、非等熵过程。波后 $Ma_2$ 降，压力 $p$、温度 $T$、密度 $\rho$ 及熵 $s$ 骤升。非等熵导致总压损失，产生波阻。正激波后必为亚声速；斜激波后强解为亚声速，弱解多为超声速。
* 膨胀波：由无数微弱扰动线（马赫线 $\mu = \sin^{-1}(1/Ma)$）组成的连续膨胀扇，为等熵过程，总压 $p_0$ 及总温 $T_0$ 保持不变。气体通过膨胀扇后马赫数 $Ma$ 升高，静压 $p$、温度 $T$ 下降。
...
\lookq{小查表：空气正激波对照表}
$Ma_1=1.5\Rightarrow Ma_2=0.701,\ p_2/p_1=2.46,\ p_{02}/p_{01}=0.93$ \\
$Ma_1=2.0\Rightarrow Ma_2=0.577,\ p_2/p_1=4.50,\ p_{02}/p_{01}=0.72$ \\
$Ma_1=3.0\Rightarrow Ma_2=0.475,\ p_2/p_1=10.33,\ p_{02}/p_{01}=0.33$
